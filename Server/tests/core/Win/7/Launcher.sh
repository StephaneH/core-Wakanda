...
WORKSPACE=$(echo "$1" | sed 's/\\/\//g')
BUILD_TEST_DIR=$(echo "$2" | sed 's/\\/\//g')
WAKANDA_SERVER_PATH=$(echo "$3" | sed 's/\\/\//g')

source "${WORKSPACE}/Config.sh" "${WORKSPACE}" "${BUILD_TEST_DIR}"
...
if [ -e "${HomeDirectory}${Directory}global_log_${DATELOG}" ]
then
...
cd "${WORKSPACE}"
./Main.sh "${WORKSPACE}" "${BUILD_TEST_DIR}" "${WAKANDA_SERVER_PATH}" 2>&1 >"$LOGFILE" ; cat "$LOGFILE"
echo "[INFO]-------------------------------------------------------------------------------------------"
