...
public class TestCasesTabletRouting_HTTPS extends SeleniumRuntimeTemplate {
	...
	public static void beforeClass() throws URISyntaxException, IOException 
	{
		...
		AdminCommand.startServer(TestCasesTabletRouting_HTTPS.class, "/solutions/waPageRouting/waPageRouting Solution/waPageRouting.waSolution");
	}
	...
	public void test_indexHtml() throws InterruptedException {
		selenium.getDriver().get("https://127.0.0.1:4444/index.waPage/index-tablet.html?cacheBreaker"+System.currentTimeMillis());
			try{
		selenium.getDriver().findElement(By.id("tabletBtt"));
			}catch(Exception e){
		assertEquals("Fail to get https://127.0.0.1:4444/index.waPage/index-tablet.html : ", true, false);		
			}
		Thread.sleep(500);
		String result = (String) selenium.getJsConsole().executeScript("return $$('tabletBtt').getPosition().left +'/'+ $$('tabletBtt').getPosition().top");
		assertEquals("Check if tabletBtt has right index.css : ", "20/20", result);		
