...
function generateJSON()
{
	...
	var jsonResult =
	{
		...
		"value" : 
		[
			env.WORKSPACE + "/file.js", 
			env.WORKSPACE + "/args-solution/solutionArgs Solution/solutionArgs.waSolution",
			"foo"
		],
		"separator" : 
		[
	...
	for (var jsonI = 0; jsonI < allCase.length;jsonI++)
	{
		for (var jsonI0 = 0; jsonI0 < jsonResult.separator.length;jsonI0++) 
		{
			...
			{	
				...
				if ((allCase[jsonI].match(/^(-s)/g) !=null && (jsonResult.separator[jsonI0] == " " || jsonResult.separator[jsonI0] == "=")))
					var linux = mac = true;
				else
					var linux = mac = false;

				if ((allCase[jsonI].match(/^(-s)/g) !=null && (jsonResult.separator[jsonI0] == " " || jsonResult.separator[jsonI0] == "=" || jsonResult.separator[jsonI0] == ":")) || (allCase[jsonI].match(/^(\/s)/g) !=null && (jsonResult.separator[jsonI0] == " " || jsonResult.separator[jsonI0] == "=" || jsonResult.separator[jsonI0] == ":")))
					var win = true;
				else	
					var win = false;

					jsonResult.fileLogic.push(
					{
						"case" : caseU,
						"ae":
						{
...
var fileLogicCase = generateJSON();
...
// --***-- Test to check that fileLogic (windows : , Linux/Mac : ) exist
testCLIargs_fileOptionLogicXExist: function() 
{
...
*/
	console.log("ServerPath :" + " " + newServerBin.path); 
 	...
 	//Windows execution tests
		...
		//var optionFile = new SystemWorker(["cmd","/C","{binFile}",caseObject.ae.key],options);
		var optionFile = new SystemWorker(cmd);
		console.log("Args to test : " + caseObject.ae.key);
		//console.log("optionFile value object : "+ JSON.stringify(optionFile)); 

		if (optionFile != null || optionFile != "undefined") 
		{
			optionFile.onmessage = function(outputText)
			{
			...
			optionFile.onerror = function(errorText)
			{
				console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>onerror>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			...
			optionFile.onterminated = function(terminatedText) {
			...
			{
				wait(5000);
				optionFile.terminate(true,true);
				wait(5000);
			} 
			else {
				wait()
			...
			{
				console.log("File Option is : " + outputMessage);
				//Asserts 
				...
				Y.Assert.isNotNull(optionFile);
				Y.Assert.isObject(optionFile);
			...
			{ 
				console.log("File Option failed : " + outputError);
				//Asserts 
				Y.Assert.areSame(statusCodeOS(caseObject.ae.expected.win),
					outputExitStatus,
			...
			console.log("File Option is null or undefined");
	} 
	else 
	{ 
...
/* Generated tests for Logic optionFile */

//Attention, different scope for() and function()
for (var hl = 0; hl < fileLogicCase.fileLogic.length; hl++) 
	{
		testCase["testCLIargs_fileOptionLogic_" + fileLogicCase.fileLogic[hl].case + "_" + argformat(fileLogicCase.fileLogic[hl].ae.key)] = function() 
		{

			var phl0 = fileLogicCase.fileLogic[index];
			var phl1 = fileLogicCase.name;
			var phl2 = fileLogicCase.keys;
			var phl3 = argformat(fileLogicCase.fileLogic[index].ae.key); 
			var phl4 = fileLogicCase.fileLogic[index].config; 
			var phl5 = fileLogicCase.fileLogic[index].ae.separator;
			var phl6 = fileLogicCase.fileLogic[index].ae.value;
