	...
	for (var jsonI = 0; jsonI < allCase.length;jsonI++)
	{
		for (var jsonI0 = 0; jsonI0 < jsonResult.separator.length;jsonI0++) 
		{
			...
			{	
				...
				if ((allCase[jsonI].match(/^(--job-id)/g) !=null && (jsonResult.separator[jsonI0] == " " || jsonResult.separator[jsonI0] == "=")))
					var linux = mac = true;
				else
					var linux = mac = false;

				if ((allCase[jsonI].match(/^(--job-id)/g) !=null && (jsonResult.separator[jsonI0] == " " || jsonResult.separator[jsonI0] == "=" || jsonResult.separator[jsonI0] == ":")) || (allCase[jsonI].match(/^(\/job-id)/g) !=null && (jsonResult.separator[jsonI0] == " " || jsonResult.separator[jsonI0] == "=" || jsonResult.separator[jsonI0] == ":")))
					var win = true;
				else	
					var win = false;

					jsonResult.jobLogic.push(
					{
						"case" : caseU,
						"ae":
						{
...
// --***-- Test to check that jobsLogic (windows : , Linux/Mac : ) exist
testCLIargs_jobsOptionLogicXExist: function() 
{
...
*/
	console.log("ServerPath :" + " " + newServerBin.path); 
 	...
 	//Windows execution tests
		...
		//var optionJob = new SystemWorker(["cmd","/C","{binFile}",caseObject.ae.key],options);
		var optionJob = new SystemWorker(cmd);
		console.log("Args to test : " + caseObject.ae.key);
		//console.log("optionJob value object : "+ JSON.stringify(optionJob)); 

		if (optionJob != null || optionJob != "undefined") 
		{
			optionJob.onmessage = function(outputText)
			{
			...
			optionJob.onerror = function(errorText)
			{
				console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>onerror>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			...
			optionJob.onterminated = function(terminatedText) {
			...
			{
				console.log("Job Option is : " + outputMessage);
				//Asserts 
				...
				Y.Assert.isNotNull(optionJob);
				Y.Assert.isObject(optionJob);
			...
			{ 
				console.log("Job Option failed : " + outputError);
				//Asserts 
				Y.Assert.areSame(statusCodeOS(caseObject.ae.expected.win),
					outputExitStatus,
			...
			console.log("Job Option is null or undefined");
	} 
	else 
	{ 
...
/* Generated tests for Logic optionJobs */

//Attention, different scope for() and function()
for (var hl = 0; hl < jobLogicCase.jobLogic.length; hl++) 
	{
		testCase["testCLIargs_jobsOptionLogic_" + jobLogicCase.jobLogic[hl].case + "_" + argformat(jobLogicCase.jobLogic[hl].ae.key)] = function() 
		{
	
			var phl0 = jobLogicCase.jobLogic[index];
			var phl1 = jobLogicCase.name;
			var phl2 = jobLogicCase.keys;
			var phl3 = argformat(jobLogicCase.jobLogic[index].ae.key); 
			var phl4 = jobLogicCase.jobLogic[index].config; 
			var phl5 = jobLogicCase.jobLogic[index].ae.separator;
			var phl6 = jobLogicCase.jobLogic[index].ae.value;
