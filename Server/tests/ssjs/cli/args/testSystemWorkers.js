...
function generateJSON()
{
	...
	var jsonResult =
	{
		...
		"value" : 
		[
			env.WORKSPACE + "/systemWorkers.json",
			"foo"
		],
		"separator" : 
		[
	...
	for (var jsonI = 0; jsonI < allCase.length;jsonI++)
	{
		for (var jsonI0 = 0; jsonI0 < jsonResult.separator.length;jsonI0++) 
		{
			...
			{	
				...
				if ((allCase[jsonI].match(/^(--system-workers)/g) != null && (jsonResult.separator[jsonI0] == " " || jsonResult.separator[jsonI0] == "=")) && jsonResult.value[jsonI1] != "foo")
					var linux = mac = true;
				else
					var linux = mac = false;

				if ((allCase[jsonI].match(/^(--system-workers)/g) != null && (jsonResult.separator[jsonI0] == " " || jsonResult.separator[jsonI0] == "=" || jsonResult.separator[jsonI0] == ":")) || (allCase[jsonI].match(/^(\/system-workers)/g) !=null && (jsonResult.separator[jsonI0] == " " || jsonResult.separator[jsonI0] == "=" || jsonResult.separator[jsonI0] == ":")) && jsonResult.value[jsonI1] != "foo")
					var win = true;
				else	
					var win = false;

					jsonResult.systemWorkerLogic.push(
					{
						"case" : caseU,
						"ae":
						{
...
var systemWorkerLogicCase = generateJSON();
...
// --***-- Test to check that systemWorkerLogic (windows : , Linux/Mac : ) exist
testCLIargs_systemWorkerOptionLogicXExist: function() 
{
...
*/
	console.log("ServerPath :" + " " + newServerBin.path); 
 	...
 	//Windows execution tests
		...
		//var optionSystemWorker = new SystemWorker(["cmd","/C","{binFile}",caseObject.ae.key],options);
		var optionSystemWorker = new SystemWorker(cmd);
		console.log("Args to test : " + caseObject.ae.key);
		//console.log("optionSystemWorker value object : "+ JSON.stringify(optionSystemWorker)); 

		if (optionSystemWorker != null || optionSystemWorker != "undefined") 
		{
			optionSystemWorker.onmessage = function(outputText)
			{
			...
			optionSystemWorker.onerror = function(errorText)
			{
				console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>onerror>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			...
			optionSystemWorker.onterminated = function(terminatedText) {
			...
			{
				wait(5000);
				optionSystemWorker.terminate(true,true);
				wait(5000);
			} 
			else {
				wait()
			...
			{
				console.log("SystemWorkers Option is : " + outputMessage);
				//Asserts 
				...
				Y.Assert.isNotNull(optionSystemWorker);
				Y.Assert.isObject(optionSystemWorker);
			...
			{ 
				console.log("SystemWorkers Option failed : " + outputError);
				//Asserts 
				Y.Assert.areSame(statusCodeOS(caseObject.ae.expected.win),
					outputExitStatus,
			...
			console.log("SystemWorkers Option is null or undefined");
	} 
	else 
	{ 
		...
		if (optionSystemWorker != null || optionSystemWorker != "undefined") 
		{
			...
			{ 
				...
				if (os.isLinux)
					Y.Assert.areSame(statusCodeOS(caseObject.ae.expected.linux),
						outputExitStatus,
						"Args type : "+ keys +", Linux : "+ caseObject.ae.arg +", format : "+ format +" : Error code value is different than "+ statusCodeOS(caseObject.ae.expected.linux) + ", "+ caseObject.ae.arg + " work or exist. The code returned is : "+ outputExitStatus +".");
				else
					Y.Assert.areSame(statusCodeOS(caseObject.ae.expected.mac),
						outputExitStatus,
						"Args type : "+ keys +", Mac : "+ caseObject.ae.arg +", format : "+ format +" : Error code value is different than "+ statusCodeOS(caseObject.ae.expected.mac) + ", "+ caseObject.ae.arg + " work or exist. The code returned is : "+ outputExitStatus +".");
				Y.Assert.isNotNull(optionSystemWorker);
				Y.Assert.isObject(optionSystemWorker);
				Y.Assert.areSame('',outputMessage);
				Y.Assert.isNotNull(outputError); 
...
var testCase = {
	name: "test CLI args SystemWorker",
	_should: {
		error: {},
		ignore: {}
	},
...
for (var hl = 0; hl < systemWorkerLogicCase.systemWorkerLogic.length; hl++) 
	{
		testCase["testCLIargs_systemWorkerOptionLogic_" + systemWorkerLogicCase.systemWorkerLogic[hl].case + "_" + argformat(systemWorkerLogicCase.systemWorkerLogic[hl].ae.key)] = function() 
		{

			var phl0 = systemWorkerLogicCase.systemWorkerLogic[index];
			var phl1 = systemWorkerLogicCase.name;
			var phl2 = systemWorkerLogicCase.keys;
			var phl3 = argformat(systemWorkerLogicCase.systemWorkerLogic[index].ae.key); 
			var phl4 = systemWorkerLogicCase.systemWorkerLogic[index].config; 
			var phl5 = systemWorkerLogicCase.systemWorkerLogic[index].ae.separator;
			var phl6 = systemWorkerLogicCase.systemWorkerLogic[index].ae.value;
