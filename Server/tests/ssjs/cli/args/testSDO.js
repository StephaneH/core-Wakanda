	...
	for (var jsonI = 0; jsonI < allCase.length;jsonI++)
	{
		for (var jsonI0 = 0; jsonI0 < jsonResult.separator.length;jsonI0++) 
		{
			...
			{	
				...
				if (allCase[jsonI].match(/^(--no-discovery)/g) != null && jsonResult.separator[jsonI0] == null)
					var linux = mac = true;
				else
					var linux = mac = false;

				if ((allCase[jsonI].match(/^(--no-discovery)/g) != null && jsonResult.separator[jsonI0] == null) || (allCase[jsonI].match(/^(\/no-discovery)/g) !=null && jsonResult.separator[jsonI0] == null))
					var win = true;
				else	
					var win = false;

					jsonResult.sdoLogic.push(
					{
						"case" : caseU,
						"ae":
						{
...
// --***-- Test to check that sdoLogic (windows : , Linux/Mac : ) exist
testCLIargs_sdoOptionLogicXExist: function() 
{
...
*/
	console.log("ServerPath :" + " " + newServerBin.path); 
 	...
 	//Windows execution tests
		...
		//var optionSDO = new SystemWorker(["cmd","/C","{binFile}",caseObject.ae.key],options);
		var optionSDO = new SystemWorker(cmd);
		console.log("Args to test : " + caseObject.ae.key);
		//console.log("optionSDO value object : "+ JSON.stringify(optionSDO)); 

		if (optionSDO != null || optionSDO != "undefined") 
		{
			optionSDO.onmessage = function(outputText)
			{
			...
			optionSDO.onerror = function(errorText)
			{
				console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>onerror>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			...
			optionSDO.onterminated = function(terminatedText) {
			...
			{
				console.log("SDO Option is : " + outputMessage);
				//Asserts 
				...
				Y.Assert.isNotNull(optionSDO);
				Y.Assert.isObject(optionSDO);
			...
			{ 
				console.log("SDO Option failed : " + outputError);
				//Asserts 
				Y.Assert.areSame(statusCodeOS(caseObject.ae.expected.win),
					outputExitStatus,
			...
			console.log("SDO Option is null or undefined");
	} 
	else 
	{ 
...
/* Generated tests for Logic optionSDO : Service discovery options */

//Attention, different scope for() and function()
for (var hl = 0; hl < sdoLogicCase.sdoLogic.length; hl++) 
	{
		testCase["testCLIargs_SDOOptionLogic_" + sdoLogicCase.sdoLogic[hl].case + "_" + argformat(sdoLogicCase.sdoLogic[hl].ae.key)] = function() 
		{

			var phl0 = sdoLogicCase.sdoLogic[index];
			var phl1 = sdoLogicCase.name;
			var phl2 = sdoLogicCase.keys;
			var phl3 = argformat(sdoLogicCase.sdoLogic[index].ae.key); 
			var phl4 = sdoLogicCase.sdoLogic[index].config; 
			var phl5 = sdoLogicCase.sdoLogic[index].ae.separator;
			var phl6 = sdoLogicCase.sdoLogic[index].ae.value;
