...
function generateJSON()
{
	...
	for (var jsonI = 0; jsonI < allCase.length;jsonI++)
	{
		for (var jsonI0 = 0; jsonI0 < jsonResult.separator.length;jsonI0++) 
		{
			...
			{	
				//Add tests on key value and separator
				if ((jsonResult.separator[jsonI0] == " " || jsonResult.separator[jsonI0] == "=") && (allCase[jsonI].match(/^(--d)/g) != null || allCase[jsonI].match(/^(-d)/g) != null))
				{
					var linux = true;
					var mac = win = false;
				}
...
var daemonLogicCase = generateJSON();
...
// --***-- Test to check that daemonLogic (windows : , Linux/Mac : ) exist
testCLIargs_daemonOptionLogicXExist: function() 
{
...
*/
	console.log("ServerPath :" + " " + newServerBin.path); 
 	...
 	//Windows execution tests
		...
		//var optionDaemon = new SystemWorker(["cmd","/C","{binFile}",caseObject.ae.key],options);
		var optionDaemon = new SystemWorker(cmd);
		console.log("Args to test : " + caseObject.ae.key);
		//console.log("optionDaemon value object : "+ JSON.stringify(optionDaemon)); 

		if (optionDaemon != null || optionDaemon != "undefined") 
		{
			optionDaemon.onmessage = function(outputText)
			{
			...
			optionDaemon.onerror = function(errorText)
			{
				console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>onerror>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			...
			optionDaemon.onterminated = function(terminatedText) {
			...
			{
				wait(5000);
				optionDaemon.terminate(true,true);
				wait(5000);
			} 
			else {
				wait()
			...
			{
				console.log("Daemon Option is : " + outputMessage);
				//Asserts 
				...
				Y.Assert.isNotNull(optionDaemon);
				Y.Assert.isObject(optionDaemon);
			...
			{ 
				console.log("Daemon Option failed : " + outputError);
				//Asserts 
				Y.Assert.areSame(statusCodeOS(caseObject.ae.expected.win),
					outputExitStatus,
			...
			console.log("Daemon Option is null or undefined");
	} 
	else 
	{ 
...
for (var hl = 0; hl < daemonLogicCase.daemonLogic.length; hl++) 
	{
		testCase["testCLIargs_daemonOptionLogic_" + daemonLogicCase.daemonLogic[hl].case + "_" + argformat(daemonLogicCase.daemonLogic[hl].ae.key)] = function() 
		{
			var phl0 = daemonLogicCase.daemonLogic[index];
			var phl1 = daemonLogicCase.name;
			var phl2 = daemonLogicCase.keys;
			var phl3 = argformat(daemonLogicCase.daemonLogic[index].ae.key); 
			var phl4 = daemonLogicCase.daemonLogic[index].config; 
			var phl5 = daemonLogicCase.daemonLogic[index].ae.separator;
			var phl6 = daemonLogicCase.daemonLogic[index].ae.value;
