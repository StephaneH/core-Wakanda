	...
	for (var jsonI = 0; jsonI < allCase.length;jsonI++)
	{
		for (var jsonI0 = 0; jsonI0 < jsonResult.separator.length;jsonI0++) 
		{
			...
			{	
				...
				if ((allCase[jsonI].match(/^(--syslog)/g) !=null || allCase[jsonI].match(/^(--verbose)/g)) && jsonResult.separator[jsonI0] == null)
					var linux = mac = true;
				else
					var linux = mac = false;

				if (((allCase[jsonI].match(/^(--syslog)/g) !=null || allCase[jsonI].match(/^(--verbose)/g)) || (allCase[jsonI].match(/^(\/syslog)/g) !=null || allCase[jsonI].match(/^(\/verbose)/g))) && jsonResult.separator[jsonI0] == null)
					var win = true;
				else	
					var win = false;

					jsonResult.loggingLogic.push(
					{
						"case" : caseU,
						"ae":
						{
...
var loggingLogicCase = generateJSON();
...
// --***-- Test to check that loggingLogic (windows : , Linux/Mac : ) exist
testCLIargs_loggingOptionLogicXExist: function() 
{
...
*/
	console.log("ServerPath :" + " " + newServerBin.path); 
 	...
 	//Windows execution tests
		...
		//var optionLogging = new SystemWorker(["cmd","/C","{binFile}",caseObject.ae.key],options);
		var optionLogging = new SystemWorker(cmd);
		console.log("Args to test : " + caseObject.ae.key);
		//console.log("optionLogging value object : "+ JSON.stringify(optionLogging)); 

		if (optionLogging != null || optionLogging != "undefined") 
		{
			optionLogging.onmessage = function(outputText)
			{
			...
			optionLogging.onerror = function(errorText)
			{
				console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>onerror>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			...
			optionLogging.onterminated = function(terminatedText) {
			...
			{
				wait(5000);
				optionLogging.terminate(true,true);
				wait(5000);
			} 
			else {
				wait()
			...
			{
				console.log("Logging Option is : " + outputMessage);
				//Asserts 
				...
				Y.Assert.isNotNull(optionLogging);
				Y.Assert.isObject(optionLogging);
			...
			{ 
				console.log("Logging Option failed : " + outputError);
				//Asserts 
				Y.Assert.areSame(statusCodeOS(caseObject.ae.expected.win),
					outputExitStatus,
			...
			console.log("Logging Option is null or undefined");
	} 
	else 
	{ 
...
var testCase = {
	name: "test CLI args LoggingFacility",
	_should: {
		error: {},
		ignore: {}
	},
...
/* Generated tests for Logic optionLogging */

//Attention, different scope for() and function()
for (var hl = 0; hl < loggingLogicCase.loggingLogic.length; hl++) 
	{
		testCase["testCLIargs_loggingFacilityOptionLogic_" + loggingLogicCase.loggingLogic[hl].case + "_" + argformat(loggingLogicCase.loggingLogic[hl].ae.key)] = function() 
		{

			var phl0 = loggingLogicCase.loggingLogic[index];
			var phl1 = loggingLogicCase.name;
			var phl2 = loggingLogicCase.keys;
			var phl3 = argformat(loggingLogicCase.loggingLogic[index].ae.key); 
			var phl4 = loggingLogicCase.loggingLogic[index].config; 
			var phl5 = loggingLogicCase.loggingLogic[index].ae.separator;
			var phl6 = loggingLogicCase.loggingLogic[index].ae.value;
