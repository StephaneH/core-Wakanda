...
var testCase = {
    ...
    _should: {
        ignore: {
			testIsErrorLogic : true, 		//////
			testGetColumnFlagsLogic	: true,
...
4
    ...
    testSelectMoneySmallMoney: function() {////////////////////////////// à discuter
        ...
        if (dbconn) dbconn.close();
        Y.Assert.isFalse(exceptionOccur, "No exception shall occur here!" + exceptionMsg);
        Y.Assert.areSame(rows[0].money, -922337203685477.5808, "expected -922337203685477,5808 as Value!"); 
        Y.Assert.areSame(rows[0].smallmoney, -214748.3648, "expected -214748,3648 as  Value!"); 
        Y.Assert.areSame(rows[1].money, 922337203685477.5807, "expected 922337203685477,5807 as  Value!");
        Y.Assert.areSame(rows[1].smallmoney, 214748.3647, "expected 214748,3647 as  Value!");
        Y.Assert.areSame(rows[2].money, 0, "expected 0 as  Value!");
        Y.Assert.areSame(rows[2].smallmoney, 0, "expected 0 as  Value!");
    },
	//test float 
    ...
    testSelectReal: function() {///
        ...
        if (dbconn) dbconn.close();
        Y.Assert.isFalse(exceptionOccur, "No exception shall occur here!" + exceptionMsg);
        Y.Assert.areSame(rows[0].value, -3.4E+38, "expected  -3.3999999521443642e+38 as Value!"); //check value
        Y.Assert.areSame(rows[1].value, -1.1799999E-38, "expected -1.179999945774631e-38 as  Value!"); //check value
        Y.Assert.areSame(rows[2].value, 1.1799999E-38, "expected 1.179999945774631e-38 as  Value!"); //check value
        Y.Assert.areSame(rows[3].value, 3.4E+38, "expected 3.3999999521443642e+38 as  Value!"); //check value
    },
    //test Date & hour
   ...
   test_smalldatetime: function() {//////////
        ...
        var dsmalldatetime	= new Date("Wed Jun 07 2079 00:59:00 GMT+0000");
...
test_char10: function() {//////////discu
    ...
    testSelectFromHavingGroupOrder: function(){
        var exceptionOccur = false;
        var exceptionMsg = "";
        try {
            var dbconn = conn.connect(params);
            var resSet = dbconn.execute("select top 30 SUM(id), [date_of_birth], COUNT(*) from [benchdb].[dbo].[people] group by [date_of_birth] HAVING SUM(id)>1 order by [date_of_birth]");
        }catch(err){
            exceptionOccur = true;
            exceptionMsg = err.message;
        }
        ...
        Y.Assert.isFalse(exceptionOccur, "No Exception shall occur here ! error message : " + exceptionMsg);
	},
	...
	testCopyTableFromDBtoDB: function(){
	    ...
	    try {
	        debugger;
	        var dbconn = conn.connect(params);
	        var resSet = dbconn.execute("  select * into "+params.database+"_copy.dbo.texto from "+params.database+".dbo.test_text");
	        dbconn.useDatabase(params.database+"_copy");
