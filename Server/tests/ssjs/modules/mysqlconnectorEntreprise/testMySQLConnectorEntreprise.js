...
var testCase = {
	...
	setUp : function () {
        if (typeof this.initDone === 'undefined') {
			...
			SystemWorker.exec('sudo mysqladmin flush-hosts');
    ...
    testConnectWithNoPassword: function() {
        ...
        Y.Assert.areSame(exceptionMsg, "An error occurred on the mysql server 192.168.4.42. The error message is \"Access denied for user 'saad'@'192.168.222.47' (using password: NO)\".");
    },
    ...
    testConnectWithAnInvalidUserNameValue: function() {
        ...
        if (dbconn) dbconn.close();
        Y.Assert.isTrue(exceptionOccur, "An exception shall occur here!");
        Y.Assert.areSame(exceptionMsg, "An error occurred on the mysql server 192.168.4.42. The error message is \"Access denied for user 'boot'@'192.168.222.47' (using password: YES)\".");
    },
    ...
    testConnectWithAnInvalidPasswordValue: function() {
        ...
        if (dbconn) dbconn.close();
        Y.Assert.isTrue(exceptionOccur, "An exception shall occur here!");
        Y.Assert.areSame(exceptionMsg, "An error occurred on the mysql server 192.168.4.42. The error message is \"Access denied for user 'saad'@'192.168.222.47' (using password: YES)\".");
    },
