...
var testCase = {
    ...
    testSelectDateTime: function() {
        ...
        var date2 = new Date(9999, 11, 31, 0, 59, 59);
        var exceptionMsg = "";
        try {
            var dbconn = mysql.connect(params);
        ...
        if (dbconn) dbconn.close();
        Y.Assert.isFalse(exceptionOccur, "No exception shall occur here!" + exceptionMsg);
        Y.Assert.areSame(rows[1].value.toLocaleString(), date1.toLocaleString(), "expected Wednesday, January 01, 1000 00:00:00 as Value!");
        Y.Assert.areSame(rows[0].value.toLocaleString(), date2.toLocaleString(), "expected Friday, December 31, 9999 23:59:59 as   Value!");
