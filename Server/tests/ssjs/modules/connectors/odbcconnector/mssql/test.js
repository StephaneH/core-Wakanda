...
4
    ...
    //test float 
    testSelectFloat: function() {   // WAK0088184
        var exceptionOccur = false;
        var exceptionMsg = "";
        try {
            var dbconn = conn.connect(params);
...
test_char10: function() {//////////discu
    ...
    testHAPI2InsertDate: function() {
        ...
        if (dbconn) dbconn.close();
        Y.Assert.isFalse(exceptionOccur, "No exception shall occur here! " + exceptionMsg);
        Y.Assert.areEqual(row.id, 7, "Incorrect id value");
        Y.Assert.areEqual(row.value, "2012-12-21 01:01:00.0000000", "Incorrect number value");
        //Y.Assert.areEqual(row.string, "aaa", "Incorrect string value");
    },
    ...
    testHAPI3UpdateDateWithoutConditionNoParams: function() {
        var exceptionOccur = false,
            errMessage = "";
        var dat1 = new Date(Date.UTC(2012, 11, 21, 1, 1, 1));
        try {
...
////////////////////////////////////////////////////
    ...
    testExecuteWithInsertDuplicatedRow: function() {
        ...
        Y.Assert.isTrue(exceptionOccur, "No exception shall occur here!" + exceptionMsg);
        Y.Assert.areSame(exceptionMsg, 'The execution of the query "INSERT INTO test_int VALUES (2, 0);" failed on the server.', "incorrect error message");
    },
    ...
    testExecuteEmptyQuery : function(){
        ...
        if (dbconn) dbconn.close();
        Y.Assert.isTrue(exceptionOccur, "An exception shall occur here! ");
        Y.Assert.areSame(errMessage, "Empty query execution forbidden!", "incorrect error message");
    },
