...
var testCase = {
	...
	_should: {
		error: {},
		ignore: {
			testHTTPServerSSL_GetCertificationPathMethodExist_6:true,
			testHTTPServerSSL_GetCertificationPathMethodValue_7:true, 
			testHTTPServerSSL_GetCertificationFolderMethodExist_8:true, 
			testHTTPServerSSL_GetCertificationFolderMethodValue_9:true
		}
	},
	/*
	...
	testHTTPServerSSL_GetCertificationPathMethodValue_7: function() {
		var result = httpServer.ssl.getCertificatePath();
	...
	testHTTPServerSSL_GetCertificationFolderMethodExist_8: function() {
		Y.Assert.isString(httpServer.ssl.getCertificatePath().path);
		Y.Assert.isObject(httpServer.ssl.getCertificatePath());
		Y.Assert.isTrue(httpServer.ssl.getCertificatePath().exists);
	},
	// 9 --**-- Value returned by GetCertificationFolder() 
	testHTTPServerSSL_GetCertificationFolderMethodValue_9: function() {
	var result = httpServer.ssl.getCertificatePath().path;
		var solutionPath = solution.getFolder("path");	
		//Y.Assert.areEqual(solutionPath + "Certificates/",result.replace(/\\/g,'/'));
		Y.Assert.areEqual(solutionPath + "Certificates/",result);
	}
