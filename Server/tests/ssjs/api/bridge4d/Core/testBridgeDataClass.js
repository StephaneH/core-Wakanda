...
//fromArray(): we can't apply FromArray to a collection
    ...
    testFromArray_NullParam: function() {	
		var oldLength = ds.MyFromArrayClass.length;
		ds.MyFromArrayClass.fromArray(null);
		var newLength = ds.MyFromArrayClass.length;
		Y.Assert.areSame(oldLength,newLength,"fromArray with null parameter fail");
    },
    ...
    testFromArray_NullArray: function() {
		...
		var newLength = ds.MyFromArrayClass.length;
		Y.Assert.areSame(oldLength,newLength,"fromArray with null array fail");
    },
    ...
    testFromArray_NonExistentAttributes: function() {
		...
		if(lastLength == (ds.MyFromArrayClass.length-1)){
		var entity = ds.MyFromArrayClass.all()[ds.MyFromArrayClass.length-1];
		if(entity.name!=array[0].name || entity.cnum!=array[0].cnum || entity.clong!=array[0].clong || entity.cword!=array[0].cword || entity.cduration!=array[0].cduration)
			isGood= false;
		}
		if(entity.cdate.getTime() != array[0].cdate.getTime() && (entity.cdate.getTime()-3600000) != array[0].cdate.getTime() ){
			isGood= false;
		}
		if(!isGood)
    ...
    testFromArray_ArrayWithUndefinedImageValue: function() {
		...
		ds.MyFromArrayClass.fromArray(array);
		var entity = ds.MyFromArrayClass.find("name == 'default image value'");
		Y.Assert.isNotNull(entity);
		if(entity.name!=array[0].name || entity.cnum!=array[0].cnum || entity.cdate.getTime() != (array[0].cdate.getTime()+7200000) || entity.clong!=array[0].clong || entity.cword!=array[0].cword || entity.cduration!=array[0].cduration){
			isGood= false;
			message = entity.name + "#" + array[0].name + " ||| " + entity.cnum + "#" + array[0].cnum + " ||| " + entity.cdate.getTime() + "#" + array[0].cdate.getTime() + " ||| " + entity.clong + "#" + array[0].clong + " ||| " + entity.cword + "#" + array[0].cword + " ||| " + entity.cduration + "#" + array[0].cduration;
		}
		/*else
			if(entity.cimage!=null)
		...
		*/
		if(!isGood)
			Y.Assert.fail("FromArray: Array with undefined image values fail. "+message);
	},
    //collection with data
    ...
    testOrderBy_TypeStringAsBlobAsc: function() {
        var isGood = false;
        try {
            var returnedCollection = ds.MyOrderbyClass.orderBy('cstringAsBlob asc');
            isGood = true;
        }
        catch (e) {
            isGood = false;
        ...
        if (!isGood) 
        	Y.Assert.fail("OrderBy should accept a string considered as a blob");
        isGood = true;
        var idOrder = [4,3,7,8,5,6,1,2];
        for (var i = 0; i < returnedCollection.length; i++) {
                if (returnedCollection[i].ID != idOrder[i])
                    {
                        isGood = false;
                        break;
                    }
        ...
        if (!isGood) 
        	Y.Assert.fail("OrderBy does not return correct result for a string considered as a blob");
    },
    testOrderBy_TypeStringAsBlobDesc: function() {
        var isGood = false;
        try {
            var returnedCollection = ds.MyOrderbyClass.orderBy('cstringAsBlob desc');
            isGood = true;
        }
        catch (e) {
            isGood = false;
        ...
        if (!isGood) 
        	Y.Assert.fail("OrderBy (asc) should accept a string considered as a blob");
        isGood = true;
        var idOrder = [2,1,6,5,8,7,3,4];
        for (var i = 0; i < returnedCollection.length; i++) {
                if (returnedCollection[i].ID != idOrder[i])
                    {
                        isGood = false;
        ...
        if (!isGood) 
        	Y.Assert.fail("OrderBy (desc) does not return correct result for a string considered as a blob");
    },
	...
	if(arrayLenght != 10)
	{
		Y.Assert.fail("toArray method without parameters fail. "+arrayLenght);
	}
	else
	{	
		for(var i = 0;i<10;i++)
...
//				if(k==0){
	...
	//******* in Bridge it a Standard Behavior, see WAK0083646 *******
	testAdd_TestDuplication: function(){
		...
		coll.add(entity1);
		coll.add(entity2);
		Y.Assert.areSame(2,coll.length,"add method shouldn't duplicat entities in the entity collection");
	},
