...
var testCase = {
    ...
    _should: {
        ignore: {
            testOrderBy_TypeImage: true,
            //TBD
            testToArray_TypeImage: true,
            //TBD
            testToArray_TypeBlob: true,
            //TBD
            testToArray_TypeUuid: true,
            //TBD
            testFromArray_TypeImage: true,
            //TBD
            testFromArray_TypeBlob: true,
            //TBD
            testFromArray_TypeUuid: true,
            //TBD
            ...
            testFromArray_ArrayWhithDataContainingImages: true,
            testToArray_DataClassWithImageAttribute_ValuesShouldNotBeNull: true, //TBD
            testToArray_DatastoreWithImageAttribute: true, //TBD
            testForEach_DeleteAnEntity: true
    ...
    testAverage_BadAttributeParamTypeDuration: function() {
        Y.Assert.areSame(2.875,ds.MyClass1.average("cduration"), "Syntax failed : ds.MyClass1.average(duration).");
    },
    //bool
...
//        isGood = true;
//        try {
//    cette instruction cause un crash du server 4D, mais c'est pas reproductible
...
//        Y.Assert.areSame(12, ds.MyClass1.average(ds.MyClass1.cnum), "average of Data class failed.");   ***modified***
    },
    //Data class with data
    ...
    testCompute_BadAttributeParamTypeDate: function() {
		...
		var min = array.cdate.min;
		var max = array.cdate.max;
		for (var i=0; i<array.length; i++) {
			if(array[i].cdate.getTime() < min.getTime() || array[i].cdate.getTime() > max.getTime())
			{
				b = false;
				break;
			}
...
//		catch(e){}
    ...
    testCreateEntity_IDAttributInitialValue: function() {
        var myEntity = ds.MyClass1.createEntity();
        //Y.Assert.areSame(12, myEntity.ID, "bad attribute value failed.");
		//LR: "Yes, any code that is run on the remote server will only be executed once you save your entity".
    },
...
//		catch(e){}
    ...
    testForEach_SaveNewEntity: function() {
        ...
        var theEntity = ds.MyForEachClass.find("ID = :1",total+1);
        Y.Assert.isNotNull(theEntity, "Save new entity failed.");
        Y.Assert.areSame("new one",theEntity.cname,"Save new entity failed.");
...
//fromArray(): we can't apply FromArray to a collection
    ...
    testFromArray_ArrayWhithData: function() {
		...
		var array = new Array();
		array[0] = {name:"M",cnum: 12,cdate: dates[0], clong:123456, cword:3625, cduration:3};
		array[1] = {name:"B",cnum: 10,cdate: dates[1], clong:123400, cword:3629, cduration:1};
		array[2] = {name:"P",cnum: 13,cdate: dates[2], clong:123430, cword:3620, cduration:2};
		ds.MyFromArrayClass.fromArray(array);
		for(var i = 0; i<3; i++)
		{
	...
	testFromArray_ArrayWithoutData: function() {
		...
		var s = "";
		ds.MyFromArrayClass.fromArray(array);
		if(ds.MyFromArrayClass.length == lastArray.length){
			for(var i= 0;i<ds.MyFromArrayClass.length;i++){
				var entity = ds.MyFromArrayClass.find("ID="+(i+1));
				s+=entity.ID;
				if(lastArray[i].ID!= entity.ID || lastArray[i].name!= entity.name || lastArray[i].cnum!= entity.cnum ||lastArray[i].clong!= entity.clong ||lastArray[i].cword!= entity.cword || lastArray[i].cduration!= entity.cduration )
					{
						isGood = false;
						break;
					}
		...
		else Y.Assert.fail("fromArray using array without data has failed. The length of the dataclass was changed");
		if(!isGood)
			Y.Assert.fail("fromArray using array without data has failed"+s);
	},
	//fromArray(): attribute not found.
    testFromArray_MissingAttributes: function() {
		var isGood = true;
		var lastLength = ds.MyFromArrayClass.length;
		var array = new Array();
		array[0] = {clong:123456, cword:3625, cduration:0};
		ds.MyFromArrayClass.fromArray(array);
		if(lastLength == (ds.MyFromArrayClass.length-1)){
		var entity = ds.MyFromArrayClass.all()[ds.MyFromArrayClass.length-1];
		if(entity.name!=null || entity.cnum!=null || entity.cdate!= null || entity.clong!=array[0].clong || entity.cword!=array[0].cword || entity.cduration!=array[0].cduration)
			isGood= false;
		}
		else Y.Assert.fail("FromArray with missingattribute fail. The length is not correct");
		if(!isGood)
    ...
    testFromArray_NonExistentAttributes: function() {
		...
		var lastLength = ds.MyFromArrayClass.length;
		var array = new Array();
		array[0] = {nme:"oho",name:"Z",cnum:10,cdate: new Date("01/01/2012"),cnm:0, clong:5000,  cword:500, cduration:50,rien:"de rien"};
		ds.MyFromArrayClass.fromArray(array);
		if(lastLength == (ds.MyFromArrayClass.length-1)){
		var entity = ds.MyFromArrayClass.all()[ds.MyFromArrayClass.length-1];
		if(entity.name!=array[0].name || entity.cnum!=array[0].cnum || entity.cdate.getTime() != array[0].cdate.getTime() || entity.clong!=array[0].clong || entity.cword!=array[0].cword || entity.cduration!=array[0].cduration)
			isGood= false;
		}
		else Y.Assert.fail("FromArray with non existent attributes fail. The length is not correct");
		if(!isGood)
    ...
    testFromArray_replaceEntity: function() {
		...
		var lastLength = ds.MyFromArrayClass.length;
		var array = new Array();
		array[0] = {__KEY:5,name:"new one",cnum:10,cdate: new Date("01/01/2012"), clong:5000,  cword:500, cduration:50};
		ds.MyFromArrayClass.fromArray(array);
		if(lastLength == (ds.MyFromArrayClass.length-1)){
		var entity = ds.MyFromArrayClass.all()[ds.MyFromArrayClass.length-1];
		if(entity.name!=array[0].name || entity.cnum!=array[0].cnum || entity.cdate.getTime() != array[0].cdate.getTime() || entity.clong!=array[0].clong || entity.cword!=array[0].cword || entity.cduration!=array[0].cduration)
		...
		else Y.Assert.fail("FromArray: Replace an existing entity fail. The length is not correct");
		if(!isGood)
    ...
    testFromArray_ArrayWithUndefinedValues: function() {
		...
		var lastLength = ds.MyFromArrayClass.length;
		var array = new Array();
		array[0] = {name:"undefinedTest", cnum:undefined ,cdate:undefined , clong: undefined};
		ds.MyFromArrayClass.fromArray(array);
		if(lastLength == (ds.MyFromArrayClass.length-1)){
		var entity = ds.MyFromArrayClass.all()[ds.MyFromArrayClass.length-1];
		if(entity.name!="undefinedTest" || entity.cnum!=null || entity.cdate != null|| entity.clong!=null || entity.cword!=null || entity.cduration!=null)
			isGood= false;
		}
		else Y.Assert.fail("FromArray: Array with undefined values fail. The length is not correct");
		if(!isGood)
    ...
    testFromArray_ArrayWithUndefinedKeyValues: function() {
		...
		if(lastLength == (ds.MyFromArrayClass.length-1)){
		var entity = ds.MyFromArrayClass.all()[ds.MyFromArrayClass.length-1];
		if(entity.name!="undefined __key value" || entity.cnum!=null || entity.cdate != null || entity.clong!=null || entity.cword!=null || entity.cduration!=null)
			isGood= false;
		}
		else Y.Assert.fail("FromArray: Array with undefined values fail. The length is not correct");
		if(!isGood)
    ...
    testFromArray_ArrayWithUndefinedImageValue: function() {
		...
		var lastLength = ds.MyFromArrayClass.length;
		var array = new Array();
		array[0] = {name:"default image value",cnum:210 ,cdate: new Date("09/01/2012"), clong:5080,  cword:578, cduration:5};
		ds.MyFromArrayClass.fromArray(array);
		if(lastLength == (ds.MyFromArrayClass.length-1)){
		var entity = ds.MyFromArrayClass.all()[ds.MyFromArrayClass.length-1];
		if(entity.name!=array[0].name || entity.cnum!=array[0].cnum || dateToIso(entity.cdate).substring(0,10) != dateToIso(array[0].cdate).substring(0,10) || entity.clong!=array[0].clong || entity.cword!=array[0].cword || entity.cduration!=array[0].cduration)
			isGood= false;
		/*else
			if(entity.cimage!=null)
		...
		*/
		...
		else Y.Assert.fail("FromArray: Array with undefined image values fail. The length is not correct");
		if(!isGood)
    ...
    testMin_BadAttributeParamTypeDate: function() {
		var coll = ds.MyClass1.all();
		var result = ds.MyClass1.min("cdate");
		var b = true;
		for (var i=0; i<coll.length; i++) {
			if(result.getTime() > coll[i].cdate.getTime())
			{
				b = false;
				break;
			}
	...
	var myArray = ds.MyClass9.toArray("ID,name, cnum,cdate, cblob,clong, cword,cuuid,cduration");
	if(myArray.length!=10)
		{
	...
	var myString = "ZKFSD1A&";
	var b = true;
	for (var i = 0; i < 10; i++) {
		if (myString.indexOf(myArray[i].name) == -1) 
			{
				b = false;
				break;
			}
		}
	if(!b)	Y.Assert.fail("toArray method with Number attirbute and SortList param fail");
	},
	...
	if(myArray.length!=10)
		...
		}else if(myArray[0].__KEY !=0 || myArray[0].__STAMP !=1  || myArray[0].cnum != 1000 || myArray[0].clong!=0 || myArray[0].cduration!=9)
		{
			Y.Assert.fail("toArray method with key as string didn't return correct values");
		}else if(myArray[9].__KEY !=9 || myArray[9].__STAMP !=1  || myArray[9].cnum != -36 || myArray[9].clong!=8 || myArray[9].cduration!=0)
		{
	...
	testToArray_SortListByPathOfRelationAttributes: function() {
		var myArray  = ds.People.toArray("ID,name,sexe,PeopleFather_relation_retour.ID,PeopleFather_relation_retour.name","PeopleFather_relation_retour.name desc");
		var isGood = true;
		var names = "AZCDSFGHJ";
		for(var k = 0 ; k<9 ; k++){
			if(myArray[k].ID == k && myArray[k].name == names.charAt(k) && myArray[k].sexe==(k%2==0?"M":"F"))
			{
				if(k==0){
					...
					for(var j=0;j<4;j++) 
						if(myArray[k].PeopleFather_relation_retour[j].ID!=IDs[j] || myArray[k].PeopleFather_relation_retour[j].name != names.charAt(IDs[j])) isGood = false;
				}
				if(k==2) if(myArray[2].PeopleFather_relation_retour[0].ID !=6 || myArray[2].PeopleFather_relation_retour[0].name != "G") isGood = false;
				if(k==4) if(myArray[4].PeopleFather_relation_retour[0].ID !=7 || myArray[4].PeopleFather_relation_retour[0].name != "H") isGood = false;
				if(k==6) if(myArray[6].PeopleFather_relation_retour[0].ID !=8 || myArray[6].PeopleFather_relation_retour[0].name != "J") isGood = false;
	...
	testToArray_SortListByManyPathOfRelationAttributes: function() {
		var myArray  = ds.People.toArray("ID,name,sexe,PeopleFather_relation_retour.ID,PeopleMother_relation_retour.ID,PeopleFather_relation_retour.name,PeopleMother_relation_retour.name","PeopleMother_relation_retour.name asc , PeopleFather_relation_retour.ID desc");
		var isGood = true;
		var names = "AZCDSFGHJ";
		for(var k = 0 ; k<9 ; k++){
			if(myArray[k].ID == k && myArray[k].name==names.charAt(k) && myArray[k].sexe==(k%2==0?"M":"F"))
			{
				if(k==0){
					for(var i = 5, j=0;i>1;j++,i--) 
						if(myArray[k].PeopleFather_relation_retour[j].ID!=i || myArray[k].PeopleFather_relation_retour[j].name != names.charAt(myArray[k].PeopleFather_relation_retour[j].ID)) isGood = false;
				}
				if(k==1){
					var IDs= new Array(2,3,5,4);
					for(var j = 0 ; j<4;j++)
						if(myArray[k].PeopleMother_relation_retour[j].ID!=IDs[j] || myArray[k].PeopleMother_relation_retour[j].name != names.charAt(IDs[j])) isGood = false;
				}
				if(k==2) if(myArray[2].PeopleFather_relation_retour[0].ID !=6 || myArray[2].PeopleFather_relation_retour[0].name != "G") isGood = false;
				if(k==3) if(myArray[3].PeopleMother_relation_retour[0].ID !=6 || myArray[3].PeopleMother_relation_retour[0].name != "G") isGood = false;
				if(k==4) if(myArray[4].PeopleFather_relation_retour[0].ID !=7 || myArray[4].PeopleFather_relation_retour[0].name != "H") isGood = false;
				if(k==5) if(myArray[5].PeopleMother_relation_retour[0].ID !=7|| myArray[5].PeopleMother_relation_retour[0].name != "H") isGood = false;
				if(k==6) if(myArray[6].PeopleFather_relation_retour[0].ID !=8 || myArray[6].PeopleFather_relation_retour[0].name != "J") isGood = false;
				if(k==7) if(myArray[7].PeopleMother_relation_retour[0].ID !=8 || myArray[7].PeopleMother_relation_retour[0].name != "J") isGood = false;
	...
	testToArray_LevelTwoAttributes: function() {
		var myArray = ds.People.toArray("ID,name,sexe,PeopleFather_relation.name,PeopleMother_relation.name,PeopleFather_relation.PeopleFather_relation.name,PeopleMother_relation.PeopleFather_relation.name,PeopleMother_relation.PeopleMother_relation.name,PeopleFather_relation.PeopleMother_relation.name,PeopleFather_relation.PeopleFather_relation.PeopleFather_relation.name,PeopleMother_relation.PeopleFather_relation.PeopleFather_relation.name,PeopleFather_relation.PeopleFather_relation.PeopleMother_relation.name,PeopleMother_relation.PeopleFather_relation.PeopleMother_relation.name,PeopleFather_relation.PeopleMother_relation.PeopleMother_relation.name,PeopleMother_relation.PeopleMother_relation.PeopleMother_relation.name,PeopleFather_relation.PeopleMother_relation.PeopleFather_relation.name,PeopleMother_relation.PeopleMother_relation.PeopleFather_relation.name");
		var isGood = true;
		var names = "AZCDSFGHJ";
		for(var k = 0 ; k<9 ; k++){
			if(myArray[k].ID == k && myArray[k].name==names.charAt(k) && myArray[k].sexe==(k%2==0?"M":"F")){
				switch(k){
					case 2,3,4,5:
						if(myArray[k].PeopleFather_relation.name != "A" || myArray[k].PeopleMother_relation.name != "Z" )
							isGood =false;
						break;
					case 6:
						if(myArray[k].PeopleFather_relation.name != "C" || myArray[k].PeopleMother_relation.name != "D" )
							isGood =false;
						break;
					case 7:
						if(myArray[k].PeopleFather_relation.name != "S" || myArray[k].PeopleMother_relation.name != "F" )
							isGood =false;
						break;
					case 8:
						if(myArray[k].PeopleFather_relation.name != "G" || myArray[k].PeopleMother_relation.name != "H" )
							isGood =false;
						break;
					}
			}
	...
	testToArray_LimitNumberOfEntities: function() {
		var myArray = ds.People.toArray("ID,name,sexe,PeopleFather_relation_retour:3,PeopleMother_relation_retour");
		var isGood = "";
		var names = "AZCDSFGHJ";
		for(var k = 0 ; k<9 ; k++){
			if(myArray[k].ID == k && myArray[k].name==names.charAt(k) && myArray[k].sexe==(k%2==0?"M":"F")){
				if(k==0)
				{
					if(myArray[k].PeopleFather_relation_retour.length ==3 )
					{
						if(myArray[k].PeopleFather_relation_retour[0].ID != 2 || myArray[k].PeopleFather_relation_retour[1].ID != 3 || myArray[k].PeopleFather_relation_retour[2].ID != 4 )
							isGood += "false1";
					} else isGood += "false2";
				}
				if(k==1)
				{
					if(myArray[k].PeopleMother_relation_retour.length ==4 )
					{
						if(myArray[k].PeopleMother_relation_retour[0].ID != 2 || myArray[k].PeopleMother_relation_retour[1].ID != 3 || myArray[k].PeopleMother_relation_retour[2].ID != 4 || myArray[k].PeopleMother_relation_retour[3].ID != 5)
							isGood += "false3";
					} else isGood += "false4";
				}
			} else isGood += "false5";
		...
		if(!isGood)
			Y.Assert.fail("toArray with limit number of entities fail "+isGood);
	},
	...
	//test duplicated entity for unsorted entity collection ( should not be duplicated )
    ...
    //Test Float type
	testAttributeType_Float: function(){
		var isGood = true;
		var myColl = ds.FloatTable.all();
		try{
			myColl[0].cfloat;
		}
		catch(e){
		...
		if(!isGood){
			Y.Assert.fail("Type 'float' is not recognised.");
		}
	}
};