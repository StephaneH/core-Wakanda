var testCase = {
    	name: "Datastore test with 4D bridge (testsOfQueryMethod)",
    	_wait: {
    		before : 15000,
    		after: 15000
    	},
	setUp: function() {
		if (typeof this.initDone === 'undefined') {
			// Do it once:
			this.initDone = true;
			console.log('Init start: ' + new Date().toString());
			if (os.isWindows) {
				//var server4D = envVars.BUILD_TEST_DIR + '\\4D Server\\4D  Server.exe';
				//var base4D = application.getFolder('path').replace(/\//g, '\\') + 'Resources\\testWakandaBridge.4dbase'; 
				//var theWorker = new SystemWorker('"' + server4D + ' '  + base4D + '"');	
				// SystemWorker.exec('"' + server4D + ' '  + base4D + '"');			
			}
			if (os.isMac) {
				// var worker4D = new SharedWorker(application.getFolder('path') + 'macWorker.js', 'Worker4D');
			}
			if (os.isLinux) {
				// TODO
			}
			// wait(60000);
			console.log('Init done: ' + new Date().toString());	
		}
	},
	...
	testQuery_NToNRelationAttributes_Placeholder: function() {
		var coll = ds.Engineer.query("AttenEngi_relation_retour.AttenConf_relation.title = :1","java7");
		if(coll.length != 2 || coll[0].ID!=1 || coll[1].ID != 2 )
	...
	testQuery_NToOneRelationAttributes: function() {
		var message = "";
		var coll = ds.OneToN.query("oneToN_relation.name = 'oneToN21'");

		if(coll.length == 2)
		{
			if(coll[0].name!="oneToN11" || coll[1].name != "oneToN12")
				message += " (probleme in the entities of retured collection) ";
		}
		else
		{
...
//		var coll = ds.OneToN.query("oneToN.name = :1","oneToN2 1*");
		var coll = ds.OneToN.query("oneToN_relation.name = :1 ","oneToN21");
		if(coll.length == 2)
		{
			if(coll[0].name!="oneToN11" || coll[1].name != "oneToN12")
				message += " (probleme in the entities of retured collection) ";
...
//		var coll = ds.People.query("father.name = A");
		var coll = ds.People.query("PeopleFather_relation.name = A");
		if(coll.length == 4)
		{
			for(var i = 0 ; i < 4 ; i++ )
			{
				if(coll[i].PeopleFather_relation.ID != 0)
				{
					message += " (probleme in the entities of retured collection) ";
					break;
				}
...
//		var coll = ds.People.query("father.name = :1","A");
		var coll = ds.People.query("PeopleFather_relation.name = :1","A");
		if(coll.length == 4)
		{
			for(var i = 0 ; i < 4 ; i++ )
			{
	...
	testQuery_NToOneRelationAttributes_SameDataclass_LevelTow: function() {
		var message = "";
		var coll = ds.People.query("PeopleFather_relation.PeopleMother_relation.name = Z");
		if(coll.length == 2)
		{
			for(var i = 0 ; i < 2 ; i++ )
			{
				if(coll[i].PeopleFather_relation.PeopleMother_relation.ID != 1)
				{
					message += " (probleme in the entities of retured collection) ";
					break;
				}
	...
	testQuery_NToOneRelationAttributes_SameDataclass_LevelTow_Placeholder: function() {
		...
		var coll = ds.People.query("PeopleFather_relation.PeopleMother_relation.name = :1","Z");
		if(coll.length == 2)
		{
			for(var i = 0 ; i < 2 ; i++ )
			{
