...
var nonMatchingTableDef = getFolder("path")+"DataFolder/data.WakTDef";
...
var testCase = {
    ...
    _should: {
		...
		ignore: {
			...
			//Ignored because operates on default applications which uses journal (so conflicts with testParseJournalShallOperateOnCurrentApplicationByDefault)
    		...
    		tesParseJournalWithOldLogFormatAsString:false,
    		tesParseJournalWithOldLogFormatAsFile:false,
    		testParseJournalWithNonMatchingTableDefinition: false
		}
    },
	 setUp : function () {
    ...
    //Ensures that the journal is parsed even if we pass a definition file that does not
    //match any tables inside the journal
    testParseJournalWithNonMatchingTableDefinition: function () {
		var operations = null;
		var exceptions = 0;
		var exceptionText = "";
		
		var config = {tableDefinition:nonMatchingTableDef,journal:journalPath};
		
		Y.Assert.isTrue(File(nonMatchingTableDef).exists,"Non matching table definition file not found");
		
		try{
			operations = parseJournal(config);
		}
		...
		Y.Assert.isNotNull(operations,"No operations returned");
		Y.Assert.isArray(operations,"Not an array returned");
		Y.Assert.isFalse(hasTableNameProperty(operations),"unexpected tableName property");
    }
