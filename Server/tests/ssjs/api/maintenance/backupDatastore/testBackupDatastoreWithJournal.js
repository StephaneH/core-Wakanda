...
var testCase = {
     name: "backupDataStore: coverage for regular cases with a journaled data store",
    ...
    testBackupWAK6DataStore: function () {
		//With WAK7 the journal location is stored in the data file's extra property segment
		//Backup must fail unless such a journal is found 
		//Conversion is done when starting the related project with WAK7+ server
		//This test ensures that backup always fails for WAK6 and earlier journaled data store
		var result =null;
		var exceptions = 0;
		var config = {
			useUniqueNames: true,
			destination:Folder(WAK6BackupFolderPath),
			backupRegistryFolder:Folder(WAK6BackupRegistryPath)
		};
		var nbFoldersBefore = Folder(backupFolderPath).folders.length;
		var nbFilesBefore = Folder(backupFolderPath).files.length;
		
		try{
			 result = backupDataStore(WAK6modelFile , WAK6dataFile,config);
		}
		catch(e){
			exceptions++;
		}
