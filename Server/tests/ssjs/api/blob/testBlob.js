...
var testCase = {
	...
	// 2 --**-- Type property exist
	testBlob_TypePropertyExist_2: function() {
		var myBlob = new Blob(20, 88, "application/octet-stream");
		var myBlobPropType = myBlob.type;
		Y.Assert.areEqual("application/octet-stream", myBlobPropType);
		Y.Assert.isString(myBlobPropType);
	},
	// 3 --**-- Create a new instance of Object Blob with a default filler : 0, which should be a "null terminated" string
	testBlob_BlobWithDefaultFiller_3: function() {
	...
	// 4 --**-- Create a Blob with an empty mimeType : Default one should be : "application/octet-stream"
	testBlob_BlobWithDefaultMimeType_4: function() {
		var myBlob = new Blob(1, 65);
		var myBlobMimeType = myBlob.type;
		Y.Assert.areEqual("application/octet-stream", myBlobMimeType);
		Y.Assert.isString(myBlobMimeType);
	},
	// 5 --**-- CopyTo a Blob into a file 
	testBlob_CopyToABlob_5: function() {
		var myBlob = new Blob(20, 88, "application/octet-stream"),
	...
	// 6 --**-- CopyTo a Blob without Overwrite, should generate an error 
	testBlob_CopyToABlobWithoutOverwrite_6: function() {
		var myBlob = new Blob(20, 88, "application/octet-stream"),
			thepath = getFolder("path");
		myBlob.copyTo(thepath + "test.txt");
	},
	// 7 --**-- Slice a Blob
	testBlob_CopyToABlobInAWrongFile_7: function() {
	...
	// 8 --**-- Slice a Blob with a start/end value omitted
	testBlob_SliceABlobWithoutStartedEndedValue_8: function() {
		var myBlob = new Blob(5, 65, "application/octet-stream");
		var myString = myBlob.toString();
	...
	// 9 --**-- Slice a Blob with a start/end negative value
	testBlob_SliceABlobWithNegativeStartedEndedValue_9: function() {
		var myBlob = new Blob(5, 65, "application/octet-stream");
		var myString = myBlob.toString();
	...
	// 10 --**-- Slice a Blob without mimeType will be empty by default 
	testBlob_SliceABlobWithNegativeStartedEndedValue_10: function() {
		var myBlob = new Blob(5, 65, "application/octet-stream");
		var myString = myBlob.toString();
		var myb = myBlob.slice(-1, "application/octet-stream");
		var result = myb.toString();
	...
	// 11 --**-- Create a buffer containing a blob
	testBlob_BufferContainingABlob_11: function() {
		var myBlob = new Blob( 20 , 88, "application/octet-stream");
		var myString = myBlob.toString(); 
	...
	// 12 --**-- Get a string representation of the blob content
	testBlob_BlobTostring_12: function() {
		var myBlob = new Blob( 20 , 88, "application/octet-stream"); 
		var result = myBlob.toString();
		Y.Assert.isString(result);
    	Y.Assert.areEqual("XXXXXXXXXXXXXXXXXXXX", result);
	}, 
	// 13 --**-- Default value for a Blob without args
	testBlob_DefaultValueBlob_13: function() {
		var myBlob = new Blob(); 
