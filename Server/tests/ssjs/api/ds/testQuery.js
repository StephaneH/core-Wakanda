...
var testCase = {
	...
	//testQuery: type number (ID < 1)
    testQuery_TypeNumber_IDLessThanOne_Symbol: function() {
		var message = "";
		var col = ds.MyClass_QueryTests.query("ID < 1");
		if(col.length != 0)
		{
			message += " (the returned collection is not empty)";
		}
		if(message!="")
			Y.Assert.fail("query method applied to a number attribute fail. case: less than 1."+message);
    },
	//testQuery: type number
	...
	//testQuery: type image
	testQuery_TypeImage_notEqualNull: function() {
    	var message = "";
		var col = ds.MyOrderbyClass.query("cimage != null");
		if(col.length != 0)
		{
			Y.Assert.fail("query method applied to an image attribute fail: searching for entities containing 'image' value not null does not return correct results.");
		}
	},
	...
	testQuery_String_Btree_BeginOperator: function(){
		var coll = ds.MyClass_IndexedAttributes.query("cstring_Btree begin :1","t");
		if(coll.length <= 1)
			Y.Assert.fail("query with Btree index and Begin operator returns bud results");
	},
	testQuery_String_Cluster_BeginOperator: function(){
		var coll = ds.MyClass_IndexedAttributes.query("cstring_Cluster begin :1","z");
		if(coll.length <= 1)
			Y.Assert.fail("query with Cluster index and Begin operator returns bud results");
	},
	//test Query: 1 level
	testQuery_OneLevel: function(){
		var coll = ds.People.query("mother != null");
		if(coll.length!=22)
			Y.Assert.fail("query with one level returns bad results");
		var names = new Array();
		names = ["C","D","S","F","G","H","J","O","P","E","T","U","V","W","X","Y","AB","MN","RS","MOMO","TOTO","newChild"];
		for(var i = 0; i<coll.length;i++){
			var currentElement = coll[i];
			var currentIndex = names.indexOf(currentElement.name);
			if(currentIndex == -1){
				Y.Assert.fail("query with one level returns bad results");
			}
			else{
				names.splice(currentIndex,currentIndex+1);
			}
		}
	},
	//test Query: 2 levels
	testQuery_TwoLevels: function(){
		var coll = ds.People.query("mother.mother != null");
		if(coll.length!=13)
			Y.Assert.fail("query with two levels returns bad results");
		var names = new Array();
		names = ["G","H","J","U","V","W","X","Y","AB","MN","RS","MOMO","TOTO"];
		for(var i = 0; i<coll.length;i++){
			var currentElement = coll[i];
			var currentIndex = names.indexOf(currentElement.name);
			if(currentIndex == -1){
	...
	//test Query: 3 levels
	testQuery_ThreeLevels: function(){
		var coll = ds.People.query("mother.mother.mother != null");
		if(coll.length!=7)
			Y.Assert.fail("query with three levels returns bad results");
		var names = new Array();
		names = ["J","Y","AB","MN","RS","MOMO","TOTO"];
		for(var i = 0; i<coll.length;i++){
			var currentElement = coll[i];
			var currentIndex = names.indexOf(currentElement.name);
			if(currentIndex == -1){
	...
	//test Query: 4 levels
	testQuery_FourLevels: function(){
		var coll = ds.People.query("father.father.father.father != null");
		if(coll.length!=4)
			Y.Assert.fail("query with four levels returns bad results");
		var names = new Array();
		names = ["Y","AB","MOMO","TOTO"];
		for(var i = 0; i<coll.length;i++){
			var currentElement = coll[i];
			var currentIndex = names.indexOf(currentElement.name);
			if(currentIndex == -1){
	...
	//test Query: 5 levels
	testQuery_FiveLevels: function(){
		var coll = ds.People.query("mother.father.father.father.father != null");
		if(coll.length!=2)
			Y.Assert.fail("query with five levels returns bad results");
		var names = new Array();
		names = ["MOMO","TOTO"];
		for(var i = 0; i<coll.length;i++){
			var currentElement = coll[i];
