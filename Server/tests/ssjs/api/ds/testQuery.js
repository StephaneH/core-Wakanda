...
var testCase = {
	...
	testQuery_ExistenceOnCollection:function(){
		var myColl = ds.MyClass1.createEntityCollection();
		myColl.add(ds.MyClass1.first());
		myColl.add(ds.MyClass1.find("ID = 2"));
		myColl.add(ds.MyClass1.find("ID = 3"));
		var Coll2 = myColl.query("ID > 1");
		Y.Assert.areEqual(Coll2.length,2,"query on collection fail ");
		Y.Assert.areEqual(Coll2[0].ID,2,"query on collection fail (returned results are not correct)");
		Y.Assert.areEqual(Coll2[1].ID,3,"query on collection fail (returned results are not correct)");
	},
