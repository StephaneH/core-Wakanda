...
//testUnitaire de l'API backup/restore sans activation du journal.
	...
	//initialisation
		...
		var backup1 = Folder(getFolder().path+"Backups2/");
		var n1 = backup1.folders.length;
		for(var i = 0;i<n1;i++)
		{
			var e = Folder(backup1.folders[0].path);
...
//    	debugger;
    ...
    testMehodBackupWithOnlyOptionsParameter_testFiles: function() {
    	...
    	else
    	{
			...
			{
				//on va tester si le backup est bien ajouté et est dans le bon endroit
		    	if(manif.parent.parent.name != "Backups")
		    		Y.Assert.fail("initializing useUniqueName value to false fail" );
		    	//on va tester l'emplacement du registryFolder
		    	var _backupRegistryFolder = manif.path.substring(0,manif.path.length - manif.parent.name.length - 20);
    ...
    testMehodBackupWithOptionsAndConfigParameter_testFiles: function() {
	    ...
	    //     
	    var manif = ds.backup(config, options); 
    ...
    //13- testing default values of the parameters in the case of backupRegistryFolder:value
    testMehodBackupWithConfigParameter_OnlyBackupRegistryFolder: function() {
		var isGood = false;
		var back = "Backups2/";
		...
		if(manif.exists)
		{
	    	...
	    	if(manif.parent.name == "Backups"){
	    		initBackups();
	    		Y.Assert.fail("default value of destination parameter is not correct");
	    	}
	    	//on va tester l'emplacement du registryFolder
	    	var s = manif.path;
			var _backupRegistryFolder = s.substring(0,s.length - manif.parent.name.length - 1);
