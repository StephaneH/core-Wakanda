...
var testCase = {
    ...
    testGetLastBackupsWithEmptyBackupRegistryFile: function () {
		...
		if(!settings.backupRegistryFolder.exists)
		{
			...
			settings.backupRegistryFolder.create();
		}
		else
		{
			settings.backupRegistryFolder.removeContent(true);
		...
		try{
			var backupRegFile = new File(settings.backupRegistryFolder.path + CONSTANTS.registryFilename);
    ...
    testGetLastBackupsWithNonUniqueNames: function () {
		...
		var lastBackups = null;
		var exceptions = 0;
		settings.useUniqueNames = false;
		
		for(var i = 0;i < 5; i++){
			var manif;
			try{
				manif = ds.backup(settings);
				lastBackups = getLastBackups();
			}
	...
	testGetLastBackupsWithMultipleDuplicatesInRegistry: function () {
		...
		try{
			manif = ds.backup(settings);
			lastBackups = getLastBackups();
			var f  = new File(settings.backupRegistryFolder.path+ CONSTANTS.registryFilename);
