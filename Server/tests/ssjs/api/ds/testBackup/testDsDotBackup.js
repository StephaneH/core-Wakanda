...
var CONSTANTS={
	currentRegistryFilename : "lastBackups.json",
...
var testCase = {
	...
	testBackupAPIUsesNewBackupRegistryName: function(){
	...
	//This test ensures that for an empty setup (i.e. no previous backups), ds.backup() now automatically uses lastBackups.json for any backups.
	//And that the former registry name is not used
		var result =null;
		var manifestObj = null;
		var exceptions = 0;
		var settings = null;
		...
		Y.Assert.areEqual(0,exceptions,"ds.backup() threw an exception");
		Y.Assert.isNotNull(result,"No manifest file returned");
		Y.Assert.areEqual(1,getLastBackups().length , "Unexpected number of backups in registry");
		//Verify that only lastBackups.json exists and that lastBackup.json does not exists
		Y.Assert.isFalse(File(config.backupRegistryFolder.path+CONSTANTS.preWak9RegistryFilename).exists,"Unexpected deprecated backup registry file found");
		console.log(config.backupRegistryFolder.path+CONSTANTS.currentRegistryFilename);
		Y.Assert.isTrue(File(config.backupRegistryFolder.path+CONSTANTS.currentRegistryFilename).exists,"Backup registry file not found");
	},
	
	testBackupAPIMigratesToNewBackupRegistryName: function(){
	//This test is for regressing WAK0088009. Until WAK9 we used 'lastBackup.json' as the backup registry name.
	//From WAK9 onwards, it is 'lastBackups.json' (notice the s)
	//This test ensures that when an old registry is present, the backup API correctly transfers the existing backup history to the new registry
	//and eventually uses only the new registry.
		...
		var manifestFiles=[];
		var config = getBackupSettings();
		var deprecatedRegistryFile = new File(config.backupRegistryFolder.path+CONSTANTS.preWak9RegistryFilename);
		var currentRegistryFile = new File(config.backupRegistryFolder.path+CONSTANTS.currentRegistryFilename);
		...
		//Setup: the goal here is to create a pre-wak9 backup history
		//i.e. some backups and a backup registry with the deprecated name
		config.maxRetainedBackups = 5;
		try{
			for(var i=0;i<config.maxRetainedBackups-1;++i){
				manifestFiles.push(ds.backup(config));
			}
			currentRegistryFile.setName(CONSTANTS.preWak9RegistryFilename);
		}
		catch(e){
			exceptions++;
		}
		...
		Y.Assert.areEqual(0,exceptions,"Exceptions during test setup");
		Y.Assert.areEqual(manifestFiles.length,config.maxRetainedBackups-1, "Setup error: unexpected number of backups in registry");
		Y.Assert.isTrue(File(config.backupRegistryFolder.path+CONSTANTS.preWak9RegistryFilename).exists,"Setup error: deprecated backup registry file not found");
		Y.Assert.isFalse(File(config.backupRegistryFolder.path+CONSTANTS.currentRegistryFilename).exists,"Setup error: unexpected current backup registry file not found");
		
		//Now comes the test, we perform one more backup, and ensure the deprecated backup registry has not been modified 
		//and that the current backup registry exists and has all expected backups stored
		var modDateBefore = deprecatedRegistryFile.lastModifiedDate;
		var modDateAfter = null;
		var backups = null;
		try{
			MySleep(CONSTANTS.backupDelay);
			manifestFiles.push(ds.backup(config));
			backups = getLastBackups();
			modDateAfter = deprecatedRegistryFile.lastModifiedDate;
		}
		catch(e){
			exceptions++;
		}
		...
		Y.Assert.areEqual(0,exceptions,"Exceptions during test setup");
		Y.Assert.areEqual(manifestFiles.length,backups.length, "Unexpected number of backups in registry");
		Y.Assert.isTrue(deprecatedRegistryFile.exists,"deprecated backup registry file not found");
		Y.Assert.isTrue(currentRegistryFile.exists,"current backup registry file not found");
		Y.Assert.areEqual(modDateBefore.getTime(),modDateAfter.getTime(),"deprecated backup registry mod date has changed");
	}
};
