...
var testCase = {
    ...
    _should: {
        ignore: {
			...
			testRemove_ForCompositionRelation: true, //Waiting for L.R implementation
			testOrderBy_TypeStringAsBlobAsc: true, //WAK0077146
			testOrderBy_TypeStringAsBlobDesc: true //WAK0077146
	...
	testToArray_SortListByPathOfRelationAttributes: function() {
		var myArray = ds.People.toArray("ID,name,sexe,people_father.ID,people_father.name,father.name", "father.name desc");
		var isGood = true;
		for (var i = 0; i < myArray.length - 1; i++) {
			if (myArray[i].father != null && myArray[i + 1].father != null) {
				if (myArray[i].father.name < myArray[i + 1].father.name) {
					isGood = false;
					break;
				}
			}
	...
	testToArray_SortListByManyPathOfRelationAttributes: function() {
		var myArray  = ds.People.toArray("ID,name,sexe,people_father.ID,people_mother.ID,people_father.name,people_mother.name","mother.name asc , father.ID desc");
