...
var testCase = {
    ...
    testCompute_BadAttributeParamTypeDate: function() {
		...
		for (var i=0; i<array.length; i++) {
			if(array[i].cdate.getTime() < min || array[i].cdate.getTime() > max)
			{
				b = false;
				break;
			}
    ...
    testForEach_AutomaticSave: function() {
    	//opying the value that we will change
    	var oldValue = ds.MyForEachClass.find("ID=2").cname;
        ...
        ds.MyForEachClass.find("ID=2").cname = oldValue;
    ...
    testForEach_AutomaticSaveAnOtherEntity: function() {
    	//opying the value that we will change
    	var oldValue1 = ds.MyForEachClass.find("ID=1").cname;
    	var oldValue3 = ds.MyForEachClass.find("ID=3").cname;
    	//the begining of the test
        var value1 = "other1";
        var value3 = "other3";
        ds.MyForEachClass.forEach(function(thisEntity, iterator) {
        ...
        var theEntity3 = ds.MyForEachClass.find("ID=3");
        //Revert latest changes
        ds.MyForEachClass.find("ID=1").cname = oldValue1;
        ds.MyForEachClass.find("ID=3").cname = oldValue3;
        Y.Assert.areNotSame(value1, theEntity.cname, "Auto save an other entity fail.");
        Y.Assert.areNotSame(value3, theEntity3.cname, "Auto save an other entity fail.");
    },
    //save
    testForEach_SaveNewEntity: function() {
        ...
        //Revert latest changes
        ds.MyForEachClass.find("cname='new one'").remove();
    ...
    testForEach_SaveFollowedByAnAutomaticSave: function() {
    	//copying the value that we will change
    	var oldValue = ds.MyForEachClass.find("ID=1").cname;
    	//the begining of the test
        ...
        var theEntity = ds.MyForEachClass.find("ID=1");
        ds.MyForEachClass.find("ID=1").cname = oldValue;
        Y.Assert.areSame(secondValue, theEntity.cname, "Auto save is not done");
    },
    //delete an entity
    testForEach_DeleteAnEntity: function() {
    	ds.MyRemoveForEachClass.remove();
    	ds.MyRemoveForEachClass.setAutoSequenceNumber(1);
    	for(var i = 1;i<=3;i++)
    	{
    		(new ds.MyRemoveForEachClass()).save();
    	}
    	var result = "";
        var firstEntity = ds.MyRemoveForEachClass.find("ID = 1");
        var lastEntity = ds.MyRemoveForEachClass.find("ID = 3");
        ...
        Y.Assert.areSame("12", result, "Call back fail with entity remove.");
        var shouldBeNull = null;
        shouldBeNull = ds.MyRemoveForEachClass.find("ID = 1");
        Y.Assert.isNull(shouldBeNull,"Call back fail with entity remove.");
    },
    ...
    testFromArray_NullParam: function() {
    	var oldLegnth = ds.MyFromArrayClass.length;
		ds.MyFromArrayClass.fromArray(null);
		Y.Assert.areSame(oldLegnth,ds.MyFromArrayClass.length,"fromArray with null parameter fail");
    },
    ...
    testFromArray_NullArray: function() {
		...
		Y.Assert.areSame(oldLegnth,ds.MyFromArrayClass.length,"fromArray with null array fail");
    },
	...
	//test remove() for composition relation (WAK0075748 Bug)
	//Commented because the bug is delayed. WAK0075748)
    ...
    testSetAutoSequenceNumber_NegativeValue: function() {
    	var value = -300;
    	//deleting the entity with value = -300 if exist
    	var existingEntity = null;
    	existingEntity = ds.MyAutoSequenceNumberClass.find("ID = :1",value);
...
//				if(k==0){
	...
	testToArray_LimitNumberOfEntities: function() {
		...
		for(var k = 0 ; k<9 ; k++){
			if(myArray[k].ID == k && myArray[k].name==names.charAt(k) && myArray[k].sexe==(k%2==0?"M":"F")){
				...
				if(k==1)
				{
					if(myArray[k].people_mother.length ==5 )
					{
	...
	//Test The mechanism of renaming a dataclass and keeping data intact
	testRenameDataClass:function(){
		var myColl = ds.NewName.all();
		if(myColl.length != 0 )
			Y.Assert.fail("The old mechanism of renaming a dataclass and keeping data intact does not work.");
	},
	//
