...
/**
 * Unit tests for the backupDatastore method, when no journal is enabled
 ...
 * Copy your model to this project's NoJournal/Models model.waModel
 * Copy your data folder's content to this project NoJournal/DataFolder/
 */
...
var testCase = {
     name: "Test backupDataStore Without Journal",
	...
	testBackupDataStoreOnClosedDataStoreWithUniqueNames: function () {
		...
		for( var i =0; i < nbBackups;++i){
			...
			Y.Assert.isFalse(File(manifestFiles[i].parent.path+"journal.waJournal").exists,"Unexpected journal file found in backup "+i);
    ...
    testBackupDataStoreOnClosedDataStoreWithoutUniqueNames: function () {
		...
		//There should only be ONE FILE inside the backup folder: the manifest
		Y.Assert.areEqual(config.destination.files.length,1, "Wrong count of files in backup folder");
		...
		//there should not be a journal file inside the backup folder
		Y.Assert.isFalse(File(manifestFile.parent.path+"journal.waJournal").exists,"Unexpected journal file found in backup");
