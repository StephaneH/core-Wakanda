...
public class TestCasesSelectAPI extends SeleniumRuntimeTemplate {
	...
	@Ignore // TODO Test selectByKey(45); with a key above 40 max entity of rest request. Issue with widget sync 
	public void test_selectByKey() throws InterruptedException {
		...
		assertEquals("Check selectByKey() &  getKey()", "20", result);
		
		selenium.getJsConsole().executeScript("sources.employees.selectByKey(98);");
		Thread.sleep(500);
		result = (String) selenium.getJsConsole().executeScript("return sources.employees.getKey() +'';");
		assertEquals("Check selectByKey() &  getKey()", "98", result);
