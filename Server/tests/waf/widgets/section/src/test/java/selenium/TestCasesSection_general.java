...
public class TestCasesSection_general extends SeleniumRuntimeTemplate {
	...
	public void test_inputDefaultDisplay() throws InterruptedException {
		...
		String result = "";
		if (Common.getOS().equals("mac"))
		{
			result = (String) selenium.getJsConsole().executeScript("return $('#textField2').width() +'/'+ $('#textField2').height();");
			assertEquals("Check section input size (width/height)", "140/40", result);
		}
		else if (Common.getOS().equals("win"))
		{
			result = (String) selenium.getJsConsole().executeScript("return $('#textField2').width() +'/'+ $('#textField2').height();");
			assertEquals("Check section input size (width/height)", "133/40", result);
		}
