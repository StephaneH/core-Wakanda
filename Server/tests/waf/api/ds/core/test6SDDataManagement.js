...
var testCase = {
		...
		testAddEntityLogic1 : function(){
			...
			newEmp.company.setValue(source.CompaniesDS.getCurrentElement());
				...
				source.CitiesDS.serverRefresh(
					{
						onSuccess: function(event){
								...
								sources.eventsFlag.allEntities(
									{
										onSuccess : function(){
											testRunner.resume(function(){ 
							...
							sources.eventsFlag.allEntities(
									{
										onSuccess : function(){
											testRunner.resume(function(){ 
												//Server logic should be called
												var newFlagsLength = sources.eventsFlag.length;
												Y.Assert.areNotSame(oldFlagsLength, newFlagsLength, "The validateremove event didn't triggered oldLength : "+oldFlagsLength+", newLength : "+newFlagsLength+"!");

											});
										},
										onError: function(error){
											testRunner.resume(function(){ 
												Y.Assert.fail("Async call to allEntities failed");
											});	
										}
									});
		...
		testGetSelectionLogic : function(){
			...
			Y.Assert.areSame(5, arr[0], "The value should be the same, returned : "+arr[0]+"!");
		...
		**/
		 ...
		 */	
	    testGetValuesWithOrderByDSserverLogic: function () {
	    	var testRunner = this;
	    	var dsource = sources.CompaniesDS;
	    	var values;
	    	window.setTimeout(function () {
		    	...
		    	sources.CitiesDS.getValues("ID,name,pop","ID <= 5",
		    		{
		    			...
		    			'onSuccess':function(event){
		    				...
		    				testRunner.resume(function(){
								...
								Y.Assert.areSame(values[0].name, "Palo Alto", "The values should be the same, the value returned : "+values[0].name+"!");
								for (var i = 0; i < values.length - 2; i += 1) {
		 ...
		 * DSC-20. getValues logic with parametrs
		 */	
	    testGetValuesWithOrderByDSlocalLogic: function () {
	    	var testRunner = this;
	    	var dsource = sources.CompaniesDS;
	    	var values;
	    	window.setTimeout(function () {

		    	sources.ArrayDS.getValues("name,pop","",
		    		{
		    			orderBy : "pop",
		    			'onSuccess':function(event){
		    				values = event.result;
	    ...
	    testGetValuesWithSkipLogic: function () {
	    	...
	    	window.setTimeout(function () {
			    	sources.CitiesDS.getValues("ID,name,pop","ID <= 5",{ 'onSuccess': function(evt){
			    		oldLength = evt.result.length;
			    		
			    		afterLength = oldLength - toSkip;
				    	sources.CitiesDS.getValues("ID,name,pop","ID <= 5",
				    		{
				    			skip : toSkip,
				    			'onSuccess':function(event){
				    				...
				    				testRunner.resume(function(){
				    					var valuesLength = values.length;
				    					Y.Assert.areSame(afterLength, valuesLength, "The values should be the same, the exact value : "+afterLength+" the values length : "+valuesLength+" !");
				    					sources.CitiesDS.select(toSkip+1);
				    					Y.Assert.areSame(evt.result[3].ID,values[0].ID, "The values should be the same, Selected : "+sources.CitiesDS.ID+", Returned : "+values[0].ID+"!");								
									});
				    			},
				    			'onError':function(event){
				    				testRunner.resume(function(){
										Y.Assert.fail("The asyn call for the getValues is failed (2)!");
									});
				    			}
				    		});
						...
						testRunner.resume(function(){
										Y.Assert.fail("The asyn call for the getValues is failed (1) !");
									});
					}
				});
			},50);
	  ...
	  //   },
		 ...
		 */	
	  	 testGetElementsLogic: function () {
	    	var testRunner = this;
	    	var length = sources.CitiesDS_.length;
	    	var values;	    	
	    	var startVal = 2;
	    	var howManyVal = length - 1 - startVal;
	    	var indice = startVal;

	    	window.setTimeout(function () {

		    	sources.CitiesDS_.getElements(
		    		startVal,
		    		howManyVal,
		    		{		    			
		    			'onSuccess':function(event){
		    				//values = event.result;
		    				var howManyElement = event.result.length;
		    				var elements = event.result;
		    				testRunner.resume(function(){	
		    					Y.Assert.areSame(howManyElement, howManyVal, "The values should be the same HowMany (exc : "+howManyVal+", act : "+howManyElement+") !");
		    					for (var i = 0; i < elements.length; i++) {
		    						sources.CitiesDS_.select(indice++);
		    						Y.Assert.areSame(sources.CitiesDS_.ID, elements[i].ID, "The values should be the same exc : "+sources.CitiesDS_.ID+", act : "+elements[i].ID+"!");
		    					};
	    ...
	    /**
		 * DSC-20. collectionRefresh logic
		 */	
	    testCollectionRefreshLogic1: function () {
	    	var testRunner = this;
	    	var dsource = sources.CompaniesDS;
	    	dsource.select(2);
	    	var currentPosition = dsource.getPosition();
	    	window.setTimeout(function () {

		    	dsource.collectionRefresh(
		    		{
		    			
		    			'onSuccess':function(event){
		    				var refreshedPosition = event.position;
		    				
		    				testRunner.resume(function(){
		    					Y.Assert.areSame(currentPosition, refreshedPosition," The values should be the same : "+currentPosition+" != "+refreshedPosition);		    											
							});

		    			},
		    			'onError':function(event){
		    				testRunner.resume(function(){
								Y.Assert.fail("The asyn call for the collectionRefresh is failed !");
							});
		    			}
		    		});
	    ...
	    /**
	     * DSC-21. autoDispatch exists
	    */
	    testDispatchExists: function () {
	    	var dsource = sources.CompaniesDS;
			Y.Assert.areNotSame("undefined", typeof dsource.autoDispatch);
			Y.Assert.areSame("function", typeof dsource.autoDispatch);
		},	
		/**
	     * DSC-21. autoDispatch logic
	    */
		testAutoDispatchLogic: function () {		
	    ...
	    */
		testSourceDispatchLogic: function () {
		  ...
		  /**
		 * DSC-20. atLeastSizePage logic
		 */	
	    testAtLeastSizePageLogic: function () {
	    	var dsource = sources.CitiesDS;
	    	var varPageSize = 80;
	    	var oldPageSize = dsource._private.minPageSize;

	    	dsource.atLeastPageSize(varPageSize);

	    	Y.Assert.areNotSame(varPageSize, oldPageSize, "The valuse should not be the same !");

	    	var newPageSize = dsource._private.minPageSize;

	    	Y.Assert.areSame(varPageSize, newPageSize, "The valuse should be the same !");
