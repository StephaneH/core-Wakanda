...
var testCase = {
		...
		testSourceAttributeSetLogicUsingEntity: function () {
			var testRunner = this ;		
			source.CitiesDS_.select(2);
			var id = source.CitiesDS_.ID;
			window.setTimeout(function () {
				ds.City.getEntity(id, {
					'onSuccess': function(event){
						testRunner.resume(function(){ 
							city = event.entity ;
							Y.Assert.areNotSame(null, city, "Retrieved Entity is null !");	
							Y.Assert.areSame(id, city.ID.getValue(), "Retrieved Entity doesn't match the query !");	
					...
					'onError': function(error){
						...
						testRunner.resume(function(){ 
							Y.Assert.fail("Get Entity failed !"+error+" "+source.CitiesDS_.ID);
						});							
					}
				});
			}, 2);
						...
						source.CompaniesDS.location.load({
							'onSuccess': function(event) {
								testRunner.resume(function(){ 
									...
									var cityEntity = event.entity ;
									Y.Assert.areNotSame(null, cityEntity);
									Y.Assert.areSame(id1, cityEntity.ID.getValue());
									Y.Assert.areSame(name1, cityEntity.name.getValue());
								});					
							},
