...
var testCase = {
		...
		testServerRefreshLogic: function () {
			//add new element
		 	var lastElt = source.CitiesDS.length - 1;
 		 	source.CitiesDS.select(lastElt);
 		 	
 		 	var lastId = source.CitiesDS.ID + 1;
			source.CitiesDS.addNewElement();
			var testRunner = this;
			window.setTimeout(function () {
				source.CitiesDS.serverRefresh(
					{
						onSuccess: function(event){
							testRunner.resume(function(){ 
								...
								Y.Assert.areSame(lastId, source.CitiesDS.ID, "Server logic should be called lastId : "+lastId+" , Id : "+source.CitiesDS.ID);

								//nothing should be saved in the server
								window.setTimeout(function () {
									source.CitiesDS.query("ID="+lastId,
											{
										'onSuccess': function(event){
											testRunner.resume(function(){ 
												...
												testRunner.resume(function(){ 
													...
													ds.City.find("ID = :1 and name = :2 and pop = :3",
														...
														'onSuccess': function(event){
															testRunner.resume(function(){ 
																//Saved entity should be returned
																Y.Assert.areNotSame(null, event.entity, "Saved entity should be returned " +event.entity);
