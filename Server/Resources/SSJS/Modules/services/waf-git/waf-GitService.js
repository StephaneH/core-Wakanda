...
//gitCommand = 'git --git-dir=' + gitRepository + '\\.git --work-tree=' + gitRepository + ' checkout -f';
	...
	else if ( request. rawURL. indexOf ( "gitservice/repo/dir" ) != -1 )
	{
		...
		else
		{
			...
			// + " and user documents are at " + process. userDocuments. path;
	...
	else if ( request. rawURL. indexOf ( "gitservice/repo/winfo" ) != -1 )
	{
		...
		if ( gitRepositoryPath [ gitRepositoryPath. length - 1 ] != "/" )
			gitRepositoryPath += "/";
			
		var				strDotGit = gitRepositoryPath + ".git";
		var				fldrDotGit = new Folder ( process. userDocuments, strDotGit );
		if ( fldrDotGit. exists )
			return '{"path": "' + strDotGit + '"}';
		else
