...
*/
/*
	Module implementation for the service 'webApp'
 */
	...
	*/
	this._loadSettings = function () {
		...
		if (servicesModule.isServiceRegistered( kSERVICE_NAME)) {
			...
			// Set the web folder
			this._fApplication.internal.webAppService.setDocumentRootFolder( this._fApplication.getItemsWithRole( 'webFolder'));

			// Set the index page
			if (servicesSettings[kSERVICE_NAME].hasOwnProperty( 'directoryIndex')) {
				this._fApplication.internal.webAppService.setDirectoryIndex( servicesSettings[kSERVICE_NAME].directoryIndex);
			}
	...
	this.stop = function () {
		...
		if (this._loadContext()) {
			if ((this._fContext.state == kSTATE_STARTED) || (this._fContext.state == kSTATE_PAUSED)) {
				this._fApplication.internal.webAppService.setEnabled( false);
				if (!this._fApplication.internal.webAppService.enabled) {
					this._fContext.state = kSTATE_STOPPED;
					this._saveContext();
				}
