...
function _gitOctalToUniCode ( inText )
{
	...
	for ( var i = 0; i < inText. length; i++ )
	{
		...
		// Let's consume any digits that follow
		strDigits = "";
		while ( i + 1 < inText. length && "0" <= inText [ i + 1 ] && inText [ i + 1 ] <= "9" )
		{
			i++;
			strDigits += inText [ i ];
		}
...
//debug_alert ( "Octal decoded: " + strResult );
	return strResult;
}
 ...
 *
 */
var	gitPullImpl = function ( inURL, inWorkTree, inCallback )
{
	var				strRepoAndTreePath = _gitGetRepoAndTree ( inWorkTree );
	
	var				gitCommand;
	if ( os. isWindows )
		gitCommand = _gitExecPath + " pull " + inURL; // TODO: see if a URL needs to be "quoted" if it contains spaces or other "weird" chars
	else if ( _gitOSIsSupportedNIX ( ) )
		gitCommand = 'bash -c "' + _gitExecPath + " pull " + inURL + ' 2>&1"';
	else
		debug_alert ( "Error: unknown OS" );
...
var gitPushImpl = function ( inURL, inRepository, inCallback )
{
	...
	if ( os. isWindows )
	{
		...
		gitCommand = _gitExecPath + " --git-dir=" + inRepository + " push " + inURL + " " + localBranch + ":master";
	}
	else if ( _gitOSIsSupportedNIX ( ) )
	{
		inRepository = "'" + inRepository + "'";
		gitCommand = 'bash -c "' + _gitExecPath + ' --git-dir=' + inRepository + " push " + inURL + ' ' + localBranch + ':master' + ' --progress 2>&1"';
	}
	else
		debug_alert ( "Error: unknown OS" );
