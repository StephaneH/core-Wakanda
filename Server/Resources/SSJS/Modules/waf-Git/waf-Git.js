...
function debug_alert ( inMessage )
{
	...
	{
		; // studio does not exists on the server side, ignore the exception
	}
...
function _gitGetRepoAndTreeFSArr ( )
{
	var		arr = new Array ( );
	arr. push ( "--git-dir={R}" );
	arr. push ( "--work-tree={r}" );
...
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "config" );
	if ( inIsGlobal )
		gitCommand. push ( "--global" );

	gitCommand. push ( "--get" );
	gitCommand. push ( inProperty );
...
//var	wTest = new SystemWorker ( "git", "-version -c '{s} {s}' -all {{{s}}", fldrGitRoot, arrObjectTest );
	...
	var			gitWorker;
	var			objParameters = {	parameters : inParameters,
									folder : inRootFolder,
									stdin : "" };
	if ( os. isWindows )
		objParameters. quote = '"'; // Windows needs it quote character because the whole string is not split up into 'argc' and 'argv' as on Mac or Linux
	else
		objParameters. quote = "";

	var			arrArguments = new Array ( );
	arrArguments. push ( "git" );
	if ( inPattern instanceof Array )
		arrArguments. push. apply ( arrArguments, inPattern );
	else
		arrArguments. push ( inPattern );

/*if ( arrArguments. length > 7 )
{
	studio. alert ( "Arguments:" );
	for ( var i = 0; i < arrArguments. length; i++ )
		studio. alert ( "arrArguments[" + i + "] == " + arrArguments [ i ] );

	studio. alert ( "File list:" );
	var		arrFiles = objParameters. parameters. file_list;
	for ( var i = 0; i < arrFiles. length; i++ )
		studio. alert ( "arrFiles[" + i + "] == " + arrFiles [ i ]. path );
}*/
	gitWorker = new SystemWorker ( arrArguments, objParameters );
...
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "stash" );
	gitCommand. push ( inActionName );
	var				inParameters = null;
	if ( inActionName == "save" )
	{
		gitCommand. push ( "{comment}" );
		inComment = _gitEscapeForSystemWorkers ( inComment );
		inParameters = { comment : inComment };
	}
...
var	gitSaveStashImpl = function ( inWorkTree, inComment )
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "stash" );
	gitCommand. push ( "save" );
	gitCommand. push ( "{comment}" );
	inComment = _gitEscapeForSystemWorkers ( inComment );

	var				inParameters = { comment : inComment };
...
var gitPopLatestStashImpl = function ( inWorkTree, inID, inWithIndex )
{
	...
	if ( typeof ( inID ) != "undefined" && inID != null && inID. length > 0 )
		gitCommand. push ( _gitEscapeForSystemWorkers ( inID ) );

	if ( typeof ( inWithIndex ) != "undefined" && inWithIndex != null && inWithIndex )
		gitCommand. push ( "--index" );
	
	return _gitExecuteFS ( gitCommand, null, _gitGetRepositoryRootFolder ( ), 60 * 1000, null );
}
...
var gitGetStashIDImpl = function ( inComment, inWorkTree )
{
	...
	gitCommand. push ( "--grep=" + inComment );
...
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "pull" );
	gitCommand. push ( inURL );
	gitCommand. push ( inRemoteBranch ); // TODO: see if a URL needs to be "quoted" if it contains spaces or other "weird" chars
...
var gitPushImpl = function ( inURL, inRepository, inCallback, inLocalBranch, inRemoteBranch )
{
	...
	gitCommand. push ( inURL );
	gitCommand. push ( inLocalBranch + ":" + inRemoteBranch );
...
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "commit" );
	gitCommand. push ( "-m" );
	gitCommand. push ( "{message}" );

	var				inParameters = { message : inComment };
	...
	if ( arrLines != null )
	{
		...
		if ( err != null )
		{
			...
			else if ( strMessage != "" && strMessage. indexOf ( "Could not switch to" ) >= 0 )
			{
				...
				gitCommand = _gitGetRepoAndTreeFSArr ( );
				gitCommand. push ( "commit" );
				gitCommand. push ( "-a" );
				gitCommand. push ( "-m" );
				gitCommand. push ( "{message}" );
				inParameters = { message : inComment };
				arrLines = _gitExecuteFS ( gitCommand, inParameters, _gitGetRepositoryRootFolder ( ), 5 * 60 * 1000, null );
			}
		}
	}
...
{
	var				gitCommand = [ "clone", inURL, "{destination}", "--progress" ];
	var				flDestination = _gitConvertToFile ( inDestinationFolderPath );
	var				inParameters = { destination : flDestination };
...
var gitInitRepositoryImpl = function ( inFolderPath )
{
	...
	var				gitCommand2 = _gitGetRepoAndTreeFSArr ( );
	gitCommand2. push ( "config" );
	gitCommand2. push ( "http.sslVerify" );
	gitCommand2. push ( "false" );
...
var gitGetUntrackedFilesImpl = function ( inWorkTree, inIncludePaths )
{
	...
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "ls-files" );
	gitCommand. push ( "--exclude-standard" );
	gitCommand. push ( "--others" );
...
var gitRemoveImpl = function ( inArrPaths, inWorkTree )
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "rm" );
	gitCommand. push ( "--cached" );
	gitCommand. push ( "{file_list}" );
	var				arrFiles = _gitConvertToArrayOfFiles ( inArrPaths );
	var				inParameters = { file_list : arrFiles };
...
{
	var				gitCommand1 = _gitGetRepoAndTreeFSArr ( );
	gitCommand1. push ( "reset" );
	gitCommand1. push ( "HEAD" );
	gitCommand1. push ( "{file_list}" );
	var				gitCommand2 = _gitGetRepoAndTreeFSArr ( );
	gitCommand2. push ( "checkout" );
	gitCommand2. push ( "{file_list}" );
...
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "diff" );
	gitCommand. push ( "--name-status" );
	if ( inIsStaged )
		gitCommand. push ( "--cached" );
		
	gitCommand. push ( "--diff-filter=ACDMRUX" );
	gitCommand. push ( "--" );
	gitCommand. push ( "{file_list}" );
 ...
 * @param {String} inWorkTree
 * 	git repository root path
 * @param {bool} inRecursive
 *	optional flag to return status for all subfolders. Default - true.
 * 
 ...
 */
var gitProvideFolderStatusImpl = function ( inFolderPath, inIsStaged, inWorkTree, inRecursive )
{
	var			arrPaths = new Array ( );
	
	if ( typeof ( inRecursive ) == "undefined" || inRecursive == null )
		inRecursive = true;
	...
	{
		var			fldr = _gitCreateFolder ( inFolderPath );
		var			arrFiles = fldr. files;
		for ( var i = 0; i < arrFiles. length; i++ )
			arrPaths. push ( arrFiles [ i ]. path );
	}
...
function _gitExtractNonIndexedPaths ( inArrayPaths, inWorkTree )
{
	...
	var				inParameters = { file_list : arrFilesOrFolders };
...
function _gitIgnorePaths ( inArrayPaths, inWorkTree )
{
	...
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "update-index" );
	gitCommand. push ( "--assume-unchanged" );
	var				inParameters = { };
	...
	for ( var i = 0; i < arrFilesOrFolders. length; i++ )
	{
		...
		if ( _isFolder ( arrFilesOrFolders [ i ] ) )
		{
			...
			gitCommand. push ( "{file_list_" + i + "}" );
			inParameters [ "file_list_" + i ] = arrSubFiles;
...
//	debug_alert ( "Ignoring " + arrSubFiles [ j ]. path );
		...
		{
			nFileCount++;
			gitCommand. push ( "{file_ref_" + i + "}" );
			inParameters [ "file_ref_" + i ] = arrFilesOrFolders [ i ];
...
function _gitFollowPaths ( inArrayPaths, inWorkTree )
{
	...
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "update-index" );
	gitCommand. push ( "--no-assume-unchanged" );
...
*/
	...
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "difftool" );
	gitCommand. push ( "--cached" );
	gitCommand. push ( "-y" );
	gitCommand. push ( "--" );
	gitCommand. push ( "{file_list}" );
	var				arrFiles = _gitConvertToArrayOfFiles ( inPaths );
	var				arrParameters = { file_list : arrFiles };
...
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "rev-list" );
	gitCommand. push ( "--all" );

	var				arrResponse = _gitExecuteFS ( gitCommand, null, _gitGetRepositoryRootFolder ( ), 60 * 1000, null );
...
//debug_alert ( inToolPath );
	...
	var				gitCommand2 = [ "config", "--global", inToolType + "tool." + toolName + ".path", "{file_path}" ];
	var				inParameters = { file_path : inToolPath };

	arrResponse = _gitExecuteFS ( gitCommand2, inParameters, null, 10 * 1000, null );
	if ( inToolType == "merge" )
	{
...
var gitGetCurrentBranchImpl = function ( inWorkTree )
{
	...
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "rev-parse" );
	gitCommand. push ( "--abbrev-ref" );
...
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "checkout" );
	if ( typeof ( inCreate ) != "undefined" && inCreate )
		gitCommand. push ( "-b" );

	gitCommand. push ( inBranchName );
	var				arrLines = _gitExecuteFS ( gitCommand, null, _gitGetRepositoryRootFolder ( ), 60 * 1000, null );
...
var gitDropBranchImpl = function ( inName, inWorkTree )
{
	var			gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "branch" );
	gitCommand. push ( "--delete" );
	gitCommand. push ( inName );
...
{
	var				gitCommand = new Array ( );
	gitCommand. push ( "check-ref-format" );
	gitCommand. push ( "--branch" );
	gitCommand. push ( "{name}" );
	var				inParameters = { name : inName };
...
var gitGetCommitHistoryImpl = function ( inWorkTree, inPathsOptional )
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "log" );
	gitCommand. push ( "--format=format:\"--WAKANDA%n%H%n%aN%n%ae%n%s%n%cd\"" );
...
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "diff-tree" );
	gitCommand. push ( "--no-commit-id" );
	gitCommand. push ( "--name-only" );
	gitCommand. push ( "-r" );
	gitCommand. push ( inCommitHash );
...
var gitGetUnifiedDiffImpl = function ( inWorkTree, inCommitHash1, inCommitHash2, inPathsOptional )
{
	var				gitCommand = _gitGetRepoAndTreeFSArr ( );
	gitCommand. push ( "diff" );
	gitCommand. push ( inCommitHash1 );
	gitCommand. push ( inCommitHash2 );
