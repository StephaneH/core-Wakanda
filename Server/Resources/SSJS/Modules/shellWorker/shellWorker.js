...
var prepareCommandLine = os.isWindows ?
	function (inCommand) {
	    ...
	    // and escape cmd.exe special characters with ^
	    var r = /[\(\)\%\!\^\"\<\>\&\|\']/g;
	    var escapedCommand = inCommand.replace(r, '^$&')

	    // force unicode mode if user wants utf-16
	    var cmd = (exports.encoding == "ucs2") ? "cmd /u" : "cmd"
	    return cmd + ' /s /c "' + escapedCommand + '"';
	}
	:
	function (inCommand) {
        // on unix, pass argguments individually
