...
WAF.onAfterInit = function onAfterInit() { // @lock
    ...
    var initWebSockets = function () {
        ...
        var wsURL = 'ws://' + hostname + ':8080/remote_admin_ws/';
...
Admin.prototype.initLicense = (function() {
    ...
    var showTitle = function show_title() {
        if ( productName === "Wakanda Server") {
    ...
    var main = function () {
        webAdmin.getProductNameAsync({
            "onSuccess": function(response) {    
                productName = response === "Wakanda Server" ? response + " Administration" : response;
        ...
        sources.productName.addListener('onAttributeChange', function () {
            showTitle();
            if (productName === 'Wakanda Server Enreprise') {
                verfiyLicense();
            }
        });
    };
