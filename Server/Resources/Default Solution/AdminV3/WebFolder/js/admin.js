...
$(function() {  //focntion  main
		...
		switch($(this).prop('className')) {
			...
			case 'unlock':
				...
				$('.application')
					...
					.find('input[type=text]').prop('readonly', true)
					.end()
	...
	.delegate('#viewport .application .toolbar li', 'click', function(event) {
		...
		var application = WAF.serverAdmin.solution.current.applications[node.data('application-number')];
	...
	.delegate('#solution-open button', 'click', function(event) {
		if($(this).prop('className') == 'solution-open') {
			WAF.serverAdmin.loader.show();
