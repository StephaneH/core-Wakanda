...
echo "
 ...
 [INFO]
 [INFO]
 [INFO] 4D | Auto Test Win7 | Wakanda Server 
...
source "C:/Program Files (x86)/Jenkins/jobs/GoNoGoServer/workspace/Config.sh"
...
if [ -e "${HomeDirectory}${Directory}Junit-report/$(date +%d-%m-%Y)-GoNoGo-Server-Junit-file.xml" ]
  then 
cd "${HomeDirectory}${Directory}Junit-report/"
rm $(date +%d-%m-%Y)-GoNoGo-Server-Junit-file.xml
  else 
#Create a new file for XML Junit
> "${HomeDirectory}${Directory}Junit-report/$(date +%d-%m-%Y)-GoNoGo-Server-Junit-file.xml" 
fi
...
#Wrote in the Junit File
cat >> "${HomeDirectory}${Directory}Junit-report/$(date +%d-%m-%Y)-GoNoGo-Server-Junit-file.xml" <<EOF
<testsuite failures="DV" time="DV" errors="DV" skipped="DV" tests="DV" name="$(date +%d-%m-%Y)-GoNoGo-Server-Junit">
...
#Ajouter les solutions A, B, C de test dans Perforce et ajouter une commander pour effectuer un checkout des donn̩es.
...
if tasklist | grep Wakanda\ Server
then
...
cat >> "${HomeDirectory}${Directory}Junit-report/$(date +%d-%m-%Y)-GoNoGo-Server-Junit-file.xml" <<EOF
  <testcase name="${TESTCASE8}-[Failed]" passed="false" time="DV">
     <failure message="Test Failed, Wakanda Processus arent launched : Release Version"><![CDATA[Test Failed, Wakanda Processus arent launched : Release Version]]></failure>
  </testcase>
EOF
...
echo "With one processus launched for Wakanda Server, we have Add here the batch code for windows to see threads in Release Version"
...
# TESTCASE30="$(echo "30. Test Case Number 30 : We need to check that the RequestHandler works fine with the Solution C")"
# echo "${TESTCASE30}"
# Wrote in the Junit File
# cat >> "${HomeDirectory}${Directory}Junit-report/$(date +%d-%m-%Y)-GoNoGo-Server-Junit-file.xml" <<EOF
...
# TESTCASE31="$(echo "31. Test Case Number 31 : We need to check a simple GET request with his headers En cours")"
# echo "${TESTCASE31}"
# Wrote in the Junit File
# cat >> "${HomeDirectory}${Directory}Junit-report/$(date +%d-%m-%Y)-GoNoGo-Server-Junit-file.xml" <<EOF
...
# TESTCASE32="$(echo "32. Test Case Number 32 : We need to check a simple POST request with his headers | En cours")"
# echo "${TESTCASE32}"
# Wrote in the Junit File
# cat >> "${HomeDirectory}${Directory}Junit-report/$(date +%d-%m-%Y)-GoNoGo-Server-Junit-file.xml" <<EOF
...
# TESTCASE33="$(echo "33. Test Case Number 33 : We need to check a simple DELETE request with his headers | En cours")"
# echo "${TESTCASE33}"
# Wrote in the Junit File
# cat >> "${HomeDirectory}${Directory}Junit-report/$(date +%d-%m-%Y)-GoNoGo-Server-Junit-file.xml" <<EOF
...
# TESTCASE34="$(echo "34. Test Case Number 34 : We need to check a simple PUT request with his headers | En cours")"
# echo "${TESTCASE34}"
# Wrote in the Junit File
# cat >> "${HomeDirectory}${Directory}Junit-report/$(date +%d-%m-%Y)-GoNoGo-Server-Junit-file.xml" <<EOF
