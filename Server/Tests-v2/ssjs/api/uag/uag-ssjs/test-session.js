...
var _testCase = {
    ...
    testPromoteWithMethodPromotesSessionIntoTheGroupWhenGroupNameIsUsed: function() {
        ...
        try{
            ...
            Y.Assert.isFalse(belongsTo, "Session should not belong to group \"" + group + "\"" );
            ...
            Y.Assert.isTrue(belongsTo, "Session should now belong to group \"" + group + "\"" );
    ...
    testPromoteWithMethodReturnsZeroWhenUserBelongsToGroupAndGroupNameIsUsed: function() {
        ...
        try{
            ...
            var group = "Default";
            var belongsTo = session.belongsTo(group);
            Y.Assert.isTrue(belongsTo, "Session should belong to group \"" + group + "\"" );
            var token = session.promoteWith(group);
    ...
    testUnPromoteMethodStopsPromotionSetForCurrentSession: function() {
        ...
        try{
            ...
            belongsTo = session.belongsTo(group);
            Y.Assert.isFalse(belongsTo, "Session should not belong to group \"" + group + "\" anymore" );
        }finally{
            application.logout();
        }
    },
