...
// @region namespaceDeclaration// @startlock
	...
	var refresh = {};	// @button
	var logout = {};	// @button
	var showLoginDialog = {};	// @button
// @endregion// @endlock

// eventHandlers// @startlock
	setLabelTextColor.click = function setLabelTextColor_click (event)
	{// @endlock
		$$('login1').setLabelTextColor("#650092");
	};// @lock

	setLabelText.click = function setLabelText_click (event)// @startlock
	{// @endlock
		$$('login1').setLabelText("toto");
	};// @lock
	...
	{// @endlock
		var myPosition = $$('login1').getPosition();
	...
	{// @endlock
		$$('richText1').setValue($$('login1').getHeight());
	};// @lock

	getWidth.click = function getWidth_click (event)// @startlock
	{// @endlock
		$$('richText1').setValue($$('login1').getWidth());
	};// @lock
	...
	{// @endlock
		$$('login1').resizable(true);
	};// @lock

	draggable.click = function draggable_click (event)// @startlock
	{// @endlock
		$$('login1').draggable(true);
	};// @lock

	login.click = function login_click (event)// @startlock
	{// @endlock
		$$('login1').login('Donald', 'Duck');
	};// @lock

	refresh.click = function refresh_click (event)// @startlock
	{// @endlock
		WAF.directory.login("Donald", "Duck" , {
			onSuccess: function(event){
				$$('login1').refresh();
			}
		});
		$$('login1').refresh(); // Wrong, this is a workaround because "onSuccess" is not working.
	};// @lock
	...
	showLoginDialog.click = function showLoginDialog_click (event)// @startlock
	{// @endlock
...
// @region eventManager// @startlock
	...
	WAF.addListener("refresh", "click", refresh.click, "WAF");
	WAF.addListener("logout", "click", logout.click, "WAF");
	WAF.addListener("showLoginDialog", "click", showLoginDialog.click, "WAF");
// @endregion
};// @endlock
