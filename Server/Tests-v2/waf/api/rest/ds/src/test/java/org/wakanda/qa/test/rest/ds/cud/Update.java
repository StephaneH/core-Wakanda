...
public class Update extends AbstractTestCase {
	...
	private final DCUtil<UpdateSimple> dcus = DCUtil.getInstance(
			UpdateSimple.class, getRequestor());
	private final DCUtil<UpdateComplex> dcuc = DCUtil.getInstance(
			UpdateComplex.class, getRequestor());
	...
	private abstract class ProcessUpdate<T extends BasicEntity> {
		...
		private T prepareForAssert() {
			T eExpected = null;
			try {
				eExpected = (T) eOrigin.clone();
				eExpected
						.setUri(new RESTRequestBuilder(eOrigin.getClass()
								.getSimpleName()).addKey(eExpected.getKey())
								.toString());
		...
		public void check() throws Throwable {
			// Update the entity
			@SuppressWarnings("unchecked")
			DCUtil<T> dcu = (DCUtil<T>) DCUtil.getInstance(eOrigin.getClass(),
					getRequestor());
	...
	public void testUpdateRelatedEntityAttribute() throws Throwable {
		...
		final DCUtil<Master1> dcum = DCUtil.getInstance(Master1.class);
