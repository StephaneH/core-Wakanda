...
{
	XBOX::VJSContext	vjsContext(inContext);
	XBOX::VJSValue		result(vjsContext);
...
void VJSHTTPRequest::_GetBody (XBOX::VJSParms_getProperty& ioParms, IHTTPRequest *inRequest)
{
	...
	XBOX::VJSValue result = GetJSValueFromHTTPMessageBody (ioParms.GetContext(), contentTypeKind, charSet, stream);
...
void VJSMIMEMessagePart::_GetBodyAsText (XBOX::VJSParms_getProperty& ioParms, XBOX::VMIMEMessagePart *inMIMEPart)
{
	if (NULL != inMIMEPart)
	{
		...
		{
			XBOX::VJSValue result = GetJSValueFromHTTPMessageBody (ioParms.GetContext(), mimeTypeKind, charSet, &stream);
			ioParms.ReturnValue (result);
		}
		else
		{
...
void VJSMIMEMessagePart::_GetBodyAsPicture (XBOX::VJSParms_getProperty& ioParms, XBOX::VMIMEMessagePart *inMIMEPart)
{
	if (NULL != inMIMEPart)
	{
		...
		{
			XBOX::VJSValue result = GetJSValueFromHTTPMessageBody (ioParms.GetContext(), mimeTypeKind, XBOX::VTC_UNKNOWN, &stream);
			ioParms.ReturnValue (result);
		}
		else
		{
...
void VJSMIMEMessagePart::_GetBodyAsBlob (XBOX::VJSParms_getProperty& ioParms, XBOX::VMIMEMessagePart *inMIMEPart)
{
	if (NULL != inMIMEPart)
	{
		...
		XBOX::VJSValue result = GetJSValueFromHTTPMessageBody (ioParms.GetContext(), MIMETYPE_BINARY, XBOX::VTC_UNKNOWN, &stream);
		ioParms.ReturnValue (result);
	}
	else
	{
...
// 			name.FromCString ("Untitled");
		...
		if (XBOX::VE_OK == (error = stream.OpenWriting()))
		{
			...
			{
				XBOX::VJSObject	object = jsValue.GetObject();
...
void VJSMIMEWriter::_Construct (XBOX::VJSParms_construct& ioParms)
{
	...
	if (NULL != mimeWriter)
	{
		ioParms.ReturnConstructedObject (VJSMIMEWriter::CreateInstance (ioParms.GetContext(), mimeWriter));
	}
	else
	{
		XBOX::vThrowError (XBOX::VE_MEMORY_FULL);
...
void VJSSSL::_getEnabled (XBOX::VJSParms_getProperty& ioParms, VRIAServerProject* inRIAServerProject)
{
	...
	if (settings != NULL)
	{
		ioParms.ReturnBool (settings->GetAllowSSL() || settings->GetSSLMandatory());
	}
	else
	{
		ioParms.ReturnNullValue();
