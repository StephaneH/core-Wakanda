...
#define MYSQL_HOST					( VJSONValue( "192.168.222.28" ) )
...
#define MYSQL_DATABASE				( VJSONValue( "testSQLBridge_win" ) )
...
{
	CSQLConnector* connector = static_cast<CSQLConnector*>( VComponentManager::RetainComponent('ODBC', 'SQL ') );

	VJSONObject* params = new VJSONObject();

	params->SetProperty("dsn", VJSONValue("mysql_dsn"));
...
void testODBCConnectorPreparedStatement()
{
	...
	params->SetProperty("dsn", VJSONValue("mssql_dsn"));
...
{
	CSQLConnector* connector = static_cast<CSQLConnector*>( VComponentManager::RetainComponent('MYSQ', 'SQL ') );
