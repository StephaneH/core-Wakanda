...
namespace CommandLineParser
{
	...
	//! Command line parser: specialization for EHTTPServerPublishingOptions
	template<>
	struct Argument<enum EHTTPServerPublishingOptions> // final
	{
		static inline bool Append(EHTTPServerPublishingOptions& out, VString inValue)
		{
			out = eHSPO_UNKNOWN;
			...
			if (inValue == CVSTR("https"))
				out = eHSPO_HTTPS_ONLY;
			else if (inValue == CVSTR("http"))
				out = eHSPO_HTTP_ONLY;
			else if (inValue == CVSTR("both"))
				out = eHSPO_BOTH;
			else if (inValue == CVSTR("both-but-http-local-only"))
				out = eHSPO_BOTH_BUT_HTTP_LOCAL_ONLY;

			return (out != eHSPO_UNKNOWN);
		}
		static inline void SetFlag(EHTTPServerPublishingOptions& /*out*/)
		{
		}
		static inline void ExpectedValues(VString& out)
		{
			out += CVSTR("expected 'https', 'http', 'both' or 'both-but-http-local-only'");
		}
	};
#endif
...
{
	CREATE_BAGKEY_WITH_DEFAULT_SCALAR( serverRunningMode, XBOX::VLong, sLONG, eSRM_DEFAULT);
	CREATE_BAGKEY_WITH_DEFAULT_SCALAR( javaScriptAPIOptions, XBOX::VLong, sLONG, eJAO_DEFAULT);
...
#if ALLOW_CUSTOMIZE_ADMIN_PUBLISHING_OPTIONS
	CREATE_BAGKEY_NO_DEFAULT( administratorPublishingOptions, XBOX::VLong);
#endif
	...
	CREATE_BAGKEY( jobID);
#if ALLOW_USE_NEW_ADMIN
	CREATE_BAGKEY_WITH_DEFAULT_SCALAR( withNewAdmin, XBOX::VBoolean, bool, false);
#endif // ALLOW_USE_NEW_ADMIN
}
...
namespace // anonymous
{
	...
	static inline void AppendProductFullVersion(const VRIAServerApplication& inApplication, VString& out)
	{
	...
	static inline void Welcome(const VRIAServerApplication& inApplication)
	{
		VString welcome;
		welcome += CVSTR("Welcome to ");
		AppendProductFullVersion(inApplication, welcome);
		welcome += "\n\n";
...
void VRIAServerStartupParameters::SetServerRunningMode( EServerRunningMode inRunningMode)
{
	ServerStartupParametersKeys::serverRunningMode.Set( &fBag, inRunningMode);
}


EServerRunningMode VRIAServerStartupParameters::GetServerRunningMode() const
{
	return ServerStartupParametersKeys::serverRunningMode.Get( &fBag);
}


void VRIAServerStartupParameters::SetJavaScriptAPIOptions( EJavaScriptAPIOptions inOptions)
{
	ServerStartupParametersKeys::javaScriptAPIOptions.Set( &fBag, inOptions);
}


EJavaScriptAPIOptions VRIAServerStartupParameters::GetJavaScriptOptions() const
{
	return ServerStartupParametersKeys::javaScriptAPIOptions.Get( &fBag);
}
...
#if ALLOW_CUSTOMIZE_ADMIN_PUBLISHING_OPTIONS
void VRIAServerStartupParameters::SetAdministratorPublishingOptions( EHTTPServerPublishingOptions inPublishingOptions)
{
	fBag.SetLong( ServerStartupParametersKeys::administratorPublishingOptions, inPublishingOptions);
}


bool VRIAServerStartupParameters::GetAdministratorPublishingOptions( EHTTPServerPublishingOptions& outPublishingOptions) const
{
	return fBag.GetLong( ServerStartupParametersKeys::administratorPublishingOptions, outPublishingOptions);
}
#endif
...
#if ALLOW_USE_NEW_ADMIN
void VRIAServerStartupParameters::SetWithNewAdmin( bool inWithNewAdmin)
{
	ServerStartupParametersKeys::withNewAdmin.Set( &fBag, inWithNewAdmin);
}


bool VRIAServerStartupParameters::GetWithNewAdmin() const
{
	return ServerStartupParametersKeys::withNewAdmin.Get( &fBag);
}
#endif // ALLOW_USE_NEW_ADMIN
...
class VRIAServerJSFileRunner : public VObject
{
public:
			VRIAServerJSFileRunner( VRIAServerApplication* inServer, VFile* inFile)
			: fServer( inServer), fRunning( false), fTask( NULL)
			{
				fFile = RetainRefCountable( inFile);

			}

	virtual	~VRIAServerJSFileRunner()
			{
				ReleaseRefCountable( &fFile);
				ReleaseRefCountable( &fTask);
			}

			VError Run( bool inCreateNewTask)
			{
				if ((fServer == NULL) || (fFile == NULL))
				...
				{
					fTask = new VTask( this, 0, eTaskStylePreemptive, &_RunnerTaskProc);
					if (fTask != NULL)
					{
						fTask->SetKindData( (sLONG_PTR)this);
						fTask->SetName( L"JS File Runner");
						fTask->Run();
					}
					else
					{
			...
			void WaitUntilDone( sLONG inTimeoutMilliseconds)
			{
				if (fRunning && (fTask != NULL) && (!fTask->IsDying()))
				{
					...
					fTask->WaitForDeath( inTimeoutMilliseconds);
				}
			}

			const VString& GetResult() const { return fResult; }

			XBOX::VSignalT_1<XBOX::VString>& GetEvaluationDoneSignal() { return fEvaluationDoneSignal; }

protected:
			VError _Run()
			{
				VError err = VE_OK;
				VJSGlobalContext *globalContext = fServer->RetainJSContext( err, false);
				if (globalContext != NULL)
				{
					if (err == VE_OK)
					{
						...
						globalContext->EvaluateScript( fFile, &result, true);
						...
						if (errorContext.GetLastError() != VE_OK)
						{
							...
							if (context != NULL)
							{
								for (std::vector<VRefPtr<VErrorBase> >::const_iterator iter = context->GetErrorStack().begin() ; iter != context->GetErrorStack().end() ; ++iter)
								{
									...
									fResult.AppendString( errorMessage);
									fResult.AppendUniChar( L'\n');
								}
							}
						}
						else if (result != NULL)
						{
							result->GetString( fResult);
							fResult.AppendUniChar( L'\n');
						}
						fRunning = false;
					}

					err = fServer->ReleaseJSContext( globalContext);
					globalContext = NULL;
				}

				fEvaluationDoneSignal.Trigger( fResult);

				return err;
			}

	static	sLONG _RunnerTaskProc( VTask *inTask)
			{
				if (inTask != NULL)
				{
					VRIAServerJSFileRunner *runner = reinterpret_cast<VRIAServerJSFileRunner*>(inTask->GetKindData());
					if (testAssert(runner != NULL))
						runner->_Run();
				}
				return 0;
			}
...
private:
			...
			VString					fResult;
			VTask					*fTask;
			XBOX::VSignalT_1<XBOX::VString>		fEvaluationDoneSignal;
};
...
EServerRunningMode VRIAServerApplication::GetRunningMode() const
{
	if (fStartupParameters != NULL)
		return fStartupParameters->GetServerRunningMode();

	return eSRM_DEFAULT;
}


EJavaScriptAPIOptions VRIAServerApplication::GetJavaScriptAPIOptions() const
{
	if (fStartupParameters != NULL)
		return fStartupParameters->GetJavaScriptOptions();
...
VSolutionStartupParameters* VRIAServerApplication::RetainDefaultSolutionStartupParameters() const
{
	...
	VFilePath path;
	GetDefaultSolutionFilePath( path);
...
void VRIAServerApplication::GetDefaultSolutionFilePath( XBOX::VFilePath& outPath) const
{
	bool done = false;
	...
	if (fStartupParameters != NULL)
	{
		if (fStartupParameters->GetWithNewAdmin())
		{
			VFolder *folder = RetainApplicationResourcesFolder();
			if (folder != NULL)
			{
				folder->GetPath( outPath);
				outPath.ToSubFolder( L"Wakanda Admin");
				outPath.ToSubFolder( L"Wakanda Admin Solution");
				outPath.SetFileName( L"Wakanda Admin", false);
				outPath.SetExtension( RIAFileKind::kSolutionFileExtension);
				folder->Release();
				done = true;
			}
		}
	...
	if (!done)
	{
		...
		{
			folder->GetPath( outPath);
			outPath.ToSubFolder( L"Default Solution");
			outPath.SetFileName( L"DefaultSolution", false);
			outPath.SetExtension( RIAFileKind::kSolutionFileExtension);
			folder->Release();
		}
	}
...
void VRIAServerApplication::GetDefaultProjectFilePath( XBOX::VFilePath& outPath) const
{
	bool done = false;
	...
	if (fStartupParameters != NULL)
	{
		if (fStartupParameters->GetWithNewAdmin())
		{
			...
			if (folder != NULL)
			{
				...
				outPath.ToSubFolder( L"Wakanda Admin");
				outPath.SetFileName( L"Wakanda Admin", false);
				outPath.SetExtension( RIAFileKind::kProjectFileExtension);
				folder->Release();
				done = true;
			}
		}
	...
	if (!done)
	{
		...
		if (folder != NULL)
		{
			folder->GetPath( outPath);
			outPath.ToSubFolder( L"Default Solution");
			outPath.ToSubFolder( L"Admin");
			outPath.SetFileName( L"ServerAdmin", false);
			outPath.SetExtension( RIAFileKind::kProjectFileExtension);
			folder->Release();
		}
	}
...
VRIAServerSolution* VRIAServerApplication::OpenAndRetainSolutionForMaintenance( XBOX::VError& outError,  VSolutionStartupParameters *inStartupParameters, VRIAServerJob *inJob)
{
	...
	xbox_assert(openingParams.GetOpeningMode() == eSOM_FOR_MAINTENANCE);
...
VError VRIAServerApplication::CloseProject( VRIAServerProject* inProject)
{
	...
	if (fProjectsCollectionMutex.Lock())
	{
		VectorOfApplication_iter found = std::find( fProjectsCollection.begin(), fProjectsCollection.end(), VRefPtr<VRIAServerProject>(inProject));
...
void VRIAServerApplication::DoQuit()
{
	...
	if (fJSFileRunner != NULL)
	{
		fJSFileRunner->GetEvaluationDoneSignal().Disconnect( this);

		if (fJSFileRunner->IsRunning())
		{
			fJSFileRunner->WaitUntilDone( 5000);
			fputs_VString( fJSFileRunner->GetResult(), stdout);
		}

		delete fJSFileRunner;
		fJSFileRunner = NULL;
	}

	if (fJSContextMgr != NULL)
	{
...
bool VRIAServerApplication::_Init()
{
	...
	if (ok)
	{
		...
		VErrorBase::RegisterLocalizer( kSOLUTION_MANAGER_SIGNATURE, fLocalizer);
	}
...
void VRIAServerApplication::_DeInit()
{
	...
	xbox_assert(fJSFileRunner == NULL);
		...
		if (fStartupParameters != NULL)
		{
			...
			else
			{
				...
				if (jsFile != NULL)
				{
					...
					xbox_assert(fJSContextPool == NULL && fJSRuntimeDelegate == NULL && fJSFileRunner == NULL);
					...
					if (fJSRuntimeDelegate != NULL)
					{
						...
						if (err == VE_OK)
						{
							fJSFileRunner = new VRIAServerJSFileRunner( this, jsFile);
							if (fJSFileRunner != NULL)
							{
								// sc 18/07/2014 WAK0088770, to avoid the server to be blocked, evaluate the file into a new thread
								fJSFileRunner->GetEvaluationDoneSignal().Connect( this, VTask::GetCurrent(), &VRIAServerApplication::_OnJavaScriptFileEvaluationDone);
								err = fJSFileRunner->Run( true);
				...
				{
				#if WITH_SANDBOXED_PROJECT
					if ((GetRunningMode() & eSRM_WITH_SANDBOXED_ADMINISTRATOR) != 0)
					{
						VError err = _OpenAdministratorProject();
						continueRunning = (err == VE_OK);
					}
					else
				#endif // WITH_SANDBOXED_PROJECT
...
void VRIAServerApplication::_OnJavaScriptFileEvaluationDone( XBOX::VString inResult)
{
	fputs_VString( inResult, stdout);
	QuitAsynchronously();
}
...
VError VRIAServerApplication::_OpenSolutionAsCurrentSolution( VSolutionStartupParameters *inStartupParameters, VRIAServerJob *inJob)
{
	...
	if (fIsOpeningOrClosingCurrentSolution == 0)	// sc 21/06/2012 reentrance safeguard
	{
	...
	#if WITH_SANDBOXED_PROJECT
		else if ((GetRunningMode() & eSRM_WITH_SANDBOXED_ADMINISTRATOR) != 0)
		{
			fJSContextMgr->BeginPoolsCleanup();
			fJSContextMgr->CleanAllPools( 5000, NULL, NULL);
			fJSContextMgr->EndPoolsCleanup();
		...
		xbox_assert(openingParams.GetOpeningMode() == eSOM_FOR_RUNNING);
	...
	#if ALLOW_CUSTOMIZE_ADMIN_PUBLISHING_OPTIONS
		EHTTPServerPublishingOptions customPublishingOptions = eHSPO_UNDEFINED;
		if (fStartupParameters->GetAdministratorPublishingOptions( customPublishingOptions))
			openingParams.SetAdministratorPublishingOptions( customPublishingOptions);
	#endif
		...
		// sc 06/08/2014 WAK0088940, in case of the design solution cannot be opened, the log file may not be filled.
		// Then, we temporary install an output log listener which forward the error messages to the output console
		VOutputLogListener *outputListener = new VOutputLogListener( EML_Dump, EML_Trace, EML_Assert, EML_Debug, EML_Information, EML_Warning, -1);
		GetLogger()->AddLogListener( outputListener);

		solution = VRIAServerSolution::OpenSolution( err, inStartupParameters);

		if (err == VE_OK)
		{
			// the solution has been successfully opened, the output log listener can be removed
			GetLogger()->RemoveLogListener( outputListener);
			ReleaseRefCountable( &outputListener);
...
VError VRIAServerApplication::_OpenAdministratorProject()
{
	VError err = VE_OK;

	VFilePath path;
	GetDefaultProjectFilePath( path);
...
void VRIAServerApplication::_UpdateRunningServerFile()
{
	...
	if (jsonObject != NULL)
	{
		...
		if (fSolution != NULL)
		{
			...
			VFilePath adminProjectFilePath;
			GetDefaultProjectFilePath( adminProjectFilePath);
			VString adminProjectName;
			adminProjectFilePath.GetFileName( adminProjectName, false);
			VRIAServerProject *adminProject = fSolution->RetainApplicationByName( adminProjectName);
...
void VRIAServerApplication::_CleanupRunningServerFiles() const
{
	VFolder* enclosingFolder = VSolution::RetainRunningServerFilesFolder();
	if (enclosingFolder != NULL)
	{
		std::vector< VRefPtr < VFile > > files;
		std::vector< VRefPtr < VFolder > > folders;

		enclosingFolder->GetContents( files, folders);
		if (!files.empty())
		{
			VString wsProcessName, runningFileName( "wakandaServer");
			GetExecutableFilePath().GetName( wsProcessName);

			std::vector<pid> PIDs;
			std::set<pid> wsPIDs;
			VSystem::GetProcessList( PIDs);

			for (std::vector<pid>::iterator iter = PIDs.begin() ; iter != PIDs.end() ; ++iter)
			{
				VString procName;
				VSystem::GetProcessName( (uLONG)*iter, procName);
				if (procName.EqualToString( wsProcessName))
					wsPIDs.insert( *iter);
			}
			...
			{
				VString fileName;
				(*filesIter)->GetName( fileName);
				if (fileName.BeginsWith( runningFileName))
				{
					fileName.Remove( 1, runningFileName.GetLength());
					pid lPID = (pid)fileName.GetLong();
					if (wsPIDs.find( lPID) != wsPIDs.end())
					{
						// nothing to do
					}
					else
...
VCommandLineParser::Error  VRIAServerApplication::DoParseCommandLine(XBOX::VCommandLineParser& inParser)
{
	...
	inParser.Add(&optSolution, 's', "solution", CVSTR("Solution or JS file"));
	inParser.SetRemainingArguments(&optSolution);

	// daemonize
	bool optDaemonize = false;
	#if VERSION_LINUX
	inParser.AddFlag(&optDaemonize, 'd', "daemon", CVSTR("Go daemon and run in the background"));
	#endif

	// Administrative panel
	inParser.AddParagraph(CVSTR("\nAdministration options:"));
	// VString optAdminLogin;
	// inParser.Add(&optAdminLogin, ' ', "admin-login", CVSTR("Administrator login name (default: <empty>)"));
	VString optAdminPassword;
	inParser.Add(&optAdminPassword, ' ', "admin-password", CVSTR("Administrator login password (default: <empty>)"));

	// administration - port
	uLONG* optAdminPort = NULL;
	inParser.Add(&optAdminPort, ' ', "admin-port", CVSTR("Force the Administration panel port number"));
	// administration - ssl port
	uLONG* optAdminSSLPort = NULL;
	inParser.Add(&optAdminSSLPort, ' ', "admin-ssl-port", CVSTR("Force Administration panel ssl port number"));

	#if ALLOW_CUSTOMIZE_ADMIN_PUBLISHING_OPTIONS
	EHTTPServerPublishingOptions optAdminPublishOptions = eHSPO_UNDEFINED;
	inParser.Add( &optAdminPublishOptions, ' ', "admin-publish", CVSTR("Force Administration panel publishing mode\n \
		'https': accept only HTTPS connections\n \
		'http':  accept only HTTP connections\n \
		'both':  accept both HTTPS and HTTP connections\n \
		'both-but-http-local-only':  accept HTTPS connections and HTTP connections only from localhost"));
	#endif

	inParser.AddParagraph(CVSTR("\nService discovery options:"));
    bool optWithoutDiscovery = false;
	inParser.AddFlag(&optWithoutDiscovery, ' ', "no-discovery", CVSTR("Do not start Bonjour services"));
	// --without-discovery for compatibility
	inParser.AddFlag(&optWithoutDiscovery, ' ', "without-discovery", CVSTR("Do not start Bonjour services"));
	inParser.Hide("without-discovery");
	...
	inParser.Add(&optDebugger, 'g', "debugger", CVSTR("Debugger to launch at startup \
	...
	inParser.AddFlag(&optDebugOff, ' ', "debug-off", CVSTR("Disable the Debugger features.\
	...
	inParser.AddParagraph(CVSTR("\nJobs:"));
	VString optJobID;
	inParser.Add(&optJobID, ' ', "job-id", CVSTR("Specify the server job id"));

	#if WITH_SANDBOXED_SYSTEM_WORKERS
	inParser.AddParagraph(CVSTR("\nSystem workers:"));
	VRefPtr<VFile> optSystemWorkers;
	inParser.Add(&optSystemWorkers, ' ', "system-workers", CVSTR("Configuration file for system workers"));
	#endif
	...
	inParser.AddFlag(&optSyslog, ' ', "syslog", CVSTR("Forward Wakanda Server's log messages to the Syslog daemon"));
	#endif

	// --netdump
	bool optVerbose = false;
	inParser.AddFlag(&optVerbose, ' ', "verbose", CVSTR("Verbose mode"));

	//VString optPID;
	//inParser.Add(&optPID, ' ', "pid", CVSTR("Server PID file"));

	#if ALLOW_USE_NEW_ADMIN
	bool optWithNewAdmin = false;
	inParser.AddFlag( &optWithNewAdmin, ' ', "use-new-admin", CVSTR("Use the Wakanda Admin project"));
	inParser.Hide("use-new-admin");
	#endif // ALLOW_USE_NEW_ADMIN

	#if WITH_SANDBOXED_PROJECT
	bool optWithSandboxedProjectAtJSLevel = false;
	inParser.AddFlag( &optWithSandboxedProjectAtJSLevel, ' ', "with-sandboxed-js-api", CVSTR("Sandbox the projects at JavaScript level"));
	inParser.Hide("with-sandboxed-js-api");

	bool optWithSandboxedAdmin = false;
	inParser.AddFlag( &optWithSandboxedAdmin, ' ', "with-sandboxed-admin", CVSTR("Open the administrator in sandboxed mode"));
	inParser.Hide("with-sandboxed-admin");
	#endif // WITH_SANDBOXED_PROJECT

	bool optAllowDebugAdministrator = false;
	inParser.AddFlag( &optAllowDebugAdministrator, ' ', "debug-admin", CVSTR("Allow to debug the admin project"));
	inParser.Hide("debug-admin");

	inParser.AddParagraph(CVSTR("\nHelp:"));
	// --version
	bool optVersion = false;
	inParser.AddFlag(&optVersion, ' ', "version", CVSTR("Display the version and exit"));
	...
	{
		VString version;
		AppendProductFullVersion(*this, version);
		XBOX::VCommandLineParser::Print(version);
	...
	if (VCommandLineParser::eOK == err)
	{
		...
		#if ALLOW_CUSTOMIZE_ADMIN_PUBLISHING_OPTIONS
		// admin publication mode
		if (optAdminPublishOptions != eHSPO_UNDEFINED)
			settings->SetAdministratorPublishingOptions( optAdminPublishOptions);
		#endif
		...
		if (optSyslog)
		{
			VSysLogOutput* syslogOutput = new VSysLogOutput(CVSTR("wakanda-server"));
		...
		#if ALLOW_USE_NEW_ADMIN
		settings->SetWithNewAdmin( optWithNewAdmin);
		#endif

		#if WITH_SANDBOXED_PROJECT
		if (optWithSandboxedProjectAtJSLevel)
			settings->SetJavaScriptAPIOptions( eJAO_WITH_SANDBOXED_PROJECTS | eJAO_WITH_SERVER_OBJECT | eJAO_WITH_DEBUGGER_OBJECT);

		if (optWithSandboxedAdmin)
			settings->SetServerRunningMode( eSRM_DEFAULT | eSRM_WITH_SANDBOXED_ADMINISTRATOR);
		#endif

		settings->SetAllowDebugAdministrator( optAllowDebugAdministrator);
