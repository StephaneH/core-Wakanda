...
bool VRIAServerApplication::_Init()
{
	...
	if ( testAssert ( vfldrResources != NULL ) )
	{
		...
		if ( flSystemWorkers. Exists ( ) )
		{
			...
			VError			vError = VJSSystemWorker::Init ( fpSolution, VJSSystemWorker::eTYPE_DEFAULT );
			ok = ( vError == VE_OK );
		}
...
#if WITH_SANDBOXED_SYSTEM_WORKERS
			VJSSystemWorker::InitFileSystems ( );
#endif
						...
						if (err == VE_OK)
						{
							...
							if (globalContext != NULL)
							{
								...
								if ( !vfpSystemWorkers. IsEmpty ( ) && vfpSystemWorkers. IsValid ( ) )
									err = VJSSystemWorker::Init ( vfpSystemWorkers, VJSSystemWorker::eTYPE_DEFAULT );
#endif
...
static bool OpenSolutionOrJSFile(VRIAServerStartupParameters& inSettings, const VString& inSolution)
{
	...
	{
		VFilePath fullPath;
		VError initerr = VE_OK;

		#if VERSIONWIN
		fullPath.FromFullPath(inSolution, FPS_SYSTEM);
		#elif VERSIONMAC
		VURL::Decode(inSolution);
		fullPath.FromFullPath(inSolution, FPS_POSIX);
		#else
		PathBuffer buf;
		initerr = buf.Init(inSolution, PathBuffer::withRealPath);
		xbox_assert(initerr == VE_OK);
		if (initerr == VE_OK)
			initerr = buf.ToPath(&fullPath);
		if (initerr != VE_OK)
			err = initerr;
		#endif

		if (fullPath.IsValid() && fullPath.IsFile() && VE_OK == initerr)
		{
			VRefPtr<VFile> file = new VFile(fullPath);
			if (file->Exists())
			{
				...
				if (err != VE_OK)
				{
					...
					else if (ext == RIAFileKind::kProjectFileExtension)
					{
						...
						VCommandLineParser::PrintArgumentError(VString("file not valid (expected solution or js file): ") += inSolution);
				}
			}
		}
	}
	...
	if (err == VE_FILE_NOT_FOUND)
		VCommandLineParser::PrintArgumentError(VString("file not found: ") += inSolution);
...
{
	inParser.AddParagraph(CVSTR("Wakanda Server\n"
		"Build data centric apps. Manage. Communicate. Access your data.\n"
		"All in JavaScript.\n"
	...
	VString optAdminLogin = CVSTR("admin");
	inParser.Add(&optAdminLogin, ' ', CVSTR("admin-login"), CVSTR("Administrator login name (default: admin)"));
	VString optAdminPassword = CVSTR("admin");
	inParser.Add(&optAdminLogin, ' ', CVSTR("admin-password"), CVSTR("Administrator login password (default: admin)"));

	// administration - port
	uLONG optAdminPort = 8080;
	inParser.Add(&optAdminPort, ' ', CVSTR("admin-port"), CVSTR("Administration panel port number (default: 8080)"));
	// administration - ssl port
	uLONG optAdminSSLPort = 4343;
	inParser.Add(&optAdminSSLPort, ' ', CVSTR("admin-ssl-port"), CVSTR("Administration panel ssl port number (default: 4343)"));
	...
	#if WITH_SANDBOXED_SYSTEM_WORKERS
	inParser.AddParagraph(CVSTR("\nShared workers\n"));
	bool optSystemWorker = false;
	inParser.AddFlag(&optSystemWorker, ' ', CVSTR("system-worker"), CVSTR("Enable shared workers"));
	#endif
	...
	// --netdump
	bool optNetDump = false;
	inParser.AddFlag(&optNetDump, ' ', CVSTR("netdump"), CVSTR("Net dump"));
	...
	inParser.AddFlag(&optVersion, ' ', CVSTR("version"), CVSTR("Print the version and exits"));
	...
	if (VCommandLineParser::eOK == err)
	{
		...
		VRefPtr<VRIAServerStartupParameters> settings = new VRIAServerStartupParameters();

		// Admin port
		if (optAdminPort > 0 && optAdminPort < 65535)
			settings->SetAdministratorHttpPort(optAdminPort);
		else
			err = VCommandLineParser::PrintArgumentError(CVSTR("Invalid admin port"));

		// admin ssl port
		if (optAdminSSLPort > 0 && optAdminSSLPort < 65535)
			settings->SetAdministratorSSLPort(optAdminSSLPort);
		else
			err = VCommandLineParser::PrintArgumentError(CVSTR("Invalid admin ssl port"));
		...
		// Solution / JS file
		if (VCommandLineParser::eOK == err &&  optSolution.GetLength() > 0)
		{
			if (! OpenSolutionOrJSFile(*settings, optSolution))
		...
		if (VCommandLineParser::eOK == err)
		{
			...
			VRefPtr<VRIAServerStartupMessage> msg = new VRIAServerStartupMessage(this, settings);
