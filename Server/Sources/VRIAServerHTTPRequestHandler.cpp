...
VDebugHTTPRequestHandler::~VDebugHTTPRequestHandler()
{
	...
	{
		xbox_assert(fPersistantJSContextRunningCount == 0);
	...
	if (fApplication != NULL)
	{
		...
		if (sessionMgr != NULL)
		{
			...
			if (uagDirectory != NULL)
			{
				...
				else if (!inResponse->GetRequest().GetHTTPHeaders().IsCookieSet( kHTTP_SESSION_COOKIE_NAME))
				{
					// sc 20/08/2014 WAK0089091 create a default session only if the session cookie is not set
					VJSONObject* reqinfo = inResponse->GetRequest().BuildRequestInfo();
					// sc 03/06/2013, create a default session
					uagSession = uagDirectory->MakeDefaultSession(true, NULL, NULL, false, reqinfo);
					if (uagSession != NULL)
					{
						uagSession->GetID( userSessionID);
			...
			if (haveFile || haveScript)
			{
				...
				if (globalContext != NULL)
				{
					if (err == VE_OK)
					{
						...
						// sc 18/06/2014, new option to disallow the debugging for this context
						bool disallowDebugging = options.GetProperty( L"disallowDebugging").GetBool();
						...
						if (haveScript)
							globalContext->EvaluateScript( textToEvaluate, &sourceURL, &xvalue, true, false );
						else
