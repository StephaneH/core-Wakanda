				...
				if (httpServer != NULL)
				{
					...
					if (fHTTPServerProject != NULL)
					{
						...
						if (httpServerSettings != NULL)
						{
							...
							bool allowSSL = false;
							if ((fSettings.GetAllowSSL() || fSettings.GetSSLMandatory()) && sslCertificatePath.IsValid() && sslCertificatePath.IsFolder())	// sc 23/02/2012 check for "allowSSL" settings
							{
								VFolder sslCertificateFolder( sslCertificatePath);
								if (sslCertificateFolder.Exists())
									allowSSL = true;
							}
							httpServerSettings->SetAllowSSL(fSettings.GetAllowSSL());
