...
#if WITH_CUSTOM_ERROR_HANDLER
            /* Add IErrorProcessingHandler to handle custom errir pages (404.html, 4.html, 500.html or 5.html) */
            VErrorProcessingHandler *errorProcessingHandler = new VErrorProcessingHandler();
            if (NULL != errorProcessingHandler)
            {
				fHTTPServerProject->SetErrorProcessingHandler (errorProcessingHandler);
                XBOX::ReleaseRefCountable (&errorProcessingHandler);
            }
            else
            {
				err = vThrowError (VE_MEMORY_FULL);
            }
#endif	//	WITH_CUSTOM_ERROR_HANDLER
...
XBOX::VError VRIAServerProject::_LoadBackupSettings()
{
	...
	const VValueBag *solBackupSettingsBag  = NULL;
	
	if (fSolution != NULL)
	{
		const VSolutionSettings& solutionSettings = fSolution->GetSettings();
		solBackupSettingsBag = solutionSettings.RetainSettings( RIASettingID::backup);
	}
...
#if WITH_CUSTOM_ERROR_HANDLER
XBOX::VError VErrorProcessingHandler::ProcessError (XBOX::VError& inError, IHTTPRequestHandler * /*inHTTPRequestHandler*/, IHTTPResponse *ioResponse)
{
	XBOX::VError error = inError;
    
	if (COMPONENT_FROM_VERROR (error) == HTTP_SERVER_COMPONENT_SIGNATURE)
	{
		HTTPStatusCode  statusCode = (HTTPStatusCode)ERRCODE_FROM_VERROR (error);

        if ((statusCode != HTTP_UNAUTHORIZED) && (statusCode != HTTP_PROXY_AUTHENTICATION_REQUIRED))
        {
            if (((statusCode / 100) == 4) || ((statusCode / 100) == 5))
            {
                XBOX::VString   fileName;
                XBOX::VFilePath webFolderPath (ioResponse->GetVirtualHost()->GetProject()->GetSettings()->GetWebFolderPath());
                ...
                if (NULL == resultFile)
                {
                    ...
                    fileName.FromLong (sLONG (statusCode / 100));
                ...
                if (NULL != resultFile)
                {
                    ...
                    {
                        ioResponse->GetResponseBody().SetDataPtr (buffer.GetDataPtr(), buffer.GetDataSize());
                        buffer.ForgetData();
                        ioResponse->SetContentTypeHeader (CVSTR ("text/html"));
                    }
                }
            }
        }
