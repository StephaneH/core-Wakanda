...
VError VRIAServerProject::Start()
{
	...
	{
		VError lErr = _SetDebuggerServer( fOpeningParameters->GetDebuggerType());	// sc 08/04/2013, debugger type is now an opening parameter
		if (lErr != VE_OK)
		{
...
VError VRIAServerProject::SetDebuggerServer( VRIAContext* inContext, WAKDebuggerType_t inWAKDebuggerType, bool inAsynchronous)
{
	VError err = VE_OK;
	...
	if (context != NULL)
	{
		if (inAsynchronous)
		{
			VSetDebuggerServerMessage *setDebuggerMsg = new VSetDebuggerServerMessage( this, inWAKDebuggerType);
			VRIAServerApplication::Get()->PostMessage( setDebuggerMsg);
			ReleaseRefCountable( &setDebuggerMsg);
		}
		else
		{
			err = _SetDebuggerServer( inWAKDebuggerType);
		}
...
VError VRIAServerProject::_SetDebuggerServer( WAKDebuggerType_t inWAKDebuggerType)
{
	VError err = VE_OK;
		...
		if testAssert(nextJournalPath.IsValid() && nextJournalPath.IsFile() && !nextJournalPath.IsEmpty())
		{
			...
			if (lastJournalPath.IsEmpty())
			{
				...
				if (error == VE_OK)
				{
					...
					LogMessage( fLoggerID, eL4JML_Information, CVSTR("Successfully backed up data. Now creating journal.") );
					...
					if (error != VE_OK)
					{
						LogMessage( fLoggerID, eL4JML_Error, CVSTR("Failed to create new journal.") );
					}
				}
				else
				{
...
VSetDebuggerServerMessage::VSetDebuggerServerMessage( VRIAServerProject* inApplication, WAKDebuggerType_t inWAKDebuggerType)
{
	fApplication = RetainRefCountable( inApplication);
	fDebuggerType = inWAKDebuggerType;
}
...
void VSetDebuggerServerMessage::DoExecute()
{
	...
	if (fApplication != NULL)
	{
		...
		if (fApplication->IsOpened())
		{
			...
			VError err = fApplication->_SetDebuggerServer( fDebuggerType);
			done = (err == VE_OK);
...
XBOX::VError VRoutingPreProcessingHandler::ResolveURL (IHTTPRequest *inRequest, const XBOX::VString& inURL, XBOX::VFilePath& outFilePath, XBOX::VFilePath& outWebFolderPath)
{
	...
	virtualHost->GetMatchingVirtualFolderInfos (URL, outWebFolderPath, defaultIndexName, webFolderName);
