...
namespace ProjectOpeningParametersKeys
{
	...
	CREATE_BAGKEY_NO_DEFAULT_SCALAR( customPublishingOptions, XBOX::VLong, sLONG);
#endif
...
#if !ACCEPT_ANY_PROJECT_AS_ADMINISTRATOR
	CREATE_BAGKEY_WITH_DEFAULT_SCALAR(isAdministrator, XBOX::VBoolean, bool, false);
#endif
}
...
#if ALLOW_CUSTOMIZE_ADMIN_PUBLISHING_OPTIONS
void VRIAServerProjectOpeningParameters::SetCustomPublishingOptions( EHTTPServerPublishingOptions inPublishingOptions)
{
	ProjectOpeningParametersKeys::customPublishingOptions.Set( &fBag, inPublishingOptions);
}


bool VRIAServerProjectOpeningParameters::GetCustomPublishingOptions( EHTTPServerPublishingOptions& outPublishingOptions) const
{
	return ProjectOpeningParametersKeys::customPublishingOptions.Get( &fBag, (sLONG&)outPublishingOptions);
}
#endif
...
#if !ACCEPT_ANY_PROJECT_AS_ADMINISTRATOR
void VRIAServerProjectOpeningParameters::SetIsAdministrator( bool inIsAdministrator)
{
	ProjectOpeningParametersKeys::isAdministrator.Set( &fBag, inIsAdministrator);
}


bool VRIAServerProjectOpeningParameters::GetIsAdministrator() const
{
	return ProjectOpeningParametersKeys::isAdministrator.Get( &fBag);
}
#endif


#if ALLOW_CUSTOMIZE_ADMIN_PUBLISHING_OPTIONS
bool VRIAServerProjectOpeningParameters::PublishingOptionsToHTTPServerSettings( EHTTPServerPublishingOptions inPublishingOptions, bool& outAllowHttps, bool& outHttpsMandatory, bool& outAllowHttpsOnLocal)
{
	bool validOptions = true;

	switch (inPublishingOptions)
	{
	case eHSPO_HTTPS_ONLY:
		outAllowHttps = true;
		outHttpsMandatory = true;
		outAllowHttpsOnLocal = false;
		break;

	case eHSPO_HTTP_ONLY:
		outAllowHttps = false;
		outHttpsMandatory = false;
		outAllowHttpsOnLocal = false;
		break;
	...
	case eHSPO_BOTH_BUT_HTTP_LOCAL_ONLY:
		outAllowHttps = true;
		outHttpsMandatory = true;
		outAllowHttpsOnLocal = true;
		break;

	case eHSPO_UNDEFINED:
	case eHSPO_UNKNOWN:
		default:
			validOptions = false;
	}
	...
	virtual void SetUAGSessionAndCookie( IHTTPResponse *ioResponse)
			{
				...
				if (uagSession != NULL && uagSession->NeedsCookie())
				{
...
VError VRIAServerProject::Start()
{
	...
	{
#if 0 //USE_V8_ENGINE
		// for now, no early debug in V8
		VError lErr = _SetDebuggerServer(NO_DEBUGGER_TYPE, NULL);
#else
		...
		if (globalContext != NULL)
		{
			...
			if (session == NULL)
			{
				...
				if (dir != NULL)
				{
					...
					session = dir->MakeDefaultSession(VTask::GetCurrentData(VTask::GetHTTPServerSessionDataKey()) == TaskNeedSession, nil, nil, false, reqinfo);
			...
			if (disallowDebugging)
				VRIAServerJSContextMgr::DisallowDebuggingForContext( globalContext);
			else
				VRIAServerJSContextMgr::AllowDebuggingForContext( globalContext);
		}
...
#else
	return (fOpeningParameters != NULL) ? fOpeningParameters->GetIsAdministrator() : false;
#endif
}
						...
						if (fSolution != NULL)
						{
							...
							if (fSolution->GetDesignSolution()->GetSolutionFolderPath( solutionFolderPath))
							{
								...
								if (testAssert(wakDebuggerServer != NULL))
								{
									...
									{
										VFilePath defaultSolutionFolderPath;
										VRIAServerApplication::Get()->GetDefaultSolutionFilePath( defaultSolutionFolderPath);
										defaultSolutionFolderPath = defaultSolutionFolderPath.ToParent();
										sslCertificatePath = defaultSolutionFolderPath.ToSubFolder( L"Certificates");
									}
...
VError VRIAServerProject::_TerminateDebuggerHandling()
{
	...
	if (fOpeningParameters->GetHandlesDebuggerServer())
	{
		if (fDebuggerType != UNKNOWN_DBG_TYPE)
		{
			...
			{
				xbox_assert(debuggerServer->GetType() == fDebuggerType);
...
VError VRIAServerProject::InitializeJSContext( VJSGlobalContext* inContext)
{
	...
	if (inContext != NULL)
	{
		...
		if (err == VE_OK)
		{
			...
			#if WITH_SANDBOXED_PROJECT
				else if (fState.sandboxed && IsAdministrator())
				{
					// for the administrator project, the "solution" property is required
					VRIAServerSolution *solution = VRIAServerApplication::Get()->RetainCurrentSolution();
					VJSObject jsSolution( VJSSolution::CreateInstance( jsContext, solution));
					globalObject.SetProperty( kSSJS_PROPERTY_NAME_Solution, jsSolution, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete, NULL);
			...
			#if WITH_SANDBOXED_PROJECT
				if (CanHandlesDebuggerServer() && ((VRIAServerApplication::Get()->GetJavaScriptAPIOptions() & eJAO_WITH_DEBUGGER_OBJECT) != 0))
				{
					globalObject.SetProperty( L"wadebugger", VRIAServerJSDebugger::CreateInstance( jsContext, this));
				}
			#endif
			}
		}
...
VError VRIAServerProject::_Open( VProject* inDesignProject, VRIAServerProjectOpeningParameters *inOpeningParameters, CUAGDirectory* inUAGDirectory)
{
	...
	if (err == VE_OK)
	{
				...
				if (httpServer != NULL)
				{
					...
					if (fHTTPServerProject != NULL)
					{
						...
						if (httpServerSettings != NULL)
						{
							...
							bool allowSSL = fSettings.GetAllowSSL();
							bool SSLMandatory = fSettings.GetSSLMandatory();
							bool allowHTTPOnLocal = fSettings.GetAllowHTTPOnLocal();

						#if ALLOW_CUSTOMIZE_ADMIN_PUBLISHING_OPTIONS
							EHTTPServerPublishingOptions publishingOptions = eHSPO_UNDEFINED;
							if (fOpeningParameters->GetCustomPublishingOptions( publishingOptions))
							{
								bool lAllowSSL = false, lSSLMandatory = false, lAllowHTTPOnLocal = false;
								if (VRIAServerProjectOpeningParameters::PublishingOptionsToHTTPServerSettings( publishingOptions, lAllowSSL, lSSLMandatory, lAllowHTTPOnLocal))
								{
									allowSSL = lAllowSSL;
									SSLMandatory = lSSLMandatory;
									allowHTTPOnLocal = lAllowHTTPOnLocal;
								}
							}
						#endif

							httpServerSettings->SetSSLMandatory( SSLMandatory);
							httpServerSettings->SetAllowHTTPOnLocal( allowHTTPOnLocal);
							...
							fSettings.GetHTTPLogFolderPath( strValue);
		...
		{
			VFilePath projectFolderPath = fDesignProjectFolder->GetPath();

			VString posixSourcesRoot;
			projectFolderPath.GetPosixPath( posixSourcesRoot);
			fWAKBreakpointsManager = new VRemoteDebuggerBreakpointsManager( posixSourcesRoot);
			if (fWAKBreakpointsManager != NULL)
			{
				fWAKBreakpointsManager->SetBreakpointsFolderPath( projectFolderPath);
