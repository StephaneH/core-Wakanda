...
{
	VJSArray	arrayOfJobs(ioParms.GetContextRef());
...
void VJSApplication::_unsecureLogin( XBOX::VJSParms_callStaticFunction& ioParms, VRIAServerProject* inApplication)
{
	...
	if (directory != nil)
	{
		...
		CUAGSession* session = directory->OpenSession(username, password, nil, (VJSContext*)&(ioParms.GetContext()), false);
		if (session != nil)
		{
...
void VJSApplication::_login( XBOX::VJSParms_callStaticFunction& ioParms, VRIAServerProject* inApplication)
{
	...
	if (directory != nil)
	{
		...
		CUAGSession* session = directory->OpenSession(username, inHa1, nil, (VJSContext*)&(ioParms.GetContext()), true);
		if (session != nil)
		{
			session->SetLifeTime(nbsecs);
			VJSContext jsContext(ioParms.GetContextRef());
...
void VJSApplication::_getUserSessions( XBOX::VJSParms_callStaticFunction& ioParms, VRIAServerProject* inApplication)
{
	...
	if (sessionmgr != nil)
	{
		...
		if (directory != NULL)
		{
			...
			{
				result.PushValue(cur->Get()->CreateJSSessionObject(ioParms.GetContextRef()));
			}
			directory->Release();
		}
		sessionmgr->Release();
...
void VJSApplication::_restoreDataStore(XBOX::VJSParms_callStaticFunction& ioParms, VRIAServerProject* inApplication)
{
	...
	//Response: {ok:true|false[,dataFolder:Folder(xxxxx)][,journal:File(xxxx)]}
	{
		...
		{
			VJSObject fileObj(ioParms.GetContextRef());
...
{
	VRIAContext *riaContext = NULL;
	VFile *journalFile(NULL), *destinationFile(NULL);
	VError err = VE_OK;
	bool done = false;
	XBOX::StErrorContextInstaller errContext(true,true);
	VJSArray arrayOfOps(ioParms.GetContextRef());
	...
	if (errContext.GetLastError() == VE_OK)
	{
		...
		if (!ioParms.IsNullParam(2) &&ioParms.IsObjectParam(2))
		{
			ioParms.GetParamObject(2, optionsObj);
		}
		else
		{
			optionsObj.MakeEmpty();
		...
		if (errContext.GetLastError() == VE_OK)
		{
			...
			if(tool)
			{
				...
				else
				{
					...
					if(done)
					{
						ioParms.ReturnValue(arrayOfOps);
					}
				}
			}
