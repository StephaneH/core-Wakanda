...
void fputs_VString( const VString& inMessage, FILE *inFile)
{
	...
	fputs( stringConverter.ConvertString( inMessage), inFile);
...
void StTaskPropertiesSetter::_Test()
{
	...
	xbox_assert(curProps == initialProps);
...
{
	xbox_assert(fPreviousProperties == NULL);
