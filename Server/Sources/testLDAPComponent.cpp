...
#include "LDAPComponent/Sources/ILDAPClient.h"
#include "LDAPComponent/Sources/ILDAPEntry.h"
...
void testLDAP::TestLDAPComponentSearch()
{
	CLDAPComponent* ldap = (CLDAPComponent*)(VComponentManager::RetainComponent('LDAP'));


	//VJSONObject* params = sParams->GetProperty("search").GetObject();
	VJSONObject* paramsBind = sParams->GetProperty("bind").GetObject();

	

	std::vector<VString> attrs;
	attrs.push_back("objectClass");
	attrs.push_back("objectCategory");
	VError err = VE_OK;
	ILDAPClient* client = ldap->CreateClient(paramsBind);
	std::vector<ILDAPEntry*> ldapEntries;
	...
	if (client != NULL)
	{
		...
		ldapEntries = client->Search("CN=Users,DC=LDAPTest,DC=local", "(objectClass=*)", ILDAPClient::eLDAPSubScope, attrs, err);
		std::vector<ILDAPEntry*>::iterator it = ldapEntries.begin(), end = ldapEntries.end();
		while (it != end)
		{
			std::vector<VString> objectsClass = (*it)->GetAttributeAsString("objectClass");
			std::vector<VString>::iterator itStr = objectsClass.begin(), endStr = objectsClass.end();
...
void testLDAP::TestLDAPComponentGetUUID()
{
	CLDAPComponent* ldap = (CLDAPComponent*)(VComponentManager::RetainComponent('LDAP'));
	...
	if (client != NULL)
	{
		...
		VUUID userUUID;
		client->GetUUID(userUUID);
		userUUID.GetString(uid);
