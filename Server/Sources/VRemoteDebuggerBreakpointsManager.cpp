...
VRemoteDebuggerBreakpointsManager::VRemoteDebuggerBreakpointsManager(const XBOX::VString& inSourcesRoot) : fSourcesRoot(inSourcesRoot)
{
}
...
void VRemoteDebuggerBreakpointsManager::Set(OpaqueDebuggerContext inContext,const XBOX::VString& inUrl, intptr_t inSourceId, const XBOX::VString& inData)
{
	...
	//VectorOfVString	filenameVect;
	//VRemoteDebuggerBreakpointsManager::GetAllJSFileNames(filenameVect);
	...
	if ( testAssert(itCtx != fCtxs.end()) )
	{
		...
		if (itSource != (*itCtx).second.end())
		{
			...
			if (inUrl.GetLength())
			{
				testAssert(  sourcePath == outRelativePosixPath );
			}
		}
		else
		{
