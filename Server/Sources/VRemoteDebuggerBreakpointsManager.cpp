...
void VRemoteDebuggerBreakpointsManager::Load()
{
	...
	else if ((VRIAServerApplication::Get()->GetRunningMode() & eSRM_WITH_SOLUTION) == 0)
...
void VRemoteDebuggerBreakpointsManager::Save()
{
	...
	if (defaultSolutionFolderPath.IsValid() && defaultSolutionFolderPath.IsFolder())
	{
		VFolder *vfldrDefaultSolution = new VFolder( defaultSolutionFolderPath);
		...
		for( VectorOfProjectItems::size_type idx = 0; idx < items.size(); idx++ )
		{
			if (items[idx]->GetKind() == VProjectItem::ePROJECT)
			{
				...
				if (vProj && vProj->GetName(projName))
				{
					...
					VFilePath defaultProjectFilePath;
					VRIAServerApplication::Get()->GetDefaultProjectFilePath( defaultProjectFilePath);
					VString adminProjectName;
					defaultProjectFilePath.GetFileName( adminProjectName, false);
					if (projName != adminProjectName)
					{
...
{
	xbox_assert((VRIAServerApplication::Get()->GetRunningMode() & eSRM_WITH_SOLUTION) == 0);
