...
bool VJSDebuggerSettings::UserCanDebug(IAuthenticationInfos* inAuthenticationInfos) const
{
	...
	if (!uagSession)
	{
		...
		else
		{
			...
			uagSession = uagDirectory->MakeDefaultSession(false, &vError);
...
{
#if WITH_SANDBOXED_PROJECT
	if (((VRIAServerApplication::Get()->GetRunningMode() & eSRM_WITH_SOLUTION) == 0) ? (fProject == NULL) : (fServerSolution == NULL))
#else
	if ( fServerSolution == 0 )
#endif
	{
...
{
#if WITH_SANDBOXED_PROJECT
	xbox_assert(((VRIAServerApplication::Get()->GetRunningMode() & eSRM_WITH_SOLUTION) == 0) ? (fProject != NULL) : (fServerSolution != NULL));
#else
	...
	else
	{
		...
		VError							vError = VE_OK;
		CUAGSession*					cSession = uagDirectory-> MakeDefaultSession (false, &vError );
		xbox_assert ( vError == VE_OK );
		VUUID							vuuidAdminGroup;
		bool							bOK = uagDirectory-> GetSpecialGroupID ( CUAGDirectory::AdminGroup, vuuidAdminGroup );
		xbox_assert ( bOK );
...
#if WITH_SANDBOXED_PROJECT
	else if (((VRIAServerApplication::Get()->GetRunningMode() & eSRM_WITH_SOLUTION) == 0) && (fProject != NULL))
