...
namespace SolutionOpeningParametersKeys
{
	...
	CREATE_BAGKEY_NO_DEFAULT_SCALAR( customAdministratorPublishingOptions, XBOX::VLong, sLONG);
#endif
...
#if ALLOW_CUSTOMIZE_ADMIN_PUBLISHING_OPTIONS
void VRIAServerSolutionOpeningParameters::SetAdministratorPublishingOptions( EHTTPServerPublishingOptions inPublishingOptions)
{
	SolutionOpeningParametersKeys::customAdministratorPublishingOptions.Set( fBag, inPublishingOptions);
}


bool VRIAServerSolutionOpeningParameters::GetAdministratorPublishingOptions( EHTTPServerPublishingOptions& outPublishingOptions) const
{
	return SolutionOpeningParametersKeys::customAdministratorPublishingOptions.Get( fBag, (sLONG&)outPublishingOptions);
}
#endif
...
VRIAServerSolution* VRIAServerSolution::OpenSolution( VError& outError, VSolutionStartupParameters *inStartupParameters)
{
	...
	{
		outError = VE_RIA_CANNOT_OPEN_DESIGN_SOLUTION;
	}
...
VError VRIAServerSolution::_Open( VSolution* inDesignSolution, VRIAServerSolutionOpeningParameters *inOpeningParameters)
{
	...
	if (err == VE_OK)
	{
		...
		if (err == VE_OK || fState.inMaintenance)
		{
			...
			if (err == VE_OK || fState.inMaintenance)
			{
				// Get the ServerAdmin project file path
				VFilePath serverAdminProjectPath;
				VRIAServerApplication::Get()->GetDefaultProjectFilePath( serverAdminProjectPath);
				...
				if (fApplicationsMutex.Lock())
				{
					...
					if ((VRIAServerApplication::Get()->GetRunningMode() & eSRM_WITH_SANDBOXED_ADMINISTRATOR) != 0)
						// the administrator project remains started until the server quit
						hasAdmin = true;
				#endif
						...
						if (*iter != NULL)
						{
							...
							if (projectOpeningParams != NULL)
							{
								...
								if (isServerAdminProject)
								{
									...
									if (fOpeningParameters->GetAdministratorPublishingOptions( customPublishingOptions))
										projectOpeningParams->SetCustomPublishingOptions( customPublishingOptions);
								#endif
									...
									projectOpeningParams->SetIsAdministrator( true);
								#endif
								}
