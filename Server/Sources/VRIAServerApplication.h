...
public:
	...
	virtual	~VRIAServerStartupParameters();
			...
			void								SetServerRunningMode( EServerRunningMode inRunningMode);
			EServerRunningMode					GetServerRunningMode() const;

			void								SetJavaScriptAPIOptions( EJavaScriptAPIOptions inOptions);
			EJavaScriptAPIOptions				GetJavaScriptOptions() const;
			...
			void								SetAdministratorPublishingOptions( EHTTPServerPublishingOptions inPublishingOptions);
			bool								GetAdministratorPublishingOptions( EHTTPServerPublishingOptions& outPublishingOptions) const;
		#endif
			...
			void								SetWithNewAdmin( bool inWithNewAdmin);
			bool								GetWithNewAdmin() const;
...
public:
	...
	virtual	bool							Init(VProcess::InitOptions inOptions);
			...
			EServerRunningMode				GetRunningMode() const;
			EJavaScriptAPIOptions			GetJavaScriptAPIOptions() const;
	...
	virtual	XBOX::VString					GetLogSourceIdentifier() const;
			...
			/*	Information about the solution and the project which are opened by default when the server starts.
				The default project will take the behaviour of the administrator project
			*/
			void							GetDefaultSolutionFilePath( XBOX::VFilePath& outPath) const;
			void							GetDefaultProjectFilePath( XBOX::VFilePath& outPath) const;
...
private:
	...
	virtual	XBOX::VError					UninitializeJSContext( XBOX::VJSGlobalContext* inContext);
			...
			void							_OnJavaScriptFileEvaluationDone( XBOX::VString inResult);
		...
		#if WITH_SANDBOXED_PROJECT
			// @brief	Open the administrator project in sandboxed mode
			XBOX::VError					_OpenAdministratorProject();
	...
	static	void							_CleanupUserCacheFolder();
			void							_CleanupRunningServerFiles() const;
	...
	mutable	XBOX::VCriticalSection			fProjectsCollectionMutex;
			...
			VRIAServerJSFileRunner			*fJSFileRunner;
