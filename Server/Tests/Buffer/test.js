...
var testCase = {
    ...
    testBufferToBlob: function() {
        ...
        Y.Assert.isTrue( zeroBuffer.length == 0);

        var zeroBlob = zeroBuffer.toBlob();
        Y.Assert.isTrue( zeroBlob.size == 0);
        Y.Assert.isTrue( zeroBlob.toString() == '');
		
		// If type cannot be determined, set type as an empty string (section 6.1 of W3C File API).
		
        Y.Assert.isTrue( zeroBlob.type == '');				
        
        // test string buffer to Blob conversion
        var buffer = new Buffer( 'abcdef');
        Y.Assert.isTrue( buffer.length == 6);

        var blob = buffer.toBlob( 'text/plain');
        Y.Assert.isTrue( blob.size == 6);
        Y.Assert.isTrue( blob.toString() == 'abcdef');
        Y.Assert.isTrue( blob.type == 'text/plain');

		// test buffer slice to Blob conversion
        blob = buffer.slice(1,2).toBlob( 'text/plain');
        Y.Assert.isTrue( blob.size == 1);
        Y.Assert.isTrue( blob.toString() == 'b');
        Y.Assert.isTrue( blob.type == 'text/plain');
        
        // test binary blob to Buffer conversion
        blob = new Blob( 4, 42);
        buffer = blob.toBuffer();
        Y.Assert.isTrue( buffer.length == 4);
        Y.Assert.isTrue( buffer.readInt8(0) == 42);
        Y.Assert.isTrue( buffer.readInt8(1) == 42);
        Y.Assert.isTrue( buffer.readInt8(2) == 42);
        Y.Assert.isTrue( buffer.readInt8(3) == 42);

		// test blob slice to buffer conversion
        buffer = blob.slice(1,2).toBuffer();
        Y.Assert.isTrue( buffer.length == 1);
        Y.Assert.isTrue( buffer.readInt8(0) == 42);
    },
