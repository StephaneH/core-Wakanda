...
public class CharSetUtil {
	...
	public static char[] getSupportedCharacrters(String charsetName,
			boolean nonASCII) {
	...
	public static char[] getNonASCIISupportedCharacrters(String charsetName) {
		return getSupportedCharacrters(charsetName, true);
	}
	...
	public static boolean encodeUnicodeContentToFile1(String content,
			String charset, String path) {

		OutputStreamWriter unicodeContentFileWriter;
		try {
			unicodeContentFileWriter = new OutputStreamWriter(
					new FileOutputStream(path), charset);
			unicodeContentFileWriter.write(content);
			unicodeContentFileWriter.close();
			return true;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static byte[] encodeUnicodeContentToFile2(String content,
			String charset, String path) {
		try {
			// First encode
			CharsetEncoder encoder = Charset.forName(charset).newEncoder();
			ByteBuffer encodedContentBuffer = encoder.encode(CharBuffer
					.wrap(content));
			byte[] encodedContent = encodedContentBuffer.array();

			// then write result to file
			FileOutputStream encodedContentOutStream = new FileOutputStream(
					path);
			encodedContentOutStream.write(encodedContent);
			encodedContentOutStream.flush();
			encodedContentOutStream.close();
			// Everything is good
	...
	public static boolean encodeUnicodeContentToFile3(String content,
			String charset, String path) {
		return encodeUnicodeContentToFile2(content, charset, path) != null;
	}
