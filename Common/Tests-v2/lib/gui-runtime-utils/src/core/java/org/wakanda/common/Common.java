...
{	
	...
	public static Process startServer(String solutionPath) 
	{
		...
		if (os.contains("mac") || os.contains("linux"))
		{
			cmdLine = new String[] {
					System.getenv("SHELL"),
					"-c",
					Paths.serverPath+" \"" + solutionPath + "\" > /dev/null 2>&1 &"
			};
			logger.info(System.getenv("SHELL") + " -c " + Paths.serverPath.concat("\\") + "\" " + solutionPath + "\" > /dev/null 2>&1 &");
		}
		else if (os.contains("win"))
		{
			cmdLine = new String[] {
					"CMD",
					"/C",
					"START /B CALL \""+Paths.serverPath + "\" \"" + solutionPath + "\" > NUL 2>&1"
			};
			logger.info("CMD" + " /C START /B CALL \"" + Paths.serverPath + "\" \"" + solutionPath + "\" > NUL 2>&1");
		}
	...
	{
		copyFile(Common.class.getResourceAsStream("globalsolutions/WebEditors.zip"), new File(Paths.tempTargetPath + "WebEditors.zip"));
	...
	public static void copyDrivers() throws IOException
	{
		if(getOS().equals("win"))	
		{
			copyFile(Common.class.getResourceAsStream("drivers/win/chromedriver.exe"), new File(Paths.tempTargetPath + "chromedriver.exe"));
		}
		else
		{
			copyFile(Common.class.getResourceAsStream("drivers/mac/chromedriver"), new File(Paths.tempTargetPath + "chromedriver"));
		}
	}
	 ...
	 */
	 public static void copyStaticRessources() 
	 {
		 ...
		 {
			 logger.warning("Could not copy a static resource to temporary folder. Resources were most likely already copied!\n" + e.getMessage());
		 }
	 }
