...
public class SeleniumActions 
{
	...
	{	
		...
		String tempChromeDriver = Paths.tempTargetPath + "chromedriver";
			...
			{
				// Sets +RWX Permissions for WebDriver
				Runtime.getRuntime().exec("chmod 777 " + tempChromeDriver);
			}
	...
	public void updateGUIfile(Class<?> kls, String callingMethodName) throws ClassNotFoundException
	{
		...
		try
		{
			...
			logger.info("GUI file succesfully loaded.");
		} catch (Exception e) {
			logger.info("GUI file was not loaded. Using the previous GUI file.");
		}
	}
	 ...
	 * 
	 * Failing to call upon this method will result on overlapping every TestCase initial state on the same page.
	 * 
	 * Should be marked as deprecated once integrated on Wakanda.
	 */
	 ...
	 * 
	 */
	public int countWebElements(List<WebElement> rows) {
		return rows.size();
	}
	 ...
	 */
	public void waitWebElementToBeClickable(WebDriver driver,By by,int timeout){
