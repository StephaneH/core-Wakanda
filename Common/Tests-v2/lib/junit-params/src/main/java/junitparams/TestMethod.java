...
public class TestMethod {
	...
	public TestMethod(FrameworkMethod method, TestClass testClass) {
		...
		this.parametersAnnotation = frameworkMethod
	...
	public static List<TestMethod> listFrom(
			List<FrameworkMethod> annotatedMethods, TestClass testClass) {
	...
	public boolean equals(Object obj) {
		...
		return frameworkMethod.getName().equals(
				((TestMethod) obj).frameworkMethod.getName());
	}
	...
	Description describe() {
		if (isNotIgnored()) {
			Description parametrised = Description
					.createSuiteDescription(name());
			Object[] params = parametersSets();
			for (int i = 0; i < params.length; i++) {
				//Object paramSet = params[i];
				//String name = Utils.stringify(paramSet, i) + " (" + name() + ")";
				String name = name() + " [" + i + "]";
				parametrised.addChild(Description.createTestDescription(
						testClass(), name, annotations()));
			}
			return parametrised;
		} else {
	...
	private Object[] invokeMethodWithParams(String methodName,
			Class<?> testClass) {
		Method provideMethod = findParamsProvidingMethodInTestclassHierarchy(
	...
	private Object[] invokeParamsProvidingMethod(Class<?> testClass,
		...
		try {
			...
			throw new RuntimeException("The return type of: "
					+ provideMethod.getName() + " defined in class "
					+ testClass
			...
			if (prividerMethod.getName().startsWith("provide")) {
				...
				try {
					result.addAll(Arrays
							.asList(getDataFromMethod(prividerMethod)));
				} catch (Exception e) {
					throw new RuntimeException(
							"Cannot invoke parameters source method: "
									+ prividerMethod.getName(), e);
				}
			}
		}
		return result;
	...
	private Object[] getDataFromMethod(Method prividerMethod)
			throws IllegalAccessException, InvocationTargetException {
		return encapsulateParamsIntoArrayIfSingleParamsetPassed((Object[]) prividerMethod
				.invoke(null));
	}

	private Object[] encapsulateParamsIntoArrayIfSingleParamsetPassed(
			Object[] params) {
	...
	void warnIfNoParamsGiven() {
		if (isNotIgnored() && isParameterised() && parametersSets().length == 0)
			System.err.println("Method " + name()
					+ " gets empty list of parameters, so it's being ignored!");
	}
