...
public class Utils {
	...
	utilsLogger.info(ImageIO.write(image, "png", new File("Errors" + "//"+ Name)));
    }
    ...
    private static void killApplication(String appWin, String appMac, String appLinux) {
	String command;
	String[] shellCommand;
	String os = getOS();
	String shell;
	...
	try {
	    ...
	    utilsLogger.error(e.getMessage());
	}
    }
    ...
    public static boolean isApplicationRunning(String appWin, String appMac,String appLinux) {
	...
	if (os.contains("mac")) {
	    // Mac
	    shell = System.getenv("SHELL");
	    command = "ps -ef | grep -v grep | grep \"" + appMac+ "\" && (exit 0) || (exit 1)";
	    shellCommand = new String[] { shell, "-c", command };
	} else if (os.contains("Linux")) {
	    // Linux
	    shell = System.getenv("SHELL");
	    ...
	    command = "ps -aux | grep -v grep | grep " + appLinux + " && (exit 0) || (exit 1)";
	    shellCommand = new String[] { shell, "-c", command };
	} else {
	    // Win
	    shell = "CMD";
	    command = "tasklist /NH /FI \"imagename eq " + appWin+ "\" | find /i \"" + appWin + "\" && (exit 0) || (exit 1)";
	    shellCommand = new String[] { shell, "/C", command };
	}
	try {
	...
	return isApplicationRunning("Wakanda Server.exe", "Wakanda Server","Wakanda");
    }
    
    public static boolean isStudioRunning() {
	return isApplicationRunning("Wakanda Studio.exe", "Wakanda Studio","Wakanda");
    }
	...
	if (os.contains("mac")) {
	    ...
	    // Win
	    shell = "CMD";
	    command = "START /B CALL \"" + serverPath + "\"" + solutionPath + " > NUL 2>&1";
	    shellCommand = new String[] { "CMD", "/C", command };
	}
	...
	try {
	    ...
	    boolean isWakanda = response.getFirstHeader(HttpHeaders.SERVER).getValue().contains("Wakanda");
	...
	if (Utils.getOS().equals("win")) {
	    ...
	    focused = User32.INSTANCE.SetForegroundWindow(user32.FindWindow(null, title));
	}
	return focused;
    }
	...
	else
	{
		...
		utilsLogger.error("Impossible de mettre l'application:"+title+" en premier plan");
	    }
	}
	...
	copyFile(TemplateTestClass.class.getResourceAsStream("globalsolutions/undoRedo.zip"), new File(tempFolder+"/target/temp/undoRedo.zip"));
	extractFolder(Utils.tempFolder+"/target/temp/undoRedo.zip");
	
	if(Utils.getOS().equals("win"))			  
	    copyFile(TemplateTestClass.class.getResourceAsStream("captures/win/chromedriver.exe"), new File(tempFolder+"/target/temp/chromedriver.exe"));
	else
	{
	    copyFile(TemplateTestClass.class.getResourceAsStream("captures/mac/chromedriver"),new File(tempFolder+"/target/temp/chromedriver"));
	    System.setProperty("webdriver.chrome.driver", Utils.tempFolder+"/target/temp/chromedriver");
	}
	copyFile(TemplateTestClass.class.getResourceAsStream("globalsolutions/designer-test.js"), new File(System.getProperty("user.dir")+"/target/temp/designer-test.js"));
    }
}
