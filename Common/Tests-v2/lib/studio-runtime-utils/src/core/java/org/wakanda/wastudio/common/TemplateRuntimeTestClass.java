...
public class TemplateRuntimeTestClass extends TemplateTestClass {
	...
	if (result == null) {
		...
		case imageCurrentBrowser: 
		    result = getClass().getResource(Utils.getOS() + "//" + currentBrowserName).toString();
		    break;
		case solutionCurrentBrowser:
		    URL url = getClass().getResource("solutions" + "//" + currentBrowserName);
		    URI uri = url.toURI();
	...
	try {
	    wait(getFileFromRessources("firefoxLogo.png",FileSelector.InitalState), 8);
	...
	try {
	    wait(getFileFromRessources("chromeLogo.png",FileSelector.InitalState), 8);
	    // click(Utils.getInitialStateImage("MaximazeBrowser.png"));
	    // maximizeWindow();
	} catch (FindFailed e) {
	    logger.error(e.getMessage());
	...
	try {
	    wait(getFileFromRessources("ieLogo.png", FileSelector.InitalState),8);
	    // maximizeWindow();
	    // click(Utils.getInitialStateImage("MaximazeBrowser.png"));
	} catch (FindFailed e) {
	    logger.error(e.getMessage());
