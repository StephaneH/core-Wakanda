...
#include "JavaScript/Sources/VJSRuntime_file.h"
...
void JSReturnFolderOrFile( const VFilePath& inPath, const VFolder* inParentFolder, XBOX::VJSParms_callStaticFunction& ioParms, size_t inKindParamIndex, size_t inFormatParamIndex)
{
	bool done = false;
	if (!inPath.IsEmpty())
	{
		...
		if (ioParms.GetStringParam( inKindParamIndex, kind))
		{
			if (kind.EqualToString( L"path", true))
			{
				...
				bool posix = true;
			#if VERSIONWIN
				if (!ioParms.IsNullParam( inFormatParamIndex))
...
#define BUFFER_SIZE	4096
			...
			{
				bool encoded = (!ioParms.IsNullParam( inFormatParamIndex)) ? ioParms.GetBoolParam( inFormatParamIndex, L"encoded", L"notEncoded") : true;
			...
			else if (kind.EqualToString( L"relativePath", true))
			{
				...
				if (inParentFolder != NULL)
				{
					VFilePath parentFolderPath;
					bool posix = true;
					...
					inParentFolder->GetPath( parentFolderPath);
