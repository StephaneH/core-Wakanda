	...
	if( catalogItem )
	{
		...
		if( catalog )
		{
			...
			if( doUpdateCatalogRelatedScriptsList )
				catalog->UpdateCatalogRelatedScriptsList();
		}
	}
}
...
VError VCatalog::_ParseCatalogAndCreateProjectItems( CDB4DBase* inDB4DBase, VProjectItem *inProjectItem, VValueBag *outCatalogBag)
{
	...
	if (testAssert( outCatalogBag != NULL))
	{
		...
		if (err == VE_OK)
		{
			...
			{
				// Fix WAK00085189, ne créer les items fils que s'ils n'existent pas déjà
				// car les doublons ne sont pas autorisés
				VProjectItem* existingItem = inProjectItem->FindChildByName((*oneEntityModel)->GetEntityName());
				if( existingItem == NULL )
				{
					...
					VProjectItemDataClass* dataClassBehaviour = dynamic_cast<VProjectItemDataClass *>(newProjectItemEntityModel->GetBehaviour());
					if (dataClassBehaviour)
					{
						if ((*oneEntityModel)->IsRemote())
							dataClassBehaviour->SetType(VProjectItemDataClass::eRemote);
					}
...
void VCatalog::UpdateCatalogRelatedScriptsList()
{
	sLONG flags = DB4D_Open_As_XML_Definition | DB4D_Open_No_Respart | DB4D_Open_StructOnly | DB4D_Open_Allow_Temporary_Convert_For_ReadOnly;
	VError err = VE_OK;
	...
	XBOX::VFile catalogFile(catalogPath);
	XBOX::VString catalogContent;
	if ( catalogFile.Exists() )
	{
		VFileStream stream( &catalogFile );
		if (VE_OK == stream.OpenReading())
		{
			stream.GuessCharSetFromLeadingBytes( VTC_DefaultTextExport );
			stream.GetText( catalogContent );
			stream.CloseReading();
			catalogContent.ConvertCarriageReturns( eCRM_CR );
		}
	}
	...
	VFile permissionsFile( permissionsPath );
		
	CDB4DManager* dB4DBaseManager = CDB4DManager::RetainManager();

	unordered_map_VString< VRefPtr< VFile > >* relatedScripts = new unordered_map_VString< VRefPtr< VFile > >;
	CDB4DBase* dB4DBase = dB4DBaseManager->OpenBase(catalogContent, flags, &err, catalogPath, relatedScripts, &permissionsFile );
	if (dB4DBase != NULL)
	{
		VFile* errorFile = NULL;
		VFilePath path;
		VString message;
		sLONG lineNumber;
		
		dB4DBase->CatalogJSParsingError( errorFile, message, lineNumber );
		if( errorFile != NULL )
			path = errorFile->GetPath();

		fModelParsingErrorSignal.Trigger(path, lineNumber, message);
        dB4DBase->CloseAndRelease(false);
	}

	// Add waModel file to the list
	VFile catalogWaModel(catalogPath);
	if( catalogWaModel.Exists() )
		fRelatedScripts.insert(catalogPath);

	VString catalogName;
	catalogWaModel.GetNameWithoutExtension(catalogName);
	
	// and its associated .js
	catalogPath = catalogPath.ToParent();
	catalogPath.ToSubFile(catalogName + ".js");
	VFile catalogJs(catalogPath);
	if( catalogJs.Exists() )
		fRelatedScripts.insert(catalogPath);
	
	for ( unordered_map_VString< VRefPtr< VFile> >::const_iterator i = relatedScripts->begin(); i != relatedScripts->end(); ++i )
		fRelatedScripts.insert( (*i).second->GetPath() );
	
	relatedScripts->clear();
	delete relatedScripts;
	relatedScripts = NULL;
	
	ReleaseRefCountable(&dB4DBaseManager);
}
