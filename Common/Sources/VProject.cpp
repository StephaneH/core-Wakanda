...
#if ACTIVATE_CORE_AUTOCOMPLETE_DOC_PARSING
...
#if ACTIVATE_SERVER_SIDE_KLUDGE_PARSING
	LoadKludgeSymbols( fSymbolTable );
#endif//ACTIVATE_SERVER_SIDE_KLUDGE_PARSING
...
*/
/*
VError VCatalog::SParseCatalogAndCreateProjectItems( CDB4DBase* inDB4DBase, VProjectItem *inProjectItem, VValueBag *outCatalogBag)
{
	...
	if (testAssert( outCatalogBag != NULL))
	{
		...
		else if ( inProjectItem->ConformsTo(RIAFileKind::kRemoteCatalogFileKind))
		{
			// Another method to get entity models
			err = inDB4DBase->RetainAllEntityModels(entityModels, dB4DBaseContext, true);
		}
		if (err == VE_OK)
		{
