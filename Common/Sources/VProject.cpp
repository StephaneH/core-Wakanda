...
sLONG VProject::BackgroundDeleteFile( VTask *inTask )
{
	...
	if( inSymTable != NULL )
	{
		...
		if( files.size() == 1 )
		{
			Symbols::IFile* fileToDelete = files.front();
			
			if( inSymTable->UseNextGen() == true )
			{
				inSymTable->DeleteSource(fileToDelete->GetID());
			}
			else
			{
				inSymTable->DeleteSymbolsForFile(fileToDelete , false);
			}
		}
...
void VProject::ParseExceptions( VFilePath& inFolder )
{
	VFilePath exceptions = inFolder;

	exceptions.ToSubFile( "parsingExceptions.json" );

	VFile file( exceptions );

	if ( file.Exists() )
	{
		VJSONValue val;
		if ( VE_OK == val.LoadFromFile( &file ) )
		{
			if ( val.IsObject() )
			{
				VJSONObject* obj = val.GetObject();
				VJSONValue excludedFolders = obj->GetProperty( "excludedFolders" );

				if ( excludedFolders.IsArray() )
				{
					VString folderName;
					VJSONArray* ar = excludedFolders.GetArray();
					for ( size_t i = 0; i < ar->GetCount(); i++ )
					{
						VJSONValue folder = (*ar)[ i ];
						folder.GetString( folderName );
						fParsingExceptions.insert( folderName );
					}
				}
			}
		}
...
bool VProject::IsParsingAllowed( VProjectItem* inProjectItem )
{
	bool allowed = true;
	...
	{
		VFilePath projectFolderPath;
		project->GetProjectFolderPath( projectFolderPath );

		const std::set<VString>* exceptions = project->GetParsingExceptions();
		...
		while ( path.GetParent( path ) && allowed )
		{
			...
			if ( exceptions->find( folderName ) != exceptions->end() )
			{
				allowed = false;
				break;
			}
		...
		for (std::vector< VFilePath >::const_iterator iter = inFilePaths.begin(); iter != inFilePaths.end(); ++iter) 
		{
			...
			if (projectItem) 
			{
				...
				if (parsingManager != NULL && fSymbolTable != NULL)
				{
					...
					{
						if ( IsParsingAllowed( projectItem ) )
						{
...
VError VProject::Load( bool inOpenSymbolsTable)
{
	...
	DebugMsg( L"Load project " + lName + L"\n");
		...
		// loading parsing exceptions
		{
			VFolder* resourcesFolder = VProcess::Get()->RetainFolder( VProcess::eFS_Resources );
			VFilePath ressourcesFolderPath;
			resourcesFolder->GetPath( ressourcesFolderPath );
			resourcesFolder->Release();

			VFilePath projectPath;
			GetProjectFolderPath( projectPath );

			ParseExceptions( ressourcesFolderPath );
			ParseExceptions( projectPath );
		}
...
VProjectItem* VProject::GetProjectItemFromFullPath( const VString& inFullPath) const
{
	...
	unordered_map_VString< VProjectItem* >::const_iterator i = fMapFullPathProjectItem.find( fullPath );
		...
		if (inItem->IsPhysicalFolder())
		{
			...
			if (inItem->GetFilePath( folderPath))
			{
				...
				if (folder != NULL)
				{
					...
					VString fileName, fileExtension;
					...
					for (VFileIterator fileIter( folder, FI_WANT_FILES | FI_WANT_INVISIBLES) ; fileIter.IsValid() && (err == VE_OK) ; ++fileIter )
					{
						...
						fileIter->GetExtension( fileExtension );

						if (    fileExtension == RIAFileKind::kProjectBackupFileExtension
							 || fileExtension == RIAFileKind::kCacheUAGFileExtension
							 || fileExtension == RIAFileKind::kProjectFileExtension )
						{
							continue;
						}
						...
						if (fileItem == NULL)
						{
							if ( fileExtension == RIAFileKind::kCatalogFileExtension || fileExtension == RIAFileKind::kRemoteCatalogFileExtension )
							{
								VError lErr = VE_OK;
...
VProjectItem *VProject::_GetEntityModelProjectItem( VProjectItem *inCurItemToSearch )
{
	...
	{
		if (it->CheckExtension( RIAFileKind::kCatalogFileExtension))
			ret = it;
		
		if (!ret)
			ret = _GetEntityModelProjectItem( it );
	}
...
void VProject::_GetProjectItemsByExtension( VProjectItem *inProjectItem, const XBOX::VString &inExtension, VectorOfProjectItems &outItems )
{
	...
	if ( inProjectItem->CheckExtension( inExtension ) )
	{
