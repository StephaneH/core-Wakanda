...
public:
	...
	XBOX::VError	SetStartupProject( VProject *inProject);
	bool			IsStartupProject( const VProject *inProject) const;
	VProject*		GetStartupProject() const;
	...
	void					GetPathRelativeToFolderPath(const XBOX::VString& inFolderPath, const XBOX::VString& inFullPath, XBOX::VString& outRelativePath) const;
...
class VProfilerCall;
class VProfilerCallDesc;

typedef std::map< uLONG , VProfilerCallDesc* >					MapOfProfilerCallDesc;
typedef std::map< uLONG , VProfilerCallDesc* >::iterator		MapOfProfilerCallDesc_iter;
typedef std::map< uLONG , VProfilerCallDesc* >::const_iterator	MapOfProfilerCallDesc_citer;

class VSolutionProfiler : public XBOX::VObject
{
public:
			VSolutionProfiler();
	virtual	~VSolutionProfiler();

	static	bool					Init();
	static	void					DeInit();
	static	VSolutionProfiler*		Get() { return sProfiler; }

			bool					StartProfiling( const XBOX::VString& inProfilName, const XBOX::VFilePath& inLogFolderPath, const XBOX::VString& inSolutionName);
			void					EndProfiling();
			bool					IsProfilingStarted() const { return fProfilStarted; }

			void					SetCallDescription( uLONG inID, const XBOX::VString& inCallName, const XBOX::VString& inMethodName);
			bool					EnterCall( uLONG inID);
			void					ExitCall( uLONG inID);

			bool					VFile_ConformsTo( XBOX::VFile *inFile, const XBOX::VString& inFileKindID);
			
private:
			void					_LogCall( XBOX::VSplitableLogFile *inLogFile, VProfilerCall *inCall, sLONG inIndentationCount, bool inRecursive) const;
			VProfilerCallDesc*		_GetCallDescription( uLONG inID)  const;

	static	VSolutionProfiler				*sProfiler;
			
			bool							fProfilStarted;
			XBOX::VString					fProfilName;
			XBOX::VFilePath					fLogFolderPath;
			XBOX::VString					fSolutionName;

			std::deque< VProfilerCall* >	fCallStack;

			MapOfProfilerCallDesc			fCallsDescription;
};



class VSolutionProfilerTester : public XBOX::VObject
{
public:
	VSolutionProfilerTester();
	virtual ~VSolutionProfilerTester();
...
private:
			...
			void _BasicMethod3();
			void _BasicMethod4();
			void _RecursiveMethod1( sLONG inCallsCount);
			void _RecursiveMethod2( sLONG inCallsCount);
			void _RecursiveMethod3( sLONG inCallsCount);
};
