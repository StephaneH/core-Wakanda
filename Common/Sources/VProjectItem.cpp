...
namespace ProjectItemBagKeys
{
	...
	CREATE_BAGKEY_WITH_DEFAULT_SCALAR( startupProject, XBOX::VBoolean, bool, false);
...
bool VProjectItem::ConformsTo( const XBOX::VString& inFileKindID) const
{
	...
	VSolutionProfiler::Get()->EnterCall( PROFILER_CALL_VProjectItem_ConformsTo);
#endif
...
#if SOLUTION_PROFILING_ENABLED
	VSolutionProfiler::Get()->ExitCall( PROFILER_CALL_VProjectItem_ConformsTo);
#endif
...
VProject* VProjectItem::GetProjectOwner()
{
	...
	{
		project = GetProjectItemProjectOwner()->GetBehaviour()->GetProject();
	}
...
#if SOLUTION_PROFILING_ENABLED
	VSolutionProfiler::Get()->EnterCall( PROFILER_CALL_VProjectItem_FindChildByRelativePath);
#endif
...
#if SOLUTION_PROFILING_ENABLED
	VSolutionProfiler::Get()->ExitCall( PROFILER_CALL_VProjectItem_FindChildByRelativePath);
#endif
...
{
#if SOLUTION_PROFILING_ENABLED
	VSolutionProfiler::Get()->EnterCall( PROFILER_CALL_VProjectItem_BuildFullPath);
#endif
	...
	projectItem->GetRelativePath(currentLocalPath);
...
#if SOLUTION_PROFILING_ENABLED
	VSolutionProfiler::Get()->ExitCall( PROFILER_CALL_VProjectItem_BuildFullPath);
#endif
...
bool VProjectItemFilter::Matches(VProjectItem* inProjectItem) const
{
	...
	if (!projectItemFilters.empty())
	{
		...
		for (VectorOfProjectItemFiltersConstIterator it = projectItemFilters.begin(); 
			 it != projectItemFilters.end(); ++it)
		{
			if (ok2 = (*it)->Matches(inProjectItem))
				break;
		}
		ok = ok2;
	}
