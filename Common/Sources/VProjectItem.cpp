...
namespace ProjectItemBagKeys
{
	...
	CREATE_BAGKEY_WITH_DEFAULT( tabPosition, XBOX::VString, "" );
}
...
bool VProjectItem::CheckExtension( const XBOX::VString& inExtension ) const
{
	return fName.GetLength() > inExtension.GetLength() && fName[ fName.GetLength() - inExtension.GetLength() - 1 ] == CHAR_FULL_STOP && fName.EndsWith( inExtension );
}
