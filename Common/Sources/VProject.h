...
public:
	...
	void						ParseExceptions( XBOX::VFilePath& inFolder );
	static bool					IsParsingAllowed( VProjectItem* inProjectItem );

	const std::set<XBOX::VString>*	GetParsingExceptions() const { return &fParsingExceptions; }
...
private:
			...
			// parsing exceptions : set of folder names where content should not be parsed.
			std::set<XBOX::VString> fParsingExceptions;
...
//	MapFullPathProjectItem		fMapFullPathProjectItem;
	XBOX::unordered_map_VString< VProjectItem* >	fMapFullPathProjectItem;
