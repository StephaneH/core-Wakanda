...
import static com.wakanda.qa.utils.AbstractBasicSettings.getOS;
...
public class ServerUtil {
	...
	private static boolean doLoadSolutionAndRunOrRestartServer(String pathToServer, String solution, HttpHost webAdmin,
	...
	public static boolean loadSolutionAndRunOrRestartServerSt(
			...
			HttpHost target) {
		return doLoadSolutionAndRunOrRestartServer(pathToServer, solution, webAdmin, target, getDefaultRequestor());

	}
	
	public static boolean loadSolutionAndRunOrRestartServerSt(String solution,
			HttpHost webAdmin, HttpHost target) {
		return loadSolutionAndRunOrRestartServerSt(getDefaultRequestor().getSetting().getServerPath(), solution,
				webAdmin, target);
	}
	...
	public boolean loadSolutionAndRunOrRestartServer(
			String pathToServer, String solution, HttpHost webAdmin,
			HttpHost target) {
		return doLoadSolutionAndRunOrRestartServer(pathToServer, solution, webAdmin, target, getRequestor());
					...
					if (fixedWait) {
						...
						if (loaded) {
							long end = GregorianCalendar.getInstance()
									.getTimeInMillis();
							logger.debug("Solution is ready (after "
									+ (end - start) + " sec)");
						}
					}
				}
	...
	public static boolean isServerRespondingSt() {
		return doIsServerResponding(getDefaultRequestor().getSetting().getWebAdminTarget(), getDefaultRequestor());
	}
	...
	private static boolean doIsSolutionDeployed(HttpHost target, Requestor requestor) {
		boolean isSolutionDeployed = false;
		HttpGet request = new HttpGet("/");
		try {
			HttpResponse response = requestor.execute(target,
					new HttpGet("/"));
			boolean is200Ok = response.getStatusLine().getStatusCode() == HttpStatus.SC_OK;
			boolean isWakanda = response.getFirstHeader(HttpHeaders.SERVER)
					.getValue().contains("Wakanda");
	...
	public static boolean isSolutionDeployedSt() {
		return isSolutionDeployedSt( getDefaultRequestor().getSetting().getDefaultTarget());
	}
