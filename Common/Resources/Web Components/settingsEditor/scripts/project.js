...
function projectActivate() {
	...
	if (jsonData.http) {
		...
		/* Enable SSL */
		document.getElementById('allowSSL').checked = eval((jsonData.http[0].allowSSL) ? jsonData.http[0].allowSSL
		...
		document.getElementById('allowSSL').checked = eval(defaultValue.http.allowSSL);
			...
			if(index != -1) {
				...
				switch (service.name) {
					...
					case "Builder handler":
						...
						//document.getElementById('modulePath_7').value = (service.modulePath)? service.modulePath: defaultValue.services[service.name].modulePath;
						var stack = [];
...
function projectOnChange() {
	...
	if(boo_OK) {
		...
		switch (id) {
			...
			/*case 'directoryIndex': // !! stored in service 'webApp'
                ...
                if(index == -1) {
                    ...
                    tempService[id] = value;

                    jsonData.service.push(tempService);
                    index = jsonData.service.length;
                } else {

                    jsonData.service[index][id] = value;
...
function displayPublishingInformation() {
	...
	/* DefaultHomePage */
	//txt_html +=         '<div title="The default Interface page for every folder in your application.">';
	//txt_html +=             '<label for="directoryIndex">Default Home Page:</label>';
	//txt_html +=             '<input id="directoryIndex" type="text" class="alpha" size="31"/>';
	//txt_html +=         '</div>';
...
function displaySecureConnections() {
	...
	txt_html +=     '<h3>Secure Connections<br />(SSL -TLS)</h3>';
	txt_html +=     '<div class="group">';

	/* Enable secure connections */
	txt_html +=         '<div title="">';
	txt_html +=             '<input id="allowSSL" type="checkbox" />';
	txt_html +=             '<label for="allowSSL">Enable secure connections</label>';
	txt_html +=         '</div>';
	...
	txt_html +=             '<input id="SSLMandatory" type="checkbox" />';
	txt_html +=         '</div>';
	...
	for ( i = 0; i < wakServices.length; i++) {
		...
		switch (id) {
			...
			case 7:
				...
				txt_html += '<div class="level6">';
				txt_html +=     '<label for="hardCache_7">Dynamic cache:</label>';
