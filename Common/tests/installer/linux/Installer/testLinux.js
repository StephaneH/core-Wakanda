...
var wakBinInstallerG = ["/opt/wakanda-server/bin/wakanda-server", "/opt/wakanda-enterprise-server/bin/wakanda-enterprise-server"];
var wakBinTgzG = [env.BUILD_TEST_DIR + "/wakanda-server/bin/wakanda-server", env.BUILD_TEST_DIR + "/wakanda-enterprise-server/bin/wakanda-enterprise-server"];
var PathInstallerG = ["/opt/wakanda-server/bin", "/opt/wakanda-enterprise-server/bin"];
var PathTgzG = [env.BUILD_TEST_DIR + "/wakanda-server/bin", env.BUILD_TEST_DIR + "/wakanda-enterprise-server/bin"];
var rootPathArchiveG = ["/wakanda-server", "/wakanda-enterprise-server"];
var wakPathTgzG = [env.BUILD_TEST_DIR + "/wakanda-server", env.BUILD_TEST_DIR + "/wakanda-enterprise-server"];
var wakPathG = ["/opt/wakanda-server", "/opt/wakanda-enterprise-server"];
var wakScriptG = ["/etc/init.d/wakanda", "/etc/init.d/wakanda"];
var wakSymLinkG = ["/usr/local/bin/wakanda-server", "/usr/local/bin/wakanda-enterprise-server"];
var nameG = ["wakanda-server", "wakanda-enterprise-server"];
...
var testCase = {
	...
	testUnarchiveCommunity_8: function() {
		var filesFoldersName = [PathTgzG[0] + "/Libs/RemoteEntity.so",
			                    ...
			                    PathTgzG[0] + "/Native\\ Components/LDAPComponent.bundle/Contents/Linux/LDAPComponent.so",
	...
	testUpgradeCommunityPackage_0: function() {
		...
		var pathToBin =  wakBinInstallerG[0];
		...
		var result = installerLinux.upgradeVersion(output, buildNumber, pathToBin);
	...
	testExecutionCommunityPackage_8: function() {
		...
		Y.Assert.areEqual(' *' + ' ' + name + ' ' + 'is running', result2, "Start Status isn't equal");
	...
	testExecutionCommunityPackage_9: function() {
		...
		Y.Assert.areEqual(' *' + ' ' + name + ' ' + 'is running', result2, "Restart Status isn't equal");
	...
	testExecutionCommunityPackage_11: function() {
		...
		Y.Assert.areEqual(' *' + ' ' + name + ' ' + 'is not running', result2, "Stop Status isn't equal");
	...
	testEnterpriseFilesExistingCommunityPackage_12: function() {
		var filesFoldersName = [PathInstallerG[0] + "/Libs/RemoteEntity.so",
			                          ...
			                          PathInstallerG[0] + "/Native\\ Components/LDAPComponent.bundle/Contents/Linux/LDAPComponent.so",
	...
	testRemovingCommunityPackage_end: function() {
		var name = nameG[0];
		installerLinux.purge(name);
		var wakPath = Folder(wakPathG[0]),
			wakExe = File(wakBinInstallerG[0]),
	...
	testUnarchiveEnterprise_8: function() {
		...
		var filesFoldersName = [PathTgzG[1] + "/Libs/RemoteEntity.so",
			                    ...
			                    PathTgzG[1] + "/Native\\ Components/LDAPComponent.bundle/Contents/Linux/LDAPComponent.so",
	...
	testUpgradeEnterprisePackage_0: function() {
		...
		var pathToBin =  wakBinInstallerG[1];
		installerLinux.upgrade(packagePath);
	...
	testEnterpriseFilesExistingEnterprisePackage_12: function() {
		...
		var filesFoldersName = [PathInstallerG[1] + "/Libs/RemoteEntity.so",
			                            ...
			                            PathInstallerG[1] + "/Native\\ Components/LDAPComponent.bundle/Contents/Linux/LDAPComponent.so",
	...
	testRemovingEnterprisePackage_end: function() {
		var name = nameG[1];
		installerLinux.purge(name);
		var wakPath = Folder(wakPathG[1]),
			wakExe = File(wakBinInstallerG[1]),
