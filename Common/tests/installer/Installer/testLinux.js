...
if (typeof env === 'undefined') {
	var packagePath = '~/Desktop/WakTest/Testing/wakanda_6.0-144583_amd64.deb',
		packagePathTgz = '~/Desktop/WakTest/Testing/wakanda_6.0-144583_amd64.tgz';
} else {
	var packagePathG = [env.BUILD_TEST_DIR + '/Installer.deb', env.BUILD_TEST_DIR + '/InstallerEnterprise.deb'];
	var packagePathTgzG = [env.BUILD_TEST_DIR + '/Installer.tgz', env.BUILD_TEST_DIR + '/InstallerEnterprise.tgz'];
}
var testCase = {
	...
	testUnarchiveCommunity_4: function() {
		...
		Y.Assert.areEqual('755 drwxr-xr-x', result, "Permission are wrong");
	},
	...
	testUnarchiveEnterprise_8: function() {
		//ODBCConnector isn't finished as yet for Enterprise Version : Libs musn't be there
		var filesODBCConnector = PathTgzG[1] + "/Native\\ Components/ODBCConnector.bundle/Contents/Linux/ODBCConnector.so";
		...
		for (var i = 0; i < filesFoldersName.length; i++) {
			...
			//ODBCConnector isn't finished as yet for Enterprise Version : Libs musn't be there
			var filesODBCCheckInstaller = SystemWorker.exec(scriptPath + " " + "enterpriseSpecific," + "'" + filesODBCConnector + "'");
			if (filesODBCCheckInstaller != null) {
				console.log("filesCheck is : " + filesODBCCheckInstaller.output.toString());
			} else {
				console.log("filesCheck failed : " + filesODBCCheckInstaller.error.toString());
			}
			var result = filesODBCCheckInstaller.output.toString().replace(/\r?\n/g, '');

			Y.Assert.areEqual(filesODBCConnector.replace("\\", "") + " " + "doesn't exist", result, filesODBCConnector + " " + "exist and mustn't exist");
	...
	testUpgradeEnterprisePackage_0: function() {
		var buildNumber = env.CHANGELIST;
		var packagePath = packagePathG[1];
		installerLinux.upgrade(packagePathG);
	...
	testInstallEnterprisePackage_1: function() {
		var packageFile = File(packagePathG[1]);
		Y.Assert.isTrue(packageFile.exists, packageFile.name + ' ' + "doesn't exist");
		var wakPath = Folder(wakPathG[1]);
		var name = nameG[1];
		var packagePath = packagePathG[1];
	...
	testEnterpriseFilesExistingEnterprisePackage_12: function() {
		//ODBCConnector isn't finished as yet for Enterprise Version : Libs musn't be there
		var filesODBCConnector = PathInstallerG[1] + "/Native\\ Components/ODBCConnector.bundle/Contents/Linux/ODBCConnector.so";
		//FolderWAFAzure isn't finished as yet for Enterprise Version : Libs musn't be there
