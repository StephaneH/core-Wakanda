 ...
 **/
 var BrowserDetect = {
    ...
    searchOS: function(needle) {
        var found = false;
        if (-1 !== navigator.appVersion.indexOf(needle)){
            found = true;
        }
        return found;
    },
...
$( 'document' ).ready( function ( ) {
    ...
    if(window.webkitNotifications && window.webkitNotifications.checkPermission() === 0){ // 0 is PERMISSION_ALLOWED
        ...
        if(((BrowserDetect.browser == "Safari" && BrowserDetect.version >= 6 && BrowserDetect.searchOS("Intel Mac OS X 10_8")) || BrowserDetect.isCompatible) && window.webkitNotifications && window.webkitNotifications.checkPermission() !== 0){
