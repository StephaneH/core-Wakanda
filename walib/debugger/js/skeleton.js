...
function treatOpenWindow( contextId ) {
    ...
    if ( ctx.length > 0 ) {
        ...
        if ( $( "#frame-" + contextId ).length <= 0 ) {
			...
			localStorage.breakpoints = ctx.data( 'breakpoints');
...
function treatMessage( msg ) {
        ...
        if ( $( "#" + jsonMsg.contextId ).length > 0 ) {
			...
			for ( var j = 0 ; j < jsonMsg.breakpoints.length ; j++ ) {
				...
				for ( var i = 0 ; i < bps.lines.length ; i++ ) {
					...
					tmp += '{"sourceFileId":"' + bps.filename + '","lineNumber":' + bps.lines[ i ] + ',"condition":"","enabled":true}';
    ...
    if ( jsonMsg.method == "removeContext" || jsonMsg.method == "hideContext" ) {
...
function checkDebuggerStatus(status){
    var result = (!!status.result) ? JSON.parse(status.result) : null;
    if(result){
        var cmd = null;
...
if ( !BrowserDetect.isCompatible ) {
    ...
    window.alert("Your browser is not compatible with the Wakanda remote debugger."); 
