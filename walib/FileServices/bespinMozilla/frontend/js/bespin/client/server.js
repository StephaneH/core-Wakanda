...
dojo.declare("bespin.client.Server", null, {
    ...
    // ** {{{ remoteEval(content, onSuccess, onFailure) }}}
    //
    // eval the given javascript code on server side
    //
    // * {{{content}}} javascript code to eval
    // * {{{onSuccess}}} fires if the deletion works
    // * {{{onFailure}}} fires if the deletion failed
    remoteEval: function(content, onSuccess, onFailure) {

        //var url = bespin.util.path.combine('/file/at', project, path);
        //content = "var toto = 'tata';\nreturn toto;";
        content = content.replace(/\n/g, "\\n");
        content = content.replace(/\r/g, "\\r");
        content = content.replace(/"/g, '\\"');
        var trimpath = '{"jsonrpc":"2.0-ria","id":2,"method":{"keys":[],"source":"' + content + '"},"params":[]}';
        var url = '/ssjs/execute?template=' + trimpath;
        var opts = { onSuccess: onSuccess };
        if (dojo.isFunction(onFailure)) opts.onFailure = onFailure;

        this.request('POST', url, trimpath, opts);
    },
