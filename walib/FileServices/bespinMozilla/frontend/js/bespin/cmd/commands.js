...
// ** {{{Command: remoteEval (evalServer)}}} **
bespin.cmd.commands.add({
    name: 'remoteEval',
    aliases: ['evalServer'],
    takes: ['filename', 'project'],
    preview: 'Eval the file contents on the server side',
    completeText: 'add the filename to eval, and optionally a specific project at the end',
    execute: function(instruction, args) {
        var project = args.project || bespin.get('editSession').project;
        var filename = args.filename;

        if (!project) {
            instruction.addErrorOutput("'remoteEval' only works with the project is set.");
            return;
        }

        if (!filename) {
            instruction.addErrorOutput("give me a filename to eval");
            return;
        }

        bespin.get('server').loadFile(project, filename, function(content) {
            bespin.get('server').remoteEval(content, function(jsonrpc) {
                var result = "Error on loading file";
                if (jsonrpc !== null) {
                    jsonrpc = eval("(" + jsonrpc + ")");
                    if (jsonrpc.result !== undefined) {
                        result = jsonrpc.result;
                    } else {
                        if (jsonrpc.error !== undefined) {
                            result = jsonrpc.error.message;
                        }
                    }
                }
                instruction.addOutput('Result RemoteEval : ' + result, true);
            }, function(xhr) {
                instruction.addErrorOutput("Wasn't able to eval the file <b>" + filename + "</b><br/><em>Error</em> (probably doesn't exist): " + xhr.responseText);
            });
        }, function(xhr) {
            instruction.addErrorOutput("Wasn't able to eval the file <b>" + filename + "</b><br/><em>Error</em> (probably doesn't exist): " + xhr.responseText);
        });
