...
(function() {
    function render(json) {
        ...
        for(var name in json.attributes) {
            s += ' ' + name + '="' + json.attributes[name].replace('&', '&amp;').replace('"', '&quot;') + '"';
        }
        s += '>';
        if(json.childNodes) {
            s += json.childNodes.map(render).join('');
        }
        s += '</' + json.nodeName + '>';
        return s;
    }
    ...
    /**
     * Render the element and its content as an html string
     * @returns {string}
     * @method renderOuterHTML
     */
    WAF.dom.Element.prototype.renderOuterHTML = function(value) {
        return render(this._json);
    };

    /**
     * Render the element's content as an html string
     * @returns {string}
     * @method renderInnerHTML
     */
    WAF.dom.Element.prototype.renderInnerHTML = function(value) {
        if(this._json.childNodes) {
            return this._json.childNodes.map(render).join('');
        }
        return '';
    };
})();
