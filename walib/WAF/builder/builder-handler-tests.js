...
var WPMmodulePath,WPM;//global so tht you could switch the whole test handler between the original and the fork of the WPM
...
function dispatcher(request, response){
    ...
    if(ACTIVATE_TEST_HANDLER){
        ...
        if(builderHandlerTests[functionToCall]){
            try{
//                WPMmodulePath = getWalibFolder().path+"WAF/builder/WPM/WPM";
                WPMmodulePath = getWalibFolder().path+"WAF/studioWeb/builder/WPM/WPM";
                WPM = require(WPMmodulePath);
...
//retrieves the whole WAF packages definition you need to provide to the WPM in the package parameter (in the WD2 it is cached in storage at server start)
function getWafPackageDefinition(){
...
//    debugger;
    //test 3 : get package
    var tmpPack = WPM.getPackage(packageFilePath,'WEBFOLDER',undefined,undefined,'force',getWafPackageDefinition());
...
builderHandlerTests['getPackageList'] = function (request, response){
    ...
    tmpPackageList = WPM.getPackageList(packageJsonUrlPath,"WEBFOLDER",undefined,undefined,'force',getWafPackageDefinition());
...
//    tmpPackageList["/myWebComponent2.waComponent/myWebComponent2.package.json"].TS = "2013-04-19T12:38:14.000Z";
    ...
    output += "=== packageList "+JSON.stringify(tmpPackageList,null,"\t");
    
    output += "\n=== packageListHelper.hasTopPackageChanged('"+packageJsonUrlPath+"')  ("+(new Date()).toUTCString()+")===\n";
...
builderHandlerTests['getPackage'] = function (request, response){
    ...
    output += JSON.stringify(WPM.getPackage(packageJsonUrlPath, path, 'desktop', false,'force', getWafPackageDefinition()), null, "\t")+"\n";
...
builderHandlerTests['ricardo'] = function (request,response){    
    ...
    var path = "WEBFOLDER";
    var packageJsonUrlPath = 'index.waPage/index.package.json';
    var packageJsonFilePath = fileHelper.getFilePath(packageJsonUrlPath,path);
    ...
    output += JSON.stringify(WPM.getPackageList(packageJsonUrlPath, path, 'desktop', false,'force', getWafPackageDefinition()), null, "\t")+"\n";
