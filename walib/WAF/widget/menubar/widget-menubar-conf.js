...
WAF.addWidget({
    ...
    onCreate: function(tag, param) {
        ...
        tag.setMargin = function menubar_set_margin(value, save, force) {
            ...
            // (creation may have two different transCodeId: 'PALETTE_DRAG', and '041' which happens when dragging a new widget
            // if it is a new action is started, undo would not remove the tabView which was created
            // fixes #WAK0087810
            if (!((D.env.transCodeId === 'PALETTE_DRAG' || D.env.transCodeId === '049') && D.env.lastWidgetIdCreated === this.getId())) {
