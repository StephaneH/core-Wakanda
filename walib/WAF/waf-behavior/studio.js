...
WAF.define('waf-behavior/studio', function() {
    ...
    Studio._getConf = function() {
        ...
        Object.keys(this._configuration.on).forEach(function(eventName) {
            ...
            // callbacks are merged without being filtered so can appear several times
            // we filter the list to execute each callback only once
            // #job000048
            
            // TODO: guess using jQuery.Callbacks() would be a good idea for that
            // since we can configure the behavior of the list to only have unique callbacks
            // using 'unique' flag...
            callbacks.forEach(function(elt) {
                if (filteredCallbacks.indexOf(elt) === -1) {
                    filteredCallbacks.push(elt);
                }
            });
            
            configuration[eventName] = function(event) {
                filteredCallbacks.forEach(function(callback) {
                    try {
    ...
    Studio._addAttribute = function(name, attribute) {
        ...
        attribute.context = ['protected']; // allow properties to be displayed in subwidgets
    };
     ...
     * @method addLabel
     * @param {object|string} [options] - Option object or default value
    ...
    Studio.addLabel = function(options) {
        ...
        this._addAttribute('data-label', {
            ...
            ready: function() {
                this.htmlObject.parents('tr').find('label').slice(0, 1).attr('title', options.description || '');
            }
        });
    ...
    */
    ...
    Studio.execute = function(widget, callback) {
        var ret;
        try {
            ret = callback.apply(widget, [].slice.call(arguments, 2));
        } catch(exception) {
            console.trace();
            console.error(widget, exception, callback);
            var message = 'An error occured will running the code of the custom widget.<br />';
            if (exception.message) {
                message += " Error message: " + exception.message + " <br>";
            }
            if (typeof (exception) === 'string') {
                mee.sage += " Error message: " + exception + " <br>";
            }
            if (exception.sourceURL && exception.sourceURL.indexOf('designer-') === -1) {
                message += " File: " + exception.sourceURL.split('/')[exception.sourceURL.split('/').length - 1] + " <br>";
            }
            if (exception.line) {
                message += " Line: " + exception.line + " <br>";
            }

            Designer.env.widget.errors = [{
                widget: widget.kind,
                error: message
            }];
            Designer.ui.dialog.showWidgetsError();
        }
        return ret;
    };
