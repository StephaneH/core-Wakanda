...
WAF.define('waf-behavior/properties', function() {
    ...
    var Behavior = WAF.require('waf-core/behavior'),
        ...
        Formatters = WAF.require('waf-core/formatters');
    ...
    /**
     * Called to destroy behaviors of a widget
     * @param {BaseWidget} master - The widget cloned from
     * @private
     * @memberof Properties
     * @instance
     * @method _destroyBehavior
     */
    Properties.prototype._destroyBehavior = function() {
        Object.keys(this.constructor._properties).forEach(function(key) {
            var property = this.constructor._properties[key];
            custom(property, 'destroy').call(this, key, property);
        }.bind(this));
    };
            ...
            if(property.bindable) {
                ...
                this[property.functionName].unbindDatasource = function() {
                    if(name in this._boundAttributes) {
                        if(this._boundAttributes[name].subscriber) {
                            this._boundAttributes[name].subscriber.unsubscribe();
                        } else {
                            delete this._boundAttributes[name];
                            this.fire('datasourceBindingChange', name);
                        }
                    }
                }.bind(this);
                ...
                this[property.functionName].boundDatasource = function() {
                    function makeBinding(binding) {
                        return {
                            datasourceName: typeof binding.datasource === 'string' ? binding.datasource : binding.datasourceName,
                            datasource:     binding.datasource,
                            attribute:      binding.attribute,
                            formatters:     (binding.formatters || []).map(function(formatter) {
                                return {
                                    // work even if the formatter class is missing
                                    formatter: Class.instanceOf(formatter, Formatters) && formatter || null,
                                    name:      formatter.constructor.kind || formatter.formatter,
                            ...
                            toString: function() {
                                var str = this.datasourceName;
                                if(this.attribute) {
                                    str += '.' + this.attribute;
                                }
                                this.formatters.forEach(function(formatter) {
                                    str += '|' + formatter.name;
                                    formatter['arguments'].forEach(function(argument) {
                                        str += ' ';
                                        if(typeof argument === 'object') {
