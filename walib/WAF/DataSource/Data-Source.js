...
WAF.DataSourceEmRelatedAttributeValue = function(emAtt, source){
	...
	this.getKey = WAF.DataSourceEmRelatedAttributeValue.getKey;
};
...
WAF.DataSourceEmRelatedAttributeValue.getKey = function() {
	var key = null;
	var currentEntity = this.owner._private.currentEntity;
	if (currentEntity != null) {
		key = currentEntity[this.name].getRelatedKey();
	}
	return key;
}
...
WAF.DataSource.removeListener = function(params) {
	...
	this.unsubscribe(function() {
		...
		// search for ID
		if (id !== undefined && data && data.ID === id) {   
			return true;
		}
...
WAF.DataSourceEm.prototype.initialize = function(config) {
	...
	this.clone = WAF.DataSourceEm.clone;
...
WAF.DataSourceEm.clone = function(newName) {
	var priv = this._private;
	var config = {
		"data-autoLoad": false,
		"data-initialQueryString": priv.initialQueryStr,
		"data-filter-attributes": priv.filterAttributes,
		"data-initialOrderBy": priv.initialOrderBy,
		binding: priv.initialBinding,
		"data-scope": priv.scope,
		id: newName
	}
	
	var newsource = WAF.dataSource.create(config);
	newsource.setEntityCollection(this.getEntityCollection());
	return newsource;
}
...
WAF.DataSourceEm._Init = function(config){
	...
	this.initialBinding = config.binding;
...
WAF.DataSourceEm._gotEntity = function(event){
	...
	if (datas.mustPublish) {
		if (datas.mustPublishCollection) {
	...
	if (event.error != null && options.keepOldCollectionOnError) {
		...
		else
		{
			...
			if (source._private.entityCollection.length == 0) {
				...
				if (datas.mustPublish) {
					//source.dispatch('onCollectionChange', options);
					datas.mustPublishCollection = true;
				}
		...
		case "relatedEntities":
			...
			if (otherEntity == null) {
				...
				if (otherAtt == null) {
					...
					var keepsel = null;
					if (priv.currentEntity != null)
						keepsel = priv.currentEntity.getKey();
...
WAF.DataSourceVar.prototype.initialize = function(config){
	...
	this._private.initialBinding = config.binding;
	...
	this.clone = WAF.DataSourceVar.clone;
...
WAF.DataSourceVar.clone = function(newName) {
	var priv = this._private;
	var config = {
		binding: priv.initialBinding,
		variableReference: priv.varRef,
		"data-scope": priv.scope,
		"data-source-type": priv.sourceType,
		"data-dataType": priv.dataClass.type,
		dataClass: priv.dataClass,
		id: newName
	}
	...
	if (arr != null) {
		...
		if (howMany > 0)
		{
			...
			for (var i = pos; i < pos+howMany; ++i)
			{
				...
				elem._private.keyAtt = source._private.keyAtt;
				elem.getAttributeValue = source.getAttributeValue;
				elem.getKey = source.getKey;
				newevent.elements.push(elem);
			}
...
WAF.DataSourceVar.getElement = function(pos, options, userData){
	...
	if ((this.getPosition() == pos) || (this._private.oneElementOnly)) {
		if (successHandler != null) {
			...
			elem._private = {};
			elem._private.currentElem = elem;
			elem._private.keyAtt = this._private.keyAtt;
			elem.getAttributeValue = this.getAttributeValue;
			elem.getKey = this.getKey;
			event.element = elem;
			event.result = elem;
			successHandler(event);
		}
