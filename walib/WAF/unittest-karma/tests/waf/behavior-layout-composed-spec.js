...
moduleDescribe('waf-behavior/layout/composed', function() {
    ...
    describe('.addAliasProperty(aliasName, part, [name])', function() {
        var W, W2, w;
        beforeEach(function() {
            W2 = Widget.create('W2');
            W2.addProperty('plop');
            W = Widget.create('W');
            W.inherit('waf-behavior/layout/composed');
            W.setPart('toto', W2);
        });
        it('should throw if unknown part', function() {
            expect(function() {
                W.addAliasProperty('plop', 'titi');
            }).to.throw();
        });
        it('should throw if unknown widget class', function() {
            W.setPart('titi');
            expect(function() {
                W.addAliasProperty('plop', 'titi');
            }).to.throw();
        });
        it('should throw if unknown property', function() {
            expect(function() {
                W.addAliasProperty('plip', 'toto');
            }).to.throw();
        });
        it('should throw if property exists', function() {
            W.addProperty('plop');
            expect(function() {
                W.addAliasProperty('plop', 'toto');
            }).to.throw();
        });
        it('should throw if property exists 2', function() {
            W.addProperty('coucou');
            expect(function() {
                W.addAliasProperty('coucou', 'toto', 'plop');
            }).to.throw();
        });
        describe('runtime', function() {
            ...
            it('should have the property', function() {
                expect(w).to.have.a.property('plop').to.be.a('function');
            });
            it('should set the property at init', function() {
                w = new W({ plop: 15 });
                expect(w.plop()).to.equal(15);
            });

            it('should change with the part', function() {
                w.plop(10);
                w.setPart('plop', new W2());
                w.plop(15);
                expect(w.getPart('toto').plop()).to.equal(15);
            });
        });
    });
    /*
    describe('aliased properties runtime', function() {
        testAccessor(function() {
            var W2 = Widget.create('W2');
            W2.addProperty('plop', { defaultValue: 7 });
            W2.addProperty('plip');
    ...
    describe('aliased properties runtime change part', function() {
        testAccessor(function() {
            var W2 = Widget.create('W2');
            W2.addProperty('plop');
            W2.addProperty('plip');
            ...
            w.plop(1);
            w.plip(123);
            w.setPart('toto', new W2({ plop: 7 }));
            return w;
        }, [7, 12, 34]);
        it('should keep onChange handlers', function() {
        });
        it('should refire event', function() {
        });
    });*/
    describe('aliased properties runtime no part', function() {
