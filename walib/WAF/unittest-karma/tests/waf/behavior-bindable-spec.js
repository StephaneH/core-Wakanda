...
moduleDescribe('waf-behavior/bindable', function() {
    ...
    function testValues(name, setup, event, attribute, spyValue, sourceValue, values) {
        describe(name, function() {
            ...
            it('should not fail if called multiple time', function() {
                var subscriber = setup();
                subscriber = setup();
                expect(subscriber).to.be.a('object');
            });
    ...
    describe('#bindDatasourceAttribute', function() {
        ...
        WAF.require('waf-core/formatters').create('tva', {
            format: function(v) { return v * 0.2; },
            unformat: function(v) { return v / 0.2; }
        });
        testValues("('source.attr3|tva', 'toto4')", function() {
            //debugger;
            spy = spy3;
            return c.bindDatasourceAttribute('source.attr3|tva', 'toto4');
        }, 'event4', 'attr3', 17 / 0.2, 13 * 0.2, [[4, 4 * 0.2]]);
    ...
    describe('#bindDatasourceElement(datasource, position)', function() {
        ...
        // FIXME: the test below need a modification of the current datasource implementation to pass
    ...
    describe('#boundDatasourceElement(datasource)', function() {
        var C, c, spy, source, data;
        beforeEach(function() {
            C = Widget.create('C');
            spy = sinon.stub();
        ...
        it('should return null if the datasource is not bound to a position', function() {
            expect(c.boundDatasourceElement(source)).to.be.null;
        });
        it('should return the position', function() {
            c.bindDatasourceElement(source, 1);
            expect(c.boundDatasourceElement(source)).to.equal(1);
        });
        it('should return the position even if the datasource is not bound', function() {
            var source2 = new WAF.DataSourceVar({
                "variableReference": [],
                "data-attributes": 'attr1:string,attr2:string'
            });
            c.bindDatasourceElement(source2, 3);
            expect(c.boundDatasourceElement(source2)).to.equal(3);
        });
    });
